...
if (-not [string]::IsNullOrEmpty($FullVersionString))  {
	...
	if ($FullVersionString -NotMatch " ") {
	    # This version has just one tag. Increment patch number.
	    ...
	    # This version is tagged more than once.
        Write-Host This version is tagged more than once as `"$FullVersionString`". Remove one of the tags and try again.
    }
} else{
