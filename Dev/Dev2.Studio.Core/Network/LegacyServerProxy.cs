...
{
    public class LegacyServerProxy : IEnvironmentConnection, IDisposable
    {
        const int MillisecondsTimeout = 10000;
        readonly Dev2JsonSerializer _serializer = new Dev2JsonSerializer();

        static LegacyServerProxy()
        {
            ServicePointManager.ServerCertificateValidationCallback = ValidateServerCertificate;
        }

        public LegacyServerProxy(Uri serverUri)
        ...
        public LegacyServerProxy(string serverUri, ICredentials credentials, IAsyncWorker worker)
        {
            IsAuthorized = true;
            ...
            HubConnection = new HubConnectionWrapper(uriString) { LegacyCredentials = credentials };
            HubConnection.LegacyError += OnHubConnectionError;
            HubConnection.LegacyClosed += LegacyHubConnectionOnClosed;
            HubConnection.LegacyStateChanged += HubConnectionStateChanged;
            InitializeProxyHubs();
            AsyncWorker = worker;
        }

		void LegacyHubConnectionOnClosed() => HasDisconnected();

		public IPrincipal Principal { get; private set; }

        public LegacyServerProxy(string webAddress, string userName, string password)
        ...
        void HasDisconnected()
        {
            ...
            if (HubConnection.LegacyState != ConnectionStateWrapped.Disconnected)
            {
        ...
        public bool IsConnected 
        {
            get
            {
                if (HubConnection != null) return HubConnection.LegacyState == ConnectionStateWrapped.Connected;
                return isConnected;
            }
            set
            {
        ...
        {
            ID = id;
            return HubConnection.LegacyEnsureConnected(timeout).ContinueWith((task) => { return !task.IsFaulted && task.IsCompleted; });
        }
        ...
        public void Disconnect()
        {
            ...
            try
            {
                ...
                IsShuttingDown = true;
                HubConnection.LegacyStop(new TimeSpan(0, 0, 0, 5));
            }
            catch (AggregateException aex)
            {
                aex.Flatten();
        ...
        public void Verify(Action<ConnectResult> callback, bool wait)
        {
            ...
            {
                callback?.Invoke(
                    HubConnection.LegacyState == (ConnectionStateWrapped)HubConnectionState.Connected
