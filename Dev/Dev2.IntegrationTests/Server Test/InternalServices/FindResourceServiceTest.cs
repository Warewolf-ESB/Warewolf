...
namespace Dev2.Integration.Tests.Dev2.Application.Server.Tests.InternalServices
{
    ...
    public class FindResourceServiceTest
    {
        ...
        public void ExecutionWithNoStartNode_ExpectedInvalidValidResult()
        {
            ...
            Assert.IsTrue(result.Contains("The workflow must have at least one service or activity connected to the Start Node."), result + " does not contain string: \"The workflow must have at least one service or activity connected to the Start Node.\"");
            Assert.IsTrue(result.Contains(GlobalConstants.NoStartNodeError), result + " does not contain string: \"" + GlobalConstants.NoStartNodeError + "\"");
        }
