...
Scenario: Sending Error in error variable and calling webservice
    ...
    And call the web service "http://tst-ci-remote.premier.local:3142/services/ONERROR/OnError_WriteToFile.xml?errorLog=[[error]]"
    When the data split tool is executed
    Then the execution has "AN" error
    And the result from the web service "http://tst-ci-remote.premier.local:3142/services/ONERROR/OnError_ReadFromFile.xml" will have the same data as variable "[[error]]"
    And the debug inputs as
	| String to Split   | Process Direction | Skip blank rows | # |                       | With  | Using | Include | Escape |
