...
namespace Dev2.Tests.Activities.ActivityTests.Redis
{
    ...
    public class RedisActivityTests : BaseActivityTests
    {
        ...
        public void RedisActivity_GetDebugInputs_ShouldReturnInnerActivityOutputs()
        {
            ...
            GenerateSUTInstance(key, hostName, mockResourceCatalog, out Dictionary<string, string> evel, out TestRedisActivity sut, innerActivity, port);
            //----------------------Act--------------------------
        ...
        private static void GenerateSUTInstance(string key, string hostName, Mock<IResourceCatalog> mockResourceCatalog, out Dictionary<string, string> evel, out TestRedisActivity sut, Activity innerActivity, int port)
        {
            evel = new Dictionary<string, string> { { "", "" } };
            var impl = new RedisCacheImpl(hostName, port, "");
