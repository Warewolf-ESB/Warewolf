...
namespace Dev2.Tests.Activities.ActivityTests
{
    ...
    public class SendEmailActivityTests : BaseActivityUnitTest
    {
        ...
        [TestMethod]
        [TestCategory("SendEmail_GetDebugInputOutput")]
        public void SendEmail_GetDebugInputOutput_CorrectResults()
        {
            var emailSourceForTesting = EmailSourceForTesting();
            var mock = new Mock<IEmailSender>();
            mock.Setup(sender =>
        ...
        [TestMethod]
        [TestCategory("SendEmail_GetDebugInputOutput")]
        public void SendEmail_GetDebugInputOutput_FromAccountNotSet_CorrectResults()
        {
            var emailSourceForTesting = EmailSourceForTesting();
            var mock = new Mock<IEmailSender>();
            mock.Setup(sender =>
            ...
            activity.To = "[[CC]]";
            activity.Subject = "[[Subject]]";
            activity.FromAccount = "";
            TestStartNode = new FlowStep
            {
                Action = activity
            };
            TestData = "<root><Subject>SubJectValue</Subject><Body>BodyValue</Body><CC>to.someone@amail.account,to1.someone@amail.account,to.someone1@amail.account;to.someone@amail1.account;to.so2meone@amail.account,,</CC></root>";
            CurrentDl = "<ADL><Subject></Subject><Body></Body><CC></CC></ADL>";
            List<DebugItem> inRes;
            List<DebugItem> outRes;

            var result = CheckActivityDebugInputOutput(activity, "<root><Subject></Subject><Body></Body><CC></CC></root>",
                "<root><Subject>SubJectValue</Subject><Body>BodyValue</Body><CC>to.someone@amail.account,to1.someone@amail.account,to.someone1@amail.account;to.someone@amail1.account;to.so2meone@amail.account,,</CC></root>", out inRes, out outRes);

            // remove test datalist ;)
            DataListRemoval(result.DataListID);

            var fetchResultsList = inRes[0].FetchResultsList();

            Assert.AreEqual(DebugItemResultType.Label, fetchResultsList[0].Type);
            Assert.AreEqual("From Account", fetchResultsList[0].Value);

            Assert.AreEqual(DebugItemResultType.Label, fetchResultsList[1].Type);
            Assert.AreEqual(GlobalConstants.EqualsExpression, fetchResultsList[1].Value);

            Assert.AreEqual(DebugItemResultType.Value, fetchResultsList[2].Type);
            Assert.AreEqual(emailSourceForTesting.UserName, fetchResultsList[2].Value);

            Assert.AreEqual(1, outRes.Count);

            fetchResultsList = outRes[0].FetchResultsList();
            Assert.AreEqual(4, fetchResultsList.Count);
