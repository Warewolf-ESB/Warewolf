    ...
    public class RollbackTo : IEsbManagementEndpoint
    {
        ...
        public StringBuilder Execute(Dictionary<string, StringBuilder> values, IWorkspace theWorkspace)
        {
            ...
            if(!values.ContainsKey("resourceId"))
            {
                ...
                Dev2Logger.Debug(ErrorResource.NoResourceIdSentToServer, "Warewolf Debug");
            }
            else if(!values.ContainsKey("versionNumber") )
            {
                execMessage.HasError = true;
                execMessage.Message = new StringBuilder(ErrorResource.NoVersionNumberSentToServer);
                Dev2Logger.Debug(ErrorResource.NoVersionNumberSentToServer, "Warewolf Debug");
            }
            else
            {
                try
                {
                    ...
                    Dev2Logger.Info($"Rollback to. ResourceId:{guid} Version:{version}", "Warewolf Info");
