...
namespace Dev2.Runtime.ESB.Management.Services
{
    ...
    public class GetDatabaseColumnsForTable : IEsbManagementEndpoint
    {
        ...
        public StringBuilder Execute(Dictionary<string, StringBuilder> values, IWorkspace theWorkspace)
        {
            ...
            Dev2Logger.Log.Info(String.Format("Get Database Columns For Table. Database:{0} Schema:{1} Table{2}",
                database, schema, tableName));
            try
            {
                var dbSource = serializer.Deserialize<DbSource>(database);
                var runtTimedbSource = ResourceCatalog.Instance.GetResource<DbSource>(theWorkspace.ID,
                    dbSource.ResourceID);
                ...
                using (var connection = new SqlConnection(runtTimedbSource.ConnectionString))
                {
                    ...
                    string sql = @"select top 1 * from " + schema.Trim(new[] {'"'}) + "." + tableName.Trim(new[] {'"'});
                    using (var sqlcmd = new SqlCommand(sql, connection))
                    {
                        // force it closed so we just get the proper schema ;)
                        using (SqlDataReader sdr = sqlcmd.ExecuteReader(CommandBehavior.CloseConnection))
                        {
                ...
                if (columnInfo != null)
                {
                    foreach (DataRow row in columnInfo.Rows)
                    {
                        ...
                        bool isNullable = row["AllowDBNull"] is bool && (bool) row["AllowDBNull"];
                        bool isIdentity = row["IsIdentity"] is bool && (bool) row["IsIdentity"];
                        ...
                        int columnLength = row["ColumnSize"] is int ? (int) row["ColumnSize"] : -1;
        ...
        public DynamicService CreateServiceEntry()
        {
            var ds = new DynamicService
            {
                ...
                DataListSpecification =
                    new StringBuilder(
                        "<DataList><Database ColumnIODirection=\"Input\"/><TableName ColumnIODirection=\"Input\"/><Dev2System.ManagmentServicePayload ColumnIODirection=\"Both\"></Dev2System.ManagmentServicePayload></DataList>")
            };
