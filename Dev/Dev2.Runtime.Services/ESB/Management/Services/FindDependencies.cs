...
//            sb.Append("</node>");
            if(dependencies != null)
            {
                dependencies.ToList().ForEach(c => BuildDependency(workspaceID, c.ResourceName, ref sb));
            }
...
//                        return sb.ToString();
        ...
        void BuildDependency(Guid workspaceID, string resourceName,ref StringBuilder sb)
        {
            var internalSb = sb;
            var dependentResource = ResourceCatalog.Instance.GetResource(workspaceID, resourceName);
            var brokeString = dependentResource == null ? "true" : "false";
            internalSb.Append(string.Format("<node id=\"{0}\" x=\"\" y=\"\" broken=\"{1}\">", resourceName, brokeString));
            if(dependentResource != null)
            {
                if(dependentResource.Dependencies != null)
                {
                    dependentResource.Dependencies.ForEach(tree =>
                    {
                        var resource = ResourceCatalog.Instance.GetResource(workspaceID, tree.ResourceName);
                        if(resource != null && resource.Dependencies != null && resource.Dependencies.Any())
                        {
                            BuildDependency(workspaceID, resource.ResourceName, ref internalSb);
                        }
                        else
                        {
                            internalSb.Append(string.Format("<dependency id=\"{0}\" />", tree.ResourceName));
                        }
                    });
                }
            }
            internalSb.Append("</node>");
            internalSb.Append(string.Format("<node id=\"{0}\" x=\"\" y=\"\" broken=\"false\">", resourceName));
            internalSb.Append("</node>");
            if(dependentResource != null)
            {
                if(dependentResource.Dependencies != null)
                {
                    ...
                    {
                        var resource = ResourceCatalog.Instance.GetResource(workspaceID, tree.ResourceName);
                        var brokenString = resource == null ? "true" : "false";
                        internalSb.Append(string.Format("<node id=\"{0}\" x=\"\" y=\"\" broken=\"{1}\">", tree.ResourceName, brokenString));
                        internalSb.Append("</node>");
                    });
                }
            }
