...
namespace Dev2.Runtime.ESB.Management.Services
{
    ...
    public class RenameResourceCategory : IEsbManagementEndpoint
    {
        public StringBuilder Execute(Dictionary<string, StringBuilder> values, IWorkspace theWorkspace)
        {
            ...
            Dev2Logger.Log.Info(String.Format("Rename Category. Old {0} New {1} Type{2}", oldCategory, newCategory,
                resourceType));
            ResourceCatalogResult saveResult = ResourceCatalog.Instance.RenameCategory(Guid.Empty, oldCategory,
                newCategory);
        ...
        public DynamicService CreateServiceEntry()
        {
            var newDs = new DynamicService
            {
                ...
                DataListSpecification =
                    new StringBuilder(
                        "<DataList><OldCategory ColumnIODirection=\"Input\"/><NewCategory ColumnIODirection=\"Input\"/><ResourceType ColumnIODirection=\"Input\"/><Dev2System.ManagmentServicePayload ColumnIODirection=\"Both\"></Dev2System.ManagmentServicePayload></DataList>")
            };
