...
namespace Dev2.Runtime.Hosting
{
    public class ServerExplorerRepository : IExplorerServerResourceRepository
    {
        ...
        private IExplorerRepositorySync _sync;
        ...
        static ServerExplorerRepository()
        {
            Instance = new ServerExplorerRepository
            {
                ...
                ExplorerItemFactory =
                    new ExplorerItemFactory(ResourceCatalog.Instance, new DirectoryWrapper(),
                        ServerAuthorizationService.Instance),
                Directory = new DirectoryWrapper(),
                VersionRepository =
                    new ServerVersionRepository(new VersionStrategy(), ResourceCatalog.Instance, new DirectoryWrapper(),
        ...
        internal ServerExplorerRepository(IResourceCatalog resourceCatalog, IExplorerItemFactory explorerItemFactory,
            IDirectory directory, IExplorerRepositorySync sync, IServerVersionRepository versionRepository, IFile file)
        {
        ...
        {
            return ExplorerItemFactory.CreateRootExplorerItem(EnvironmentVariables.GetWorkspacePath(workSpaceId),
                workSpaceId);
        }

        public IExplorerItem Load(ResourceType type, Guid workSpaceId)
        {
            return ExplorerItemFactory.CreateRootExplorerItem(type, EnvironmentVariables.GetWorkspacePath(workSpaceId),
                workSpaceId);
        }
        ...
        public IExplorerRepositoryResult RenameFolder(string path, string newPath, Guid workSpaceId)
        {
            try
            {
                ...
                {
                    return new ExplorerRepositoryResult(ExecStatus.NoMatch,
                        "Requested folder does not exist on server. Folder: " + path);
                }
                ResourceCatalogResult resourceCatalogResult = ResourceCatalogue.RenameCategory(workSpaceId, path,
                    newPath);
        ...
        public IExplorerRepositoryResult DeleteItem(IExplorerItem itemToDelete, Guid workSpaceId)
        {
            ...
            switch (itemToDelete.ResourceType)
            {
                ...
                default:
                    ResourceCatalogResult result = ResourceCatalogue.DeleteResource(workSpaceId,
                        itemToDelete.DisplayName, itemToDelete.ResourceType.ToString());
            ...
            switch (itemToAdd.ResourceType)
            {
                case ResourceType.Folder:
                {
                    try
                    {
                        ...
                        {
                            return new ExplorerRepositoryResult(ExecStatus.Fail,
                ...
                default:
                    return new ExplorerRepositoryResult(ExecStatus.Fail,
                        "Only user resources can be added from this repository");
            }
        }
        ...
        public IExplorerRepositoryResult MoveItem(IExplorerItem itemToMove, string newPath, Guid workSpaceId)
        {
            ...
            {
                return new ExplorerRepositoryResult(ExecStatus.Fail,
                    "There is an item that exists with the same name and path");
            }
            MoveVersions(itemToMove, newPath);
            ResourceCatalogResult result = ResourceCatalogue.RenameCategory(workSpaceId, itemToMove.ResourcePath,
                newPath, new List<IResource> {ResourceCatalogue.GetResource(workSpaceId, itemToMove.ResourceId)});
        ...
        {
            return ExplorerItemFactory.CreateRootExplorerItem(type,
                Path.Combine(EnvironmentVariables.GetWorkspacePath(Guid.Empty), filter), Guid.Empty);
        }

        public IExplorerItem Load(string filter)
        {
            return
                ExplorerItemFactory.CreateRootExplorerItem(
                    Path.Combine(EnvironmentVariables.GetWorkspacePath(Guid.Empty), filter), Guid.Empty);
        }

        private IExplorerRepositoryResult RenameExplorerItem(IExplorerItem itemToRename, Guid workSpaceId)
        {
            IEnumerable<IResource> item =
                ResourceCatalogue.GetResourceList(workSpaceId)
                    .Where(
            ...
            ResourceCatalogResult result = ResourceCatalogue.RenameResource(workSpaceId, itemToRename.ResourceId,
                itemToRename.DisplayName);
            return new ExplorerRepositoryResult(result.Status, result.Message);
        }

        private void MoveVersionFolder(string path, string newPath)
        {
            if (Directory.Exists(DirectoryStructureFromPath(path)) &&
                Directory.Exists(DirectoryStructureFromPath(newPath)))
            {
        ...
        public IExplorerRepositoryResult DeleteFolder(string path, bool deleteContents, Guid workSpaceId)
        {
            ...
            {
                return new ExplorerRepositoryResult(ExecStatus.Fail,
                    "Requested folder contains existing valid resources " + path);
            }
            if (path.Trim() == "")
            {
            ...
            try
            {
                List<ResourceCatalogResult> deletedResources = ResourceCatalogue.GetResourceList(workSpaceId)
                    ...
                    .Select(
                        a => ResourceCatalogue.DeleteResource(workSpaceId, a.ResourceName, a.ResourceType.ToString()))
                    .ToList();
        ...
        private void MoveVersions(IExplorerItem itemToMove, string newPath)
        {
