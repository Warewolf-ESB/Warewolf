...
namespace Dev2.Runtime.ServiceModel
{
    public class HubFactory: IHubFactory
	{
		public IHubProxy CreateHubProxy(Connection connection)
		{
			...
			try
			{
				...
				using (var client = new WebClient())
				{
					...
					var hub = new Microsoft.AspNet.SignalR.Client.HubConnection(connectionAddress) { Credentials = client.Credentials };
					hub.Error += exception => { };
					ServicePointManager.ServerCertificateValidationCallback = (sender, certificate, chain, errors) => true;
					var proxy = hub.CreateHubProxy("esb");
					if (!hub.Start().Wait(GlobalConstants.NetworkTimeOut))
					{
						throw new Common.HttpClientException(new HttpResponseMessage(HttpStatusCode.GatewayTimeout));
					}
					return proxy;
				}
			}
		...
		public Microsoft.AspNetCore.SignalR.Client.HubConnection GetHubConnection(Connection connection)
        {
            var serverUser = Common.Utilities.OrginalExecutingUser;
            var principle = serverUser;
