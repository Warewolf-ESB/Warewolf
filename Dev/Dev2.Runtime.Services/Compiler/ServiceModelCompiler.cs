...
namespace Dev2.Runtime.Compiler
{
    ...
    public class ServiceModelCompiler
    {
        ...
        /// <returns></returns>
        public IList<CompileMessageTO> Compile(Guid serviceId, ServerCompileMessageType typeOf,
            StringBuilder beforeAction, StringBuilder afterAction)
        {
            IList<CompileMessageTO> result = new List<CompileMessageTO>();

            // fetch rules for this service type ;)
            IEnumerable<IServiceCompileRule> ruleSet = _ruleRepo.FetchRulesFor(typeOf);

            if (ruleSet != null)
            {
                foreach (IServiceCompileRule rule in ruleSet)
                {
                    CompileMessageTO msg = rule.ApplyRule(serviceId, beforeAction, afterAction);
