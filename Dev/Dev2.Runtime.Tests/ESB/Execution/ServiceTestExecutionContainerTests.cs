...
namespace Dev2.Tests.Runtime.ESB.Execution
{
    ...
    public class ServiceTestExecutionContainerTests
    {
        ...
        [TestMethod]
        [Owner("Nkosinathi Sangweni")]
        public void Execute_GivenStopExecutionAndUnAuthorized_ShouldAddFailureMessageWithResourceName()
        {
            //---------------Set up test pack-------------------
            var resourceId = Guid.NewGuid();
            const string Datalist = "<DataList><scalar1 ColumnIODirection=\"Input\"/><persistantscalar ColumnIODirection=\"Input\"/><rs><f1 ColumnIODirection=\"Input\"/><f2 ColumnIODirection=\"Input\"/></rs><recset><field1/><field2/></recset></DataList>";
            ...
            mock.Setup(resource => resource.ResourceName).Returns(helloWorld);
            resourceCat.Setup(catalog => catalog.GetResource(GlobalConstants.ServerWorkspaceID, It.IsAny<Guid>())).Returns(mock.Object);
            var dsfObj = new Mock<IDSFDataObject>();
            dsfObj.SetupProperty(o => o.ResourceID);
            const string TestName = "test1";
            dsfObj.Setup(o => o.TestName).Returns(TestName);
            ...
            var format = string.Format(Warewolf.Resource.Messages.Messages.Test_NotAuthorizedMsg, helloWorld);
            dsfObj.Setup(o => o.Environment.FetchErrors()).Returns(format);
            ...
            try
            {
                ...
                Assert.IsNotNull(serviceTestModelTO, "Execute results Deserialize returned Null.");
                resourceCat.Verify(catalog => catalog.GetResource(GlobalConstants.ServerWorkspaceID, It.IsAny<Guid>()));
                var count = dsfObj.Object.Environment.AllErrors.Count;
                Assert.AreEqual(0, count);
                var debugCount = serviceTestModelTO.Result.DebugForTest.Count;
