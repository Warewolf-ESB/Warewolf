...
namespace Microsoft.Win32.TaskScheduler
{
	...
	public sealed class TaskEvent : IComparable<TaskEvent>
	{
        ...
        private void ParseXML(string toXml)
        {
            XDocument doc = XDocument.Parse(toXml);
            var desc = from a in doc.Descendants()
                       select a;
            var xElements = desc as IList<XElement> ?? desc.ToList();
            if (xElements.Any(a => a.Name.LocalName == "EventID"))
            {
                var eventId = xElements.First(a => a.Name.LocalName == "EventID").Value;
                switch (eventId)
                {
                    case "102":
                        TaskCategory = "Task Completed";
                        break;
                    case "100":
                        TaskCategory = "Task Started";
                        break;
                    case "101":
                        TaskCategory = "Failed To Start";
                        break;
                    case "103":
                        TaskCategory = "Job Failed";
                        break;
                    case "104":
                        TaskCategory = "Logon Failed";
                        break;
                }
            }
            if (xElements.Any(a => a.Name.LocalName == "Correlation"))
            {
                var corr = xElements.FirstOrDefault(a => a.Name.LocalName == "Correlation");
                if (corr != null)
                {
                    var correlation = corr.Attributes("ActivityID").FirstOrDefault();
                    if (correlation != null) this.Correlation = correlation.Value;
                }
            }
        ...
        /// <summary>
        /// Correlation value used for grouping
        /// </summary>
