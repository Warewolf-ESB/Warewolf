...
if ($TestList -eq "") {
	...
	$FullArgsList = " `"" + $SolutionDir + "\Warewolf.SecuritySpecs\bin\Debug\Warewolf.SecuritySpecs.dll`" /logger:trx /Settings:`"" + $TestSettingsFile + "`"" + " /TestCaseFilter:`"TestCategory=ServerPermissionsSecurity`""
} else {
	# Create full VSTest argument string.
...
<AnalyseParams>
	...
	<Output>$PSScriptRoot\ServerPermissionsSecuritySpecsDotCoverOutput.dcvr</Output>
	<Scope>
