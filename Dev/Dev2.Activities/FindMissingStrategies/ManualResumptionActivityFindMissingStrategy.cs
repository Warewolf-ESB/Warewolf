...
namespace Dev2.FindMissingStrategies
{
    ...
    public class ManualResumptionActivityFindMissingStrategy : IFindMissingStrategy
    {
        #region Implementation of ISpookyLoadable<Enum>

        public Enum HandlesType() => enFindMissingType.ManualResumption;
        ...
        public List<string> GetActivityFields(object activity)
        {
            ...
            if (activity is ManualResumptionActivity manualResumptionActivity)
            {
                IFindMissingStrategy strategy;
                enFindMissingType findMissingType;
                var boolAct = manualResumptionActivity.OverrideDataFunc.Handler as DsfNativeActivity<bool>;
                if (boolAct == null)
                {
                    if (manualResumptionActivity.OverrideDataFunc.Handler is DsfNativeActivity<string> stringAct)
                    {
