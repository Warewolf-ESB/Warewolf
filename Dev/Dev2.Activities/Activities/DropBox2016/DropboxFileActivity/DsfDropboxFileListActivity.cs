...
namespace Dev2.Activities.DropBox2016.DropboxFileActivity
{
    ...
    public class DsfDropboxFileListActivity : DsfDropBoxBaseActivity, IEquatable<DsfDropboxFileListActivity>
    {
        ...
        protected override List<string> PerformExecution(Dictionary<string, string> evaluatedValues)
        {
            ...
            Dev2Logger.Warn($"[DropboxList] EnsureSourceLoaded={ensured} CatalogHit={liveSource != SelectedSource} AccessToken={tokenPreview}(len={liveSource.AccessToken?.Length}) RefreshToken={(string.IsNullOrEmpty(liveSource.RefreshToken) ? "MISSING" : "present")} AppKey={(string.IsNullOrEmpty(liveSource.AppKey) ? "MISSING" : "present")}", GlobalConstants.WarewolfInfo);
