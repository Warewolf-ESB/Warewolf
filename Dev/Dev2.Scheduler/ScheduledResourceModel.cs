...
Please contact your Warewolf System Administrator.", resource.WorkflowName));
            ...
            ITaskFolder folder = TaskService.GetFolder(WarewolfFolderPath);
            IDev2TaskDefinition created = CreateNewTask(resource);
        ...
        public IList<IResourceHistory> CreateHistory(IScheduledResource resource)
        {
            ...
            ITaskEventLog evt =
                _factory.CreateTaskEventLog(String.Format("\\{0}\\", _warewolfFolderPath) + resource.Name);
            var groupings =
                from a in
                    evt.Where(
                        x =>
                            !String.IsNullOrEmpty(x.Correlation) && !String.IsNullOrEmpty(x.TaskCategory) &&
            ...
            IList<IResourceHistory> eventList =
                (groupings.OrderBy(a => a.StartDate)
                    .Reverse()
                    ...
                    .Select(
                        a =>
                            new ResourceHistory("", CreateDebugHistory(DebugHistoryPath, a.Key),
                                new EventInfo(a.StartDate.Value,
                                    a.StartDate.HasValue && a.EndDate.HasValue
                                        ? a.EndDate.Value.Subtract(a.StartDate.Value)
                                        : TimeSpan.MaxValue, a.EndDate.Value,
                                    GetRunStatus(a.EventId, DebugHistoryPath, a.Key), a.Key,
                                    a.EventId < 103 ? "" : _taskStates[a.EventId]), GetUserName(DebugHistoryPath, a.Key))
        ...
        private IScheduledResource CreateScheduledResource(IDev2Task arg)
        {
            ...
            DateTime nextDate;
            List<string> output;
            using (IExecAction action = ConvertorFactory.CreateExecAction(arg.Action))
            {
                trigger = arg.Trigger;
            ...
            {
                List<string> split = output.SelectMany(a => a.Split(new[] {NameSeperator})).ToList();
                try
                {
            ...
            throw new InvalidScheduleException(String.Format("Invalid resource found:{0}", arg.Definition.Data));
                // this should not be reachable because isvaliddev2task checks same conditions
        }


        private ScheduleRunStatus GetRunStatus(int eventId, string debugHistoryPath, string key)
        {
        ...
        private bool DebugHasErrors(string debugHistoryPath, string correlationId)
        {
        ...
        private bool DebugHistoryExists(string debugHistoryPath, string correlationId)
        {
        ...
        private IDev2TaskDefinition CreateNewTask(IScheduledResource resource)
        {
            ...
            ITrigger trigger = _factory.SanitiseTrigger(resource.Trigger.Trigger);
