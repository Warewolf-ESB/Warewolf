...
//            else 
                ...
                let evaled = 
                    ...
                    |> (List.fold (+) "")
                if (evaled = start || (not (evaled.Contains("[[")))) then DataString evaled
...
and reduceForCalculate (env : WarewolfEnvironment) (update : int) (langs : string) : string = 
    ...
    | RecordSetExpression a -> 
        ...
        | IndexExpression exp -> 
            ...
            | _ -> 
                sprintf "[[%s(%s).%s]]" a.Name (eval env update (languageExpressionToString exp) |> evalResultToString) 
                    a.Column
        | _ -> lang
