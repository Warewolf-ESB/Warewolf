...
let rec  evalUpdate  (env: WarewolfEnvironment) (lang:string) (update:int) (func: WarewolfAtom->WarewolfAtom) : WarewolfEnvironment=   
    ...
    match buffer with
        ...
        | RecordSetNameExpression x -> let data = env.RecordSets.[x.Name].Data
                                       Map.map (fun a _->   (applyStarToColumn func env {Name=x.Name;Column =a; Index=Star} )) data |> ignore
                                       env
    ...
    if  not (env.RecordSets.ContainsKey recset.Name)       then 
        failwith "invalid recordset"     
            ...
            match recset.Index with
                | IntIndex _     ->  let data = WarewolfDataEvaluationCommon.eval env update  (languageExpressionToString (RecordSetExpression recset)) |> evalResultToString |> DataString |> func
                ...
                | IndexExpression b -> 
                                   ...
                                   match b with 
                                        | WarewolfAtomAtomExpression atom ->
                                                    match atom with
                                                    | Int _ ->  let data = WarewolfDataEvaluationCommon.eval env update   (languageExpressionToString (RecordSetExpression recset)) |> evalResultToString |> DataString |> func
                                                                AssignEvaluation.evalAssign (languageExpressionToString (RecordSetExpression recset))  (data.ToString()) update env 
