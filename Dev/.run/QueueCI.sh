...
if [ "$branch" == "$DefaultBranchName" ]; then
	...
	branch=${branch//\//-}
	JSONDATA=$(curl -s "http://bamboo.opswolf.com/rest/api/latest/plan/WOLF-CI.json?os_username=$BambooUsername&os_password=$BambooPassword&expand=branches&max-result=99")
