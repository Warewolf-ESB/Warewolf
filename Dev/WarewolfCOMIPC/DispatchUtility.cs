...
namespace WarewolfCOMIPC
{
	...
	public static class DispatchUtility
	{
		...
		public static Type GetType(object obj, bool throwIfNotFound)
		{
			...
			var result = GetType((IDispatchInfo)obj, throwIfNotFound);
            return result;
		}
		...
		public static object Invoke(object obj, int dispId, object[] args)
		{
            ...
            var result = Invoke(obj, memberName, args);
            return result;
		}
		...
		public static object Invoke(object obj, string memberName, object[] args)
		{
            ...
            var result = type.InvokeMember(memberName, BindingFlags.InvokeMethod | BindingFlags.GetProperty ,null, obj, args, null);
            return result;
		}
        ...
        static void RequireReference<T>(T value, string name) where T : class
        {
            if (value == null)
            {
        ...
        /// If the method returns false, this value should be ignored.</param>
        /// <returns>True if the member was found and resolved to a DISPID.  False otherwise.</returns>
        static bool TryGetDispId(IDispatchInfo dispatch, string name, out int dispId)
        {
            RequireReference(dispatch, "dispatch");
            ...
            int hr = dispatch.GetDispId(ref iidNull, ref name, 1, LocaleSystemDefault, out dispId);

            const int dispEUnknownname = unchecked((int)0x80020006); //From WinError.h
            const int dispidUnknown = -1; //From OAIdl.idl
            if (hr == SOk)
            {
                result = true;
            }
            else if (hr == dispEUnknownname && dispId == dispidUnknown)
            {
