...
namespace WPF.JoshSmith.ServiceProviders.UI
{
    ...
    public static class UnloadedManager
    {
        ...
        private static void OnIsManagedChanged(DependencyObject depObj, DependencyPropertyChangedEventArgs e)
        {
            var elem = depObj as FrameworkElement;
            if (elem == null)
                return;

            var isManaged = (bool) e.NewValue;
            if (isManaged)
        ...
        private static void OnManagedFrameworkElementUnloaded(object sender, RoutedEventArgs e)
        {
            ...
            var unloadable = elem.DataContext as IUnloadable;
            if (unloadable == null)
                return;
