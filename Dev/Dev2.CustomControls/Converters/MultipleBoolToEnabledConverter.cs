...
namespace Dev2.CustomControls.Converters
{
    public class MultipleBoolToEnabledConverter : IMultiValueConverter
    {
        ...
        {
            bool? selectedTaskIsNull = values[0] as bool?;
            bool? isSaveEnabled = values[1] as bool?;
            bool? isDirty = values[2] as bool?;
            if(!selectedTaskIsNull.GetValueOrDefault())
            {
                return false;
            }
            if(!isSaveEnabled.GetValueOrDefault())
            {
                return false;
            }
            if(!isDirty.GetValueOrDefault())
            {
                return false;
            }
            return true;
