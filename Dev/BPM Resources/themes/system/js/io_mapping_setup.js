...
(function(){
	...
	Dev2ServiceFetch.prototype.extractInputs = function(){
		...
		$(xDoc).find("Input").each(function(){
			...
			var emptyToNull = $(this).attr("EmptyToNull");
			var baseType = $(this).attr("BaseType");
			
			// Travis.Frisinger : 16.10.2012 - Added EmptyToNull
			if(baseType == undefined){
				baseType = "Object";
			}
			// Name + Default Value + Is Required + EmptyToNull + BaseType
			result += name+","+defVal+","+req + ","+ emptyToNull + "," + baseType +"|";
	...
	Dev2ServiceFetch.prototype.extractRawInputs = function(){
		// Inputs
		var xDoc = $.parseXML(_payload);
		var result = "";
...
(function(){
		...
		/* Edit or Create mode*/
		Dev2InputMapper.prototype.isEditMode = function(){
			...
			// use plugin inputs data for rendering
				...
				if(rawPluginInputs.length > 0){
					...
					for(var i = 0; i < inputParts.length; i++){
						...
						if(tmpParts[0].length > 0){
							var disPath = tmpParts[0];
							if(disPath.length >= 25){
								disPath = disPath.substr(0,24) + "...";
							}
							var titleOf = tmpParts[0] + " is of type " + tmpParts[1];
							
							namePayload += "<div class='ioMappingInputRow2'><div class='ioMappingInputCol1' title='" + titleOf +"' >" + disPath +"</div>";
							namePayload += "<div><input id='Dev2IOMappingDefaultValue' type='text' class='ioMappingInputCol2 Dev2IOMappingDefaultValue' title='The default value for this input'/></div>";
							namePayload += "<div><input type='checkbox' id='Dev2IOMappingRequired' class='ioMappingInputCol3 Dev2IOMappingRequired' title='Require this input to be set'/></div>";
							namePayload += "<div><input type='checkbox' id='Dev2IOMappingNull' class='ioMappingInputCol3 Dev2IOMappingNull' title='Convert empty values to NULL'/></div></div>";
						}
					}
					...
					for(var i = 0; i < inputParts.length; i++){
						...
						if(tmpParts[2] == "true"){
							namePayload += "<div><input type='checkbox' id='Dev2IOMappingRequired' class='ioMappingInputCol3 Dev2IOMappingRequired' title='Require this input to be set' checked/></div>";
						}else{
							namePayload += "<div><input type='checkbox' id='Dev2IOMappingRequired' class='ioMappingInputCol3 Dev2IOMappingRequired' title='Require this input to be set' /></div>";
						}
						
						// Travis.Frisinger : 16.10.2012 - Handle the Empty to Null
						if(tmpParts[3] == "true"){
							namePayload += "<div><input type='checkbox' id='Dev2IOMappingNull' class='ioMappingInputCol3 Dev2IOMappingNull' title='Convert empty values to NULL' checked/></div></div>"
						}else{
							namePayload += "<div><input type='checkbox' id='Dev2IOMappingNull' class='ioMappingInputCol3 Dev2IOMappingNull' title='Convert empty values to NULL' /></div></div>"
						}	
					}
					...
					namePayload += "<div class='ioMappingInputRow2'><div class='ioMappingInputCol1'>NA</div><div class='ioMappingInputCol2'>NA</div><div class='ioMappingInputCol3'>NA</div><div class='ioMappingInputCol3'>NA</div></div>";
				}
		...
		Dev2InputMapper.prototype.GeneratePluginDef = function(){
			...
			var inputMapping;
			if(rawPluginInputs.length > 0){
				 inputMapping = generateInputMapping(rawPluginInputs);
			}else{
								
				// mappedPluginInputs
				var parts = mappedPluginInputs.split("|");
				var regions = "";
				for(var i = 0; i < parts.length; i++){
					regions += parts[i].split(",")[0] + ",";
				}
				
				inputMapping = generateInputMapping(regions);
			}
...
function buildNewService(serviceName, sourceName, sourceMethod, typeOf, outputDescription, outputMapping, inputMapping, help, icon, description, tooltip, category, tags ){
	...
	// We need to do futher string op, oh crap....
	outputDescription = outputDescription.substr(46);
	outputDescription = outputDescription.substr(0, (outputDescription.length - 48));
	
	// Dev2System.ManagmentServicePayload - 47 chars
...
function generateInputMapping(inputs){

	var outerParts = inputs.split(",");
	var result = "";
	...
	// gather Empty to Null
	i = 0;
	$(".Dev2IOMappingNull").each(function(){
		var required = $(this).is(":checked");
		if(required){
			emptyToNull[i] = 'true';
		}else{
	...
	for(var i = 0; i < outerParts.length; i++){
		var inputParts = outerParts[i].split(":");
		if(inputParts[0].length > 0){
			result += '<Input Name="'+inputParts[0]+'" Source="' + inputParts[0] + '" ';
			// blank input mapping
			//result += '<Input Name="'+inputParts[i]+'" Source="" ';
			if(defVal[i] != ""){
				result += 'DefaultValue="' + defVal[i] + '" '
			}
			// append the EmptyToNull attribute
			if(emptyToNull[i] != ""){
				result += ' EmptyToNull="true" '
			}else{
				result += ' EmptyToNull="false" '
			}
			// Append input type
			if(inputParts[1] != "" && inputParts[1] != undefined){
				result += ' NativeType="'+inputParts[1] +'"';
			}
			result += '>';
			result += req[i];
			result += "</Input>";
	...
	$(document).ready(function(){
		...
		if(step2Data == ''){
			...
			if(mappingData.length > 0){
				...
				var inputs = fetchObj.extractInputs();
				serviceDef.setDefinedPluginInputs(inputs);
				
			}else{
				// Travis.Frisinger - 24.01.2013 : Valid to not have data for no input methods ;)
