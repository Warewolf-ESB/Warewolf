...
let innerConvert (i : int) (a : WarewolfAtom) = 
    match a with
    | PositionedValue(a, b) -> new RecordSetSearchPayload(a, atomtoString b)
    | b -> new RecordSetSearchPayload(i, atomtoString b)
...
let EvalEnvExpressionToRecordSet (name : string) (update : int) (env : WarewolfEnvironment) =
    ...
    | _ -> raise (new NullValueInVariableException("recordset not found",EvaluationFunctions.languageExpressionToString buffer))
...
let EvalEnvExpressionToArrayTable (name : string) (update : int) (env : WarewolfEnvironment) (throwsifnotexists : bool) =
    ...
    try
        ...
        | _ -> raise (new NullValueInVariableException("recordset not found","recordset"))
    with
