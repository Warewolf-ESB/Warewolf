...
namespace Infragistics.Windows.Design.Editors
{
    ...
    /// </summary>
    public class DALXamDateTimeEditor : DesignerActionList
    {
        #region Constructors
		...
		public DALXamDateTimeEditor(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
			: base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_XamDateTimeEditor") : alternateAdornerTitle;
			...
			DALHelpers.AddAppearancePropertiesGroup(typeof(XamDateTimeEditor), this.Items, true, ref groupSequence);

			#endregion //AppearanceProperties group

			#region DateTimeEditorProperties group

			DesignerActionItemGroup groupDateTimeEditor = new DesignerActionItemGroup(SR.GetString("SmartTag_G_XamDateTimeEditor"), groupSequence++);
			groupDateTimeEditor.IsExpanded = true;
			using (PropertyItemCreator pic = new PropertyItemCreator(typeof(XamDateTimeEditor), groupDateTimeEditor))
			{
				this.Items.Add(pic.GetPropertyActionItem("AllowDropDown"));
				this.Items.Add(pic.GetPropertyActionItem("DropDownButtonDisplayMode"));
			}

			#endregion //DateTimeEditorProperties group

			#region MaskedEditorProperties group

			DALHelpers.AddMaskedEditorPropertiesGroup(typeof(XamDateTimeEditor), this.Items, false, ref groupSequence, SR.GetString("SmartTag_G_GeneralMasked"));
