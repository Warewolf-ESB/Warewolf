...
namespace Infragistics.Windows.Design.Editors
{
    ...
    /// </summary>
    public class DALXamTextEditor : DesignerActionList
    {
        #region Constructors
		...
		public DALXamTextEditor(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
			: base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_XamTextEditor") : alternateAdornerTitle;
			...
			DALHelpers.AddAppearancePropertiesGroup(typeof(XamTextEditor), this.Items, true, ref groupSequence);

			#endregion //AppearanceProperties group

			#region TextEditorProperties group

			DesignerActionItemGroup groupTextEditor	= new DesignerActionItemGroup(SR.GetString("SmartTag_G_XamTextEditor"), groupSequence++);
			groupTextEditor.IsExpanded				= true;
			using (PropertyItemCreator pic = new PropertyItemCreator(typeof(XamTextEditor), groupTextEditor))
			{
				this.Items.Add(pic.GetPropertyActionItem("HorizontalScrollBarVisibility"));
				this.Items.Add(pic.GetPropertyActionItem("TextAlignment"));
				this.Items.Add(pic.GetPropertyActionItem("TextWrapping"));
				this.Items.Add(pic.GetPropertyActionItem("VerticalScrollBarVisibility"));
			}

			#endregion //TextEditorProperties group

			#region TextEditorBaseProperties group

			DALHelpers.AddTextEditorBasePropertiesGroup(typeof(XamTextEditor), this.Items, false, ref groupSequence);

			#endregion //TextEditorBaseProperties group

			#region ValueEditorProperties group

			DALHelpers.AddValueEditorPropertiesGroup(typeof(XamTextEditor), this.Items, false, ref groupSequence);
