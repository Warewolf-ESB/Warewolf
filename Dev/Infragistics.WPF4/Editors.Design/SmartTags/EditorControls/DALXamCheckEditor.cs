...
namespace Infragistics.Windows.Design.Editors
{
    ...
    /// </summary>
    public class DALXamCheckEditor : DesignerActionList
    {
        #region Constructors
		...
		public DALXamCheckEditor(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
            : base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_XamCheckEditor") : alternateAdornerTitle;
			...
			DALHelpers.AddAppearancePropertiesGroup(typeof(XamCheckEditor), this.Items, true, ref groupSequence);

			#endregion //AppearanceProperties group

			#region CheckEditorProperties group

			DesignerActionItemGroup groupCheckEditor	= new DesignerActionItemGroup(SR.GetString("SmartTag_G_XamCheckEditor"), groupSequence++);
			groupCheckEditor.IsExpanded					= true;
			using (PropertyItemCreator pic = new PropertyItemCreator(typeof(XamCheckEditor), groupCheckEditor))
			{
				this.Items.Add(pic.GetPropertyActionItem("IsChecked"));
				this.Items.Add(pic.GetPropertyActionItem("IsThreeState"));
			}

			#endregion //CheckEditorProperties group

			#region ValueEditorProperties group

			DALHelpers.AddValueEditorPropertiesGroup(typeof(XamCheckEditor), this.Items, true, ref groupSequence);
