...
{
    internal class StackedColumnBucketCalculator : StackedBucketCalculator
    {
        internal StackedColumnBucketCalculator(CategorySeriesView view) : base(view) { }

        internal override float[] GetBucket(
            AnchoredCategorySeries series, 
            int index, 
            int sortingIndex,
            Rect windowRect,
            Rect viewportRect,
            CategoryFrame currentFrame)
        {
            ...
            StackedColumnSeries columnSeries = this.View.CategoryModel as StackedColumnSeries;
            ...
            int count = Math.Min(columnSeries.Lows != null ? columnSeries.Lows.Length : 0, columnSeries.Highs != null ? columnSeries.Highs.Length : 0);
            ...
            ScalerParams yParams = new ScalerParams(windowRect, viewportRect, columnSeries.YAxis.IsInverted);

            bucket = new[]
                {
                    currentFrame.Buckets[index - FirstBucket][0],
                    (float)columnSeries.YAxis.GetScaledValue(max, yParams),
                    (float)columnSeries.YAxis.GetScaledValue(min, yParams)
                };
