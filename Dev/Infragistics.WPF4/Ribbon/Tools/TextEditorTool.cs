...
{
	/// <summary>
	/// The TextEditorTool supports editing of text.  
	/// </summary>
    /// <remarks>
    /// <para class="note"><b>Note:</b> since the tool is derived from <see cref="Infragistics.Windows.Editors.XamTextEditor"/> 
	/// the usage semantics of the tool are the same as a XamTextEditor with the addition of certain common tool properties, e.g. <see cref="Caption"/>, <see cref="SmallImage"/>, <see cref="Id"/>, <see cref="KeyTip"/> etc.</para>
    /// </remarks>
    ...
    public class TextEditorTool : XamTextEditor, IRibbonTool
	{
		internal static DependencyProperty[] CommonEditorProperties = 
		{ 
			ValueEditor.FormatProperty, 
			ValueEditor.FormatProviderProperty, 
			ValueEditor.InvalidValueBehaviorProperty,
			ValueEditor.IsReadOnlyProperty, 
			ValueEditor.ValueProperty, 
			ValueEditor.ValueConstraintProperty, 
			ValueEditor.ValueToTextConverterProperty,
			ValueEditor.ValueTypeProperty, 
		};

		internal static DependencyProperty[] TextEditorBaseProperties = 
		{
			TextEditorBase.NullTextProperty,
			XamTextEditor.ValueToDisplayTextConverterProperty,
		};

		#region Constructor

		/// <summary>
		/// Initializes a new instance of a <see cref="TextEditorTool"/> class.
		/// </summary>
		public TextEditorTool()
		{
		}

		static TextEditorTool()
		{
			FrameworkElement.DefaultStyleKeyProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(typeof(TextEditorTool)));
			RibbonGroup.MaximumSizeProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(RibbonKnownBoxes.RibbonToolSizingModeImageAndTextNormalBox));
			RibbonGroup.MinimumSizeProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(RibbonKnownBoxes.RibbonToolSizingModeImageAndTextNormalBox));
			...
			// 
			FrameworkElement.FocusVisualStyleProperty.OverrideMetadata( typeof( TextEditorTool ), new FrameworkPropertyMetadata( new Style( ) ) );

			ToolTipService.ShowOnDisabledProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(KnownBoxes.TrueBox));

			// AS 9/17/09 TFS20559
			EventManager.RegisterClassHandler(typeof(TextEditorTool), AccessKeyManager.AccessKeyPressedEvent, new AccessKeyPressedEventHandler(OnEditorAccessKeyPressed));

            // JJD 4/13/10 - NA2010 Vol 2 - Added support for VisualStateManager
            XamRibbon.IsActiveProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(new PropertyChangedCallback(OnVisualStatePropertyChanged)), XamRibbon.IsActivePropertyKey);
            XamRibbon.LocationProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(new PropertyChangedCallback(OnVisualStatePropertyChanged)), XamRibbon.LocationPropertyKey);
            UIElement.IsEnabledProperty.OverrideMetadata(typeof(TextEditorTool), new FrameworkPropertyMetadata(new PropertyChangedCallback(OnVisualStatePropertyChanged)));
		...
		protected override void OnAccessKey(AccessKeyEventArgs e)
		{
			...
			{
				// AS 9/17/09 TFS20559
				// While the default templates for editor tools would show a mnemonic if 
				// allowed and the caption had one we were not registering an access key 
				// pressed handler nor were we handling the OnAccessKey. In the case of 
				// an editor tool we should put it into edit mode.
				//
				this.StartEditMode();
			}
		}
		...
		/// <summary>
		/// Returns an automation peer that exposes the <see cref="TextEditorTool"/> to UI Automation.
		/// </summary>
		/// <returns>A <see cref="Infragistics.Windows.Automation.Peers.Ribbon.TextEditorToolAutomationPeer"/></returns>
		protected override AutomationPeer OnCreateAutomationPeer()
		{
			return new TextEditorToolAutomationPeer(this);
		}
        ...
        {
            TextEditorTool tool = target as TextEditorTool;
		...
		//
		internal static readonly RoutedEvent ActivatedEvent = MenuTool.ActivatedEvent.AddOwner(typeof(TextEditorTool));
		...
		/// <seealso cref="Cloned"/>
		/// <seealso cref="OnRaiseToolEvent"/>
		public static readonly RoutedEvent ClonedEvent = MenuTool.ClonedEvent.AddOwner(typeof(TextEditorTool));
		...
		/// <seealso cref="OnRaiseToolEvent"/>
		public static readonly RoutedEvent CloneDiscardedEvent = MenuTool.CloneDiscardedEvent.AddOwner(typeof(TextEditorTool));
		...
		// Changed to internal for now since there is no defined use case and they are not consistently raised.
		//
		internal static readonly RoutedEvent DeactivatedEvent = MenuTool.DeactivatedEvent.AddOwner(typeof(TextEditorTool));
		...
		public static readonly DependencyProperty CaptionProperty = RibbonToolHelper.CaptionProperty.AddOwner(typeof(TextEditorTool));
		...
		public string Caption
		{
			get
			{
				return (string)this.GetValue(TextEditorTool.CaptionProperty);
			}
			set
			{
				this.SetValue(TextEditorTool.CaptionProperty, value);
			}
		}
		...
		/// <seealso cref="CaptionProperty"/>
		/// <seealso cref="Caption"/>
		public static readonly DependencyProperty HasCaptionProperty = RibbonToolHelper.HasCaptionProperty.AddOwner(typeof(TextEditorTool));
		...
		public bool HasCaption
		{
			get
			{
				return (bool)this.GetValue(TextEditorTool.HasCaptionProperty);
			}
		}
		...
		public static readonly DependencyProperty HasImageProperty = RibbonToolHelper.HasImageProperty.AddOwner(typeof(TextEditorTool));
		...
		public bool HasImage
		{
			get
			{
				return (bool)this.GetValue(TextEditorTool.HasImageProperty);
			}
		}
		...
		/// </summary>
		/// <seealso cref="Id"/>
		public static readonly DependencyProperty IdProperty = RibbonToolHelper.IdProperty.AddOwner(typeof(TextEditorTool));
		...
		public string Id
		{
			get
			{
				return (string)this.GetValue(TextEditorTool.IdProperty);
			}
			set
			{
				this.SetValue(TextEditorTool.IdProperty, value);
			}
		}
		...
		/// </summary>
		public static readonly DependencyProperty ImageResolvedProperty = RibbonToolHelper.ImageResolvedProperty.AddOwner(typeof(TextEditorTool));
		...
		public ImageSource ImageResolved
		{
			get
			{
				return (ImageSource)this.GetValue(TextEditorTool.ImageResolvedProperty);
			}
		}
		...
		public static readonly DependencyProperty IsActiveProperty = XamRibbon.IsActiveProperty.AddOwner(typeof(TextEditorTool));
		...
		public bool IsActive
		{
			get
			{
				return (bool)this.GetValue(TextEditorTool.IsActiveProperty);
			}
		}
		...
		public static readonly DependencyProperty IsQatCommonToolProperty = RibbonToolHelper.IsQatCommonToolProperty.AddOwner(typeof(TextEditorTool));
		...
		public bool IsQatCommonTool
		{
			get
			{
				return (bool)this.GetValue(TextEditorTool.IsQatCommonToolProperty);
			}
			set
			{
				this.SetValue(TextEditorTool.IsQatCommonToolProperty, value);
			}
		}
		...
		public static readonly DependencyProperty IsOnQatProperty = RibbonToolHelper.IsOnQatProperty.AddOwner(typeof(TextEditorTool));
		...
		public bool IsOnQat
		{
			get
			{
				return (bool)this.GetValue(TextEditorTool.IsOnQatProperty);
			}
		}
		...
		/// </summary>
		/// <seealso cref="KeyTip"/>
		public static readonly DependencyProperty KeyTipProperty = RibbonToolHelper.KeyTipProperty.AddOwner(typeof(TextEditorTool));
		...
		public string KeyTip
		{
			get
			{
				return (string)this.GetValue(TextEditorTool.KeyTipProperty);
			}
			set
			{
				this.SetValue(TextEditorTool.KeyTipProperty, value);
			}
		}
		...
		public static readonly DependencyProperty LargeImageProperty = RibbonToolHelper.LargeImageProperty.AddOwner(typeof(TextEditorTool));
		...
		public ImageSource LargeImage
		{
			get
			{
				return (ImageSource)this.GetValue(TextEditorTool.LargeImageProperty);
			}
			set
			{
				this.SetValue(TextEditorTool.LargeImageProperty, value);
			}
		}
		...
		/// </summary>
		/// <seealso cref="Location"/>
		public static readonly DependencyProperty LocationProperty = XamRibbon.LocationProperty.AddOwner(typeof(TextEditorTool));
		...
		public ToolLocation Location
		{
			get
			{
				return (ToolLocation)this.GetValue(TextEditorTool.LocationProperty);
			}
		}
		...
		/// </summary>
		/// <seealso cref="SizingMode"/>
		public static readonly DependencyProperty SizingModeProperty = RibbonToolHelper.SizingModeProperty.AddOwner(typeof(TextEditorTool));
		...
		public RibbonToolSizingMode SizingMode
		{
			get
			{
				return (RibbonToolSizingMode)this.GetValue(TextEditorTool.SizingModeProperty);
			}
		}
		...
		/// </summary>
		/// <seealso cref="SmallImage"/>
		public static readonly DependencyProperty SmallImageProperty = RibbonToolHelper.SmallImageProperty.AddOwner(typeof(TextEditorTool));
		...
		public ImageSource SmallImage
		{
			get
			{
				return (ImageSource)this.GetValue(TextEditorTool.SmallImageProperty);
			}
			set
			{
				this.SetValue(TextEditorTool.SmallImageProperty, value);
			}
		}
		...
		/// </summary>
		public static readonly DependencyProperty EditAreaWidthProperty = DependencyProperty.Register(
  			"EditAreaWidth",
			typeof( double ),
			typeof( TextEditorTool ),
			new FrameworkPropertyMetadata( (double)100.0, FrameworkPropertyMetadataOptions.AffectsMeasure,
				null ) 
			);
		...
		#region Methods
			...
			#region Internal methods
				...
				// AS 9/17/09 TFS20559
				#region OnEditorAccessKeyPressed
		internal static void OnEditorAccessKeyPressed(object sender, AccessKeyPressedEventArgs e)
		{
			ValueEditor editor = sender as ValueEditor;
			Debug.Assert(editor != null);

			if (editor != null && !e.Handled && e.Scope == null && e.Target == null)
			{
				// note if we mark ourselves as the target then when the editor is in a 
				// menuitem, nothing will happen because the ancestor menuitem will not mark
				// itself as the scope. so to maintain the existing behavior i'm going to 
				// limit the editor's handling of the mnemonic to only when its in the 
				// appmenu footer
				//
				switch(XamRibbon.GetLocation(editor))
				{
				...
				#endregion //OnEditorAccessKeyPressed
		...
		{
			get { return TextEditorToolProxy.Instance; }
		}

		#endregion

		#region TextEditorToolProxy
		/// <summary>
		/// Derived <see cref="RibbonToolProxy"/> for <see cref="TextEditorTool"/> instances
		/// </summary>
		protected class TextEditorToolProxy : RibbonToolProxy<TextEditorTool>
		{
			// AS 5/16/08 BR32980 - See the ToolProxyTests.NoInstanceVariablesOnProxies proxy for details.
			//[ThreadStatic()]
			internal static readonly TextEditorToolProxy Instance = new TextEditorToolProxy();

			#region Bind

			private static DependencyProperty[] _bindProperties = 
			{ 
				XamTextEditor.HorizontalScrollBarVisibilityProperty, 
				XamTextEditor.TextAlignmentProperty, 
				XamTextEditor.TextWrappingProperty, 
				XamTextEditor.VerticalScrollBarVisibilityProperty
			};
			...
			protected override void Bind(TextEditorTool sourceTool, TextEditorTool targetTool)
			{
			...
			protected override ToolMenuItemDisplayMode GetMenuItemDisplayMode(TextEditorTool tool)
			{
...
#region Infragistics Source Cleanup (Region)
			...
			/// </summary>
			/// <remarks>
			/// <para class="body">This method will be called to raise a commmon tool event, e.g. <see cref="TextEditorTool.Cloned"/>, <see cref="TextEditorTool.CloneDiscarded"/>.</para>
			/// <para class="note"><b>Note:</b> the implementation of this method calls a protected virtual method named <see cref="TextEditorTool.OnRaiseToolEvent"/> that simply calls the RaiseEvent method. This allows derived classes the opportunity of adding custom logic.</para>
			/// </remarks>
			...
			/// <seealso cref="ToolClonedEventArgs"/>
			/// <seealso cref="ToolCloneDiscardedEventArgs"/>
			protected override void RaiseToolEvent(TextEditorTool sourceTool, RoutedEventArgs args)
			{
