...
{
    /// <summary>
    /// A collection of <see cref="DateRange"/> instances.
    /// </summary>
    public sealed class CalendarDateRangeCollection : ObservableCollectionExtended<DateRange>
    {
        ...
        #region Constructor
        internal CalendarDateRangeCollection(CalendarBase owner)
        {
        ...
        {
            int disabledDays = (int)this._owner.DisabledDaysOfWeekInternal;
            Debug.Assert(disabledDays != CalendarBase.AllDays);
            ...
            {
                DateRange item = this._combinedRanges[i];
        ...
        private DateTime? GetPreviousAvailableDate(ref DateTime start, ref DateTime end, int index)
        {
            ...
            int startingIndex = Array.BinarySearch(this._combinedRanges, new DateRange(end, end));
        ...
        private void InitializeCombinedRanges()
        {
            ...
            DateRange[] sortedRanges = new DateRange[this.Count];
        ...
        internal void AddAvailableDates(DateRange range, List<DateTime> dates)
        {
            int disabledDays = (int)this._owner.DisabledDaysOfWeekInternal;
            Debug.Assert(disabledDays != CalendarBase.AllDays);
            ...
            CalendarZoomMode minMode = this._owner.MinCalendarModeResolved;
            bool offsetWithAdd = minMode != CalendarZoomMode.Days;

            // make sure the flattened list of dates is up to date
            this.VerifyCombinedRanges();

            int index = Array.BinarySearch(this._combinedRanges, new DateRange(range.Start, range.End));
