...
namespace Infragistics.Controls.Primitives
{
    ...
    /// </summary>
    internal abstract class SelectionStrategyBase
    {
        ...
        {
			CoreUtilities.ValidateNotNull(selectionHost, "selectionHost");
		...
		#region Properties
			...
			#region Public Properties
				...
				#region IsMouseCaptured

		internal bool IsMouseCaptured { get { return _IsMouseCaptured; } }

				#endregion //IsMouseCaptured	
...
#region Infragistics Source Cleanup (Region)
                ...
                return null != _currentSelectableItemRef ? CoreUtilities.GetWeakReferenceTargetSafe(_currentSelectableItemRef) as ISelectableItem : null;
            }
			set 
            {
                // JJD 10/10/14
		...
		{
			get { return PresentationUtilities.IsModifierPressed(ModifierKeys.Control); }
		}
		...
		{
			get { return PresentationUtilities.IsModifierPressed(ModifierKeys.Shift); }
		}
                                                ...
                                                bool controlKeyDown)
        {
            return item != null && item.IsTabStop;
				...
				#region CaptureMouseForRoot

		internal bool CaptureMouseForRoot()
		{
			FrameworkElement root = this._selectionHost.RootElement;
			...
			bool oldIgnore = this.IgnoreNextMouseMove;
			this.IgnoreNextMouseMove = true;

			// JJD 4/7/11 - TFS71647
			// Call CaptureMouse and store whether it was successful
			//_IsMouseCaptured = true;
			_IsMouseCaptured = root.CaptureMouse();
			...
			try
			{
				...
				root.MouseLeftButtonUp += new MouseButtonEventHandler(OnRootMouseLeftButtonUp);
				root.LostMouseCapture += new MouseEventHandler(OnRootLostMouseCapture);

			}
			finally
			{
				this.IgnoreNextMouseMove = oldIgnore;
			}

			return true;
		}

				#endregion //CaptureMouseForRoot	
        ...
        public static bool IsMultiSelectStrategy(SelectionType selectionType)
        {
            switch (selectionType)
            {
                ...
                default:
                case SelectionType.Default:
                    Debug.Assert(false, "Unexpected selection type");
                    return false;
            }
        }   
        ...
        public static bool IsSingleSelectStrategy(SelectionType selectionType)
        {
            switch (selectionType)
            {
				...
				#region OnRootLostMouseCapture

		internal void OnRootLostMouseCapture(object sender, MouseEventArgs e)
		{
			_IsMouseCaptured = false;
			this.UnwireEvents(sender);
		}

				#endregion //OnRootLostMouseCapture	
        ...
        public abstract bool OnMouseLeftButtonDown(ISelectableItem item, System.Windows.Input.MouseEventArgs e);
				...
				#region OnRootMouseLeftButtonUp

		internal void OnRootMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
		{
			_IsMouseCaptured = false;
			this.UnwireEvents(sender);
		}

				#endregion //OnRootMouseLeftButtonUp	
		...
		{
			_IsMouseCaptured = false;
			this.UnwireEvents(this._selectionHost.RootElement);

			// JJD 4/7/11 - TFS71647
			// Make sure root is not null
			//this._selectionHost.RootElement.ReleaseMouseCapture();
			FrameworkElement root = this._selectionHost.RootElement;
       ...
       /// <returns>True is successful.</returns>
		...
		private void UnwireEvents(object sender)
		{
		...
		protected void DragStart(System.Windows.Input.MouseEventArgs mouseEventArgs)
		{
			if (null != this.SelectionHost && null != this.CurrentSelectableItem)
			{
				...
				// Just in case.
				okToDrag = okToDrag && this.IsMouseCaptured;
		...
		{
			return mouseEventArgs.GetPosition(this.SelectionHost.RootElement);
		}
		...
		private static SelectionType? GetSelectionType(SelectionStrategyBase strategy)
		{
			...
			Debug.Assert(false, "Unrecognized selection type:" + (strategy != null ? strategy.ToString() : string.Empty));
			return null;
		} 
		...
		private void SetupHorizontalAutoScrollTimer(double interval)
		{
			...
			{
				this._horizontalAutoScrollTimer			 = new DispatcherTimer();
		...
		private void SetupVerticalAutoScrollTimer(double interval)
		{
			...
			{
				this._verticalAutoScrollTimer = new DispatcherTimer();
		...
		protected class AutoScrollManager
		{
			...
			{
				CoreUtilities.ValidateNotNull(selectionStrategy, "selectionStrategy");
				CoreUtilities.ValidateNotNull(mouseEventArgs, "mouseEventArgs");
			...
			private void Initialize()
			{
				...
				GeneralTransform transform = this._autoScrollInfo.ScrollArea.TransformToVisual(this._selectionStrategy.SelectionHost.RootElement);
				
				this._scrollAreaRect		= new Rect(transform.Transform(new Point(0, 0)),
				...
				this._scrollAreaInnerRect.X += this._selectionStrategy.PixelThresholdForHorizontalAutoScroll;
				this._scrollAreaInnerRect.Y += this._selectionStrategy.PixelThresholdForVerticalAutoScroll;
				this._scrollAreaInnerRect.Width = Math.Max( this._scrollAreaInnerRect.Width - ( 2 * this._selectionStrategy.PixelThresholdForHorizontalAutoScroll), 0);
				this._scrollAreaInnerRect.Height = Math.Max( this._scrollAreaInnerRect.Height - ( 2 * this._selectionStrategy.PixelThresholdForVerticalAutoScroll), 0);
				...
				if (this._timerInterval != -1)
				{
					...
					this._scrollDirection			= mouseThresholdAreaPenetration < 0 ? ScrollDirection.Decrement : ScrollDirection.Increment;
					this._scrollSpeed				= this.GetScrollSpeedFromTimerInterval(this._timerInterval);
					return;
				}


				// If we're here, scrolling is not required.
				this._isScrollingRequired = false;
			}
	...
	/// </summary>
	internal class SelectionStrategySingle : SelectionStrategyBase
	{
		#region Private Members
...
#region Infragistics Source Cleanup (Region)
		...
		protected virtual void SelectItemAtPoint(System.Windows.Input.MouseEventArgs mouseEventArgs)
		{
			...
			if (this.IgnoreNextMouseMove || this.IsMouseCaptured == false)
			{
	...
	/// </summary>
	internal class SelectionStrategyExtended : SelectionStrategyBase
	{
		#region Member Variables
		...
		public override bool OnMouseMove(ISelectableItem item, System.Windows.Input.MouseEventArgs e)
		{
			...
			if (this.IgnoreNextMouseMove || false == this.IsMouseCaptured)
			{
				this.IgnoreNextMouseMove = false;
				return true;
			}
		...
		protected void StartDragDelayTimer()
		{
			...
			this._dragDelayTimer			= new DispatcherTimer();
    ...
    /// </summary>
    internal class SelectionStrategyContiguous : SelectionStrategyExtended
    {
        #region Constructor
	...
	/// </summary>
	internal class SelectionStrategyExtendedAutoDrag : SelectionStrategyExtended
	{
		#region Constructor
	...
	/// </summary>
	internal class SelectionStrategySingleAutoDrag : SelectionStrategySingle
	{
		#region Constructor
