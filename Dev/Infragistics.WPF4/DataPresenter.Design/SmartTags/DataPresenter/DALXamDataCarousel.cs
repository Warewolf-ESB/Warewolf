...
namespace Infragistics.Windows.Design.DataPresenter
{
    ...
    /// </summary>
    public class DALXamDataCarousel : DesignerActionList
    {
        #region Constructors
		...
		public DALXamDataCarousel(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
			: base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_XamDataCarousel") : alternateAdornerTitle;
			...
			#region ViewSettings Group

			DesignerActionItemGroup groupViewSettings	= new DesignerActionItemGroup(SR.GetString("SmartTag_G_ViewSettings"), groupSequence++);
			...
			#endregion //ViewSettings Group

			#region Basic Items

			DALHelpers.AddBasicItems(typeof(XamDataCarousel), this, this.Items, context, modelItem, groupSequence);
