...

{
	...
	internal partial class MetadataStore : IProvideAttributeTable
	{
		...
		private void AddCustomAttributes(AttributeTableBuilder builder)
		{
			...
			builder.AddCallback(typeof(Infragistics.Controls.Editors.XamDateTimeInput), delegate(AttributeCallbackBuilder callbackBuilder)
			{
				callbackBuilder.AddCustomAttributes("AllowDropDown", new TypeConverterAttribute(typeof(NullableBoolConverter)));
				callbackBuilder.AddCustomAttributes("DateValue", new TypeConverterAttribute(typeof(NullableConverter<DateTime>)));
			});
		}
