using System;
using System.Collections.Generic;
using System.Text;
using System.Diagnostics;
using System.Globalization;
using Infragistics.Documents.Excel.Serialization;

namespace Infragistics.Documents.Excel.FormulaUtilities.Tokens
{
	// MD 6/13/12 - CalcEngineRefactor
	#region Old Code

	//#if DEBUG
	//    [DebuggerDisplay( "{GetType().Name,nq}: Type - {whitespaceType}, {numberOfWhitespaceChars}" )]
	//#endif
	//    internal class AttrSpaceToken : AttrTokenBase
	//    {
	//        private byte numberOfWhitespaceChars;
	//        private WhitespaceType whitespaceType;

	//        // MD 10/22/10 - TFS36696
	//        // We don't need to store the formula on the token anymore.
	//        //public AttrSpaceToken( Formula formula )
	//        //    : base( formula ) { }
	//        public AttrSpaceToken()
	//            : base() { }

	//        // MD 10/22/10 - TFS36696
	//        // We don't need to store the formula on the token anymore.
	//        //public AttrSpaceToken( Formula formula , byte numberOfWhitespaceChars, WhitespaceType whitespaceType )
	//        //    : this( formula )
	//        public AttrSpaceToken(byte numberOfWhitespaceChars, WhitespaceType whitespaceType)
	//            : this()
	//        {
	//            this.numberOfWhitespaceChars = numberOfWhitespaceChars;
	//            this.whitespaceType = whitespaceType;
	//        }

	//        // MD 10/22/10 - TFS36696
	//        // We can use the default implementation of this now.
	//        //public override FormulaToken Clone( Formula newOwningFormula )
	//        //{
	//        //    return new AttrSpaceToken( newOwningFormula, this.numberOfWhitespaceChars, this.whitespaceType );
	//        //}

	//        public override void Load( WorkbookSerializationManager manager, BiffRecordStream stream, bool isForExternalNamedReference, ref byte[] data, ref int dataIndex )
	//        {
	//            this.whitespaceType = (WhitespaceType)stream.ReadByteFromBuffer( ref data, ref dataIndex );
	//            this.numberOfWhitespaceChars = stream.ReadByteFromBuffer( ref data, ref dataIndex );
	//        }

	//        // MD 10/22/10 - TFS36696
	//        // To save space, the positionInRecordStream is no longer stored on the token, so it needs to be passed in here.
	//        //public override byte GetSize( BiffRecordStream stream, bool isForExternalNamedReference )
	//        public override byte GetSize(BiffRecordStream stream, bool isForExternalNamedReference, Dictionary<FormulaToken, TokenPositionInfo> tokenPositionsInRecordStream)
	//        {
	//            return 4;
	//        }

	//        // MD 12/22/11 - 12.1 - Table Support
	//        public override bool IsEquivalentTo(FormulaToken comparisonToken,
	//            WorksheetRow sourceRow, short sourceColumnIndex,
	//            WorksheetRow comparisonRow, short comparisonColumnIndex)
	//        {
	//            if (base.IsEquivalentTo(comparisonToken, sourceRow, sourceColumnIndex, comparisonRow, comparisonColumnIndex) == false)
	//                return false;

	//            AttrSpaceToken comparisonAttrSpaceToken = (AttrSpaceToken)comparisonToken;
	//            return
	//                this.numberOfWhitespaceChars == comparisonAttrSpaceToken.numberOfWhitespaceChars &&
	//                this.whitespaceType == comparisonAttrSpaceToken.whitespaceType;
	//        }

	//        // MD 10/22/10 - TFS36696
	//        // To save space, the positionInRecordStream is no longer stored on the token, so it needs to be passed in here.
	//        //protected override void SaveAttr( BiffRecordStream stream, bool isForExternalNamedReference )
	//        protected override void SaveAttr(BiffRecordStream stream, bool isForExternalNamedReference, Dictionary<FormulaToken, TokenPositionInfo> tokenPositionsInRecordStream)
	//        {
	//            stream.Write( (byte)this.whitespaceType );
	//            stream.Write( this.numberOfWhitespaceChars );
	//        }

	//        // MD 10/22/10 - TFS36696
	//        // The token no longer stores the formula, so it needs to be passed into this method, and we can get the source cell from the formula.
	//        //public override string ToString( IWorksheetCell sourceCell, CellReferenceMode cellReferenceMode, CultureInfo culture )
	//        public override string ToString(Formula owningFormula, CellReferenceMode cellReferenceMode, CultureInfo culture)
	//        {
	//            switch ( this.whitespaceType )
	//            {
	//                case WhitespaceType.SpacesBeforeNextToken:
	//                case WhitespaceType.SpacesBeforeOpeningParens:
	//                case WhitespaceType.SpacesBeforeClosingParens:
	//                case WhitespaceType.SpacesFollowEqualitySign:
	//                    return new string( ' ', this.numberOfWhitespaceChars );

	//                default:
	//                    return new string( '\n', this.numberOfWhitespaceChars );
	//            }
	//        }

	//        public byte NumberOfWhitespaceChars
	//        {
	//            get { return this.numberOfWhitespaceChars; }
	//        }

	//        public override AttrType Type
	//        {
	//            get { return AttrType.Space; }
	//        }

	//        public WhitespaceType WhitespaceType
	//        {
	//            get { return this.whitespaceType; }
	//        }
	//    }

	#endregion // Old Code



	internal class AttrSpaceToken : AttrTokenBase
	{
		#region Member Variables

		private byte numberOfWhitespaceChars;
		private WhitespaceType whitespaceType;

		#endregion // Member Variables

		#region Constructor

		public AttrSpaceToken()
			: base() { }

		public AttrSpaceToken(byte numberOfWhitespaceChars, WhitespaceType whitespaceType)
			: this()
		{
			this.numberOfWhitespaceChars = numberOfWhitespaceChars;
			this.whitespaceType = whitespaceType;
		}

		#endregion // Constructor

		#region Base Class Overrides

		#region GetSize

		public override byte GetSize(BiffRecordStream stream, bool isForExternalNamedReference, Dictionary<FormulaToken, TokenPositionInfo> tokenPositionsInRecordStream)
		{
			return 4;
		}

		#endregion // GetSize

		#region IsEquivalentTo

		public override bool IsEquivalentTo(FormulaContext sourceContext, FormulaToken comparisonToken, FormulaContext comparisonContext)
		{
			if (base.IsEquivalentTo(sourceContext, comparisonToken, comparisonContext) == false)
				return false;

			AttrSpaceToken comparisonAttrSpaceToken = (AttrSpaceToken)comparisonToken;
			return
				this.numberOfWhitespaceChars == comparisonAttrSpaceToken.numberOfWhitespaceChars &&
				this.whitespaceType == comparisonAttrSpaceToken.whitespaceType;
		}

		#endregion // IsEquivalentTo

		#region Load

		public override void Load(BiffRecordStream stream, bool isForExternalNamedReference, ref byte[] data, ref int dataIndex)
		{
			this.whitespaceType = (WhitespaceType)stream.ReadByteFromBuffer(ref data, ref dataIndex);
			this.numberOfWhitespaceChars = stream.ReadByteFromBuffer(ref data, ref dataIndex);
		}

		#endregion // Load

		#region SaveAttr

		protected override void SaveAttr(BiffRecordStream stream, bool isForExternalNamedReference, Dictionary<FormulaToken, TokenPositionInfo> tokenPositionsInRecordStream)
		{
			stream.Write((byte)this.whitespaceType);
			stream.Write(this.numberOfWhitespaceChars);
		}

		#endregion // SaveAttr

		#region ToString

		public override string ToString(FormulaContext context, Dictionary<WorkbookReferenceBase, int> externalReferences)
		{
			switch (this.whitespaceType)
			{
				case WhitespaceType.SpacesBeforeNextToken:
				case WhitespaceType.SpacesBeforeOpeningParens:
				case WhitespaceType.SpacesBeforeClosingParens:
				case WhitespaceType.SpacesFollowEqualitySign:
					return new string(' ', this.numberOfWhitespaceChars);

				default:
					return new string('\n', this.numberOfWhitespaceChars);
			}
		}

		#endregion // ToString

		#region Type

		public override AttrType Type
		{
			get { return AttrType.Space; }
		}

		#endregion // Type

		#endregion // Base Class Overrides

		#region NumberOfWhitespaceChars

		public byte NumberOfWhitespaceChars
		{
			get { return this.numberOfWhitespaceChars; }
		}

		#endregion // NumberOfWhitespaceChars

		#region WhitespaceType

		public WhitespaceType WhitespaceType
		{
			get { return this.whitespaceType; }
		}

		#endregion // WhitespaceType
	}
}

#region Copyright (c) 2001-2012 Infragistics, Inc. All Rights Reserved
/* ---------------------------------------------------------------------*
*                           Infragistics, Inc.                          *
*              Copyright (c) 2001-2012 All Rights reserved               *
*                                                                       *
*                                                                       *
* This file and its contents are protected by United States and         *
* International copyright laws.  Unauthorized reproduction and/or       *
* distribution of all or any portion of the code contained herein       *
* is strictly prohibited and will result in severe civil and criminal   *
* penalties.  Any violations of this copyright will be prosecuted       *
* to the fullest extent possible under law.                             *
*                                                                       *
* THE SOURCE CODE CONTAINED HEREIN AND IN RELATED FILES IS PROVIDED     *
* TO THE REGISTERED DEVELOPER FOR THE PURPOSES OF EDUCATION AND         *
* TROUBLESHOOTING. UNDER NO CIRCUMSTANCES MAY ANY PORTION OF THE SOURCE *
* CODE BE DISTRIBUTED, DISCLOSED OR OTHERWISE MADE AVAILABLE TO ANY     *
* THIRD PARTY WITHOUT THE EXPRESS WRITTEN CONSENT OF INFRAGISTICS, INC. *
*                                                                       *
* UNDER NO CIRCUMSTANCES MAY THE SOURCE CODE BE USED IN WHOLE OR IN     *
* PART, AS THE BASIS FOR CREATING A PRODUCT THAT PROVIDES THE SAME, OR  *
* SUBSTANTIALLY THE SAME, FUNCTIONALITY AS ANY INFRAGISTICS PRODUCT.    *
*                                                                       *
* THE REGISTERED DEVELOPER ACKNOWLEDGES THAT THIS SOURCE CODE           *
* CONTAINS VALUABLE AND PROPRIETARY TRADE SECRETS OF INFRAGISTICS,      *
* INC.  THE REGISTERED DEVELOPER AGREES TO EXPEND EVERY EFFORT TO       *
* INSURE ITS CONFIDENTIALITY.                                           *
*                                                                       *
* THE END USER LICENSE AGREEMENT (EULA) ACCOMPANYING THE PRODUCT        *
* PERMITS THE REGISTERED DEVELOPER TO REDISTRIBUTE THE PRODUCT IN       *
* EXECUTABLE FORM ONLY IN SUPPORT OF APPLICATIONS WRITTEN USING         *
* THE PRODUCT.  IT DOES NOT PROVIDE ANY RIGHTS REGARDING THE            *
* SOURCE CODE CONTAINED HEREIN.                                         *
*                                                                       *
* THIS COPYRIGHT NOTICE MAY NOT BE REMOVED FROM THIS FILE.              *
* --------------------------------------------------------------------- *
*/
#endregion Copyright (c) 2001-2012 Infragistics, Inc. All Rights Reserved