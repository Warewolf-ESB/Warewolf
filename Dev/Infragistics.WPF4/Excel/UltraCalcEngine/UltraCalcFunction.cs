...
#region Infragistics Source Cleanup (Region)
	...
	public
		 ...
		 abstract class ExcelCalcFunction
...

	{
		...
		internal bool IsAlwaysDirty
		{
			get
			{
				Function function = this.GetEquivalentExcelFunction();
...
#region Infragistics Source Cleanup (Region)
		...
		internal virtual TokenClass ReturnTokenClass
		{
			get
			{
				Function function = this.GetEquivalentExcelFunction();
		...
		public void PerformEvaluation(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			// This could be a nested evaluation if a function asks another function to evaluate itself for part of its own evaluation, 
			// so perform some pre-evaluation steps before we evaluate the function.
			numberStack.OnBeforeFunctionEvaluation();
			...
			try
			{
				...
				// The number stack will need a reference to the current function being evaluated as well as the number of arguments 
				// being passed in this case. This is so it can properly set the ExpectedTokenClass on the values before they are returned 
				// to the functions.
				numberStack.CurrentFunction = this;
				numberStack.CurrentFunctionArgumentCount = argumentCount;

				// MD 4/10/12 - TFS108678
				// First try to perform the evaluation on a per-element basis if enumerable arguments are passed in for single value 
				// parameters. If that is not necessary, then evaluate the function in the normal way.
				result = this.PerformEvaluationForIntermediateResultArray(numberStack, argumentCount);
				if (result == null)
			...
			finally
			{
				...
				// MD 9/24/08
				// Found while fixing TFS8014
				// Perform some post evaluation steps. This will restore the state of the number stack which was being done here.
				//// MD 7/17/08 - Excel formula solving
				//// Clear any state information sotred on the number stack about this function.
				//numberStack.CurrentFunction = null;
				//numberStack.CurrentFunctionArgumentCount = -1;
				numberStack.OnAfterFunctionEvaluation();
			...
			// If any parameters to the function were errors, we may have just percolated them up. However, if it was a reference that resolved to an 
			// error and this function needs to return a Value type, get the resolved error value from the percolated error. In essence, we should unwrap 
			// the error from the reference that holds it.
			if (this.ReturnTokenClass == TokenClass.Value)
			{
				UltraCalcErrorValue resolvedValue = result.ToErrorValue();
				if (resolvedValue != null)
					result = new UltraCalcValue(resolvedValue);
			}

			// MD 7/25/08 - Excel formula solving
			// Let the value know if it was a reference created on the fly.
			if ( result.IsReference )
			{
				Debug.Assert( 
					this.ReturnTokenClass == TokenClass.Reference, 
					"References should only be returned by functions with a Reference return class." );

				result.IsDynamicReference = true;
			}
...
#region Infragistics Source Cleanup (Region)
			...
			try 
			{
				...
				for (int arg_count=0; arg_count < argumentCount; arg_count++) 
				{
					...
					else
					{
						...
						if (valueArg.IsArray || valueArg.IsArrayGroup)
						{
							ArrayProxy[] arrayGroup = valueArg.ToArrayProxyGroup();
							for (int i = 0; i < arrayGroup.Length; i++)
							{
								foreach (UltraCalcValue nestedValue in arrayGroup[i])
								{
									if (ignoreEmptyValues && nestedValue.IsNull)
										continue;

									if (checkForExceptions && nestedValue.IsError)
									{
										// exit and push an error onto the number stack 
										errorValue = valueArg.ToErrorValue();
										return false;
		...
		protected UltraCalcValue[] GetArguments(UltraCalcNumberStack numberStack, int argumentCount, bool skipEmptyValues)
		{
			...
			return this.GetArguments(numberStack, argumentCount, skipEmptyValues, false);
		}

		// MD 12/2/11 - TFS97046
		// Added a new overload to take a skipHiddenCells parameter.
		/// <summary>
		/// A helper method for extracting the <see cref="UltraCalcValue"/> instances from the stack.
		/// </summary>
		/// <param name="numberStack">Number stack whose values should be popped.</param>
		...
		protected UltraCalcValue[] GetArguments(UltraCalcNumberStack numberStack, int argumentCount, bool skipEmptyValues, bool skipHiddenCells)
		{
...
#region Infragistics Source Cleanup (Region)
		...
		internal void GetArguments(List<UltraCalcValue> list, UltraCalcValue val, bool skipEmptyValues
			...
			// MD 12/2/11 - TFS97046
			// Added a new parameter to specify whether to skip cells whose rows or columns are hidden.
			...
			)
		{
			...
			if ( val.IsReference )
			{
				...
				if ( reference.IsEnumerable )
				{
					foreach ( IUltraCalcReference r in reference.References )
					{
						...
						// MD 12/2/11 - TFS97046
						// If we should skip hidden cells and this reference is a reference to a hidden cell, return.
						if (UltraCalcFunction.ShouldSkipReference(skipHiddenCells, r))
							continue;
				...
				else
				{
					...
					// MD 12/2/11 - TFS97046
					// If we should skip hidden cells and this reference is a reference to a hidden cell, return.
					if (UltraCalcFunction.ShouldSkipReference(skipHiddenCells, reference))
						return;
			...
			{
				// MD 12/1/11 - TFS96113
				// ToArray is deprecated and replaced by ToArrayProxy.
				//this.GetArguments( list, val.ToArray(), skipEmptyValues );
				// MD 12/2/11 - TFS97046
				//this.GetArguments(list, val.ToArrayProxy(), skipEmptyValues);
				this.GetArguments(list, val.ToArrayProxy(), skipEmptyValues, skipHiddenCells);
			}
			else if ( val.IsArrayGroup )
			{
				// MD 12/1/11 - TFS96113
				// ToArrayGroup is deprecated and replaced by ToArrayProxyGroup.
				//UltraCalcValue[][ , ] arrayGroup = val.ToArrayGroup();
				//
				//foreach ( UltraCalcValue[ , ] array in arrayGroup )
				//    this.GetArguments( list, array, skipEmptyValues );
				ArrayProxy[] arrayProxyGroup = val.ToArrayProxyGroup();
				foreach (ArrayProxy arrayProxy in arrayProxyGroup)
				{
					// MD 12/2/11 - TFS97046
					//this.GetArguments(list, arrayProxy, skipEmptyValues);
					this.GetArguments(list, arrayProxy, skipEmptyValues, skipHiddenCells);
				}
			}
		...
		//private void GetArguments(List<UltraCalcValue> list, UltraCalcValue[,] array, bool skipEmptyValues)
		// MD 12/2/11 - TFS97046
		// Added a new parameter to specify whether to skip cells whose rows or columns are hidden.
		//private void GetArguments(List<UltraCalcValue> list, ArrayProxy array, bool skipEmptyValues)
		private void GetArguments(List<UltraCalcValue> list, ArrayProxy array, bool skipEmptyValues, bool skipHiddenCells)
		{
		    ...
		    for ( int row = 0; row < rowCount; row++ )
		    {
				...
				{
					// MD 12/2/11 - TFS97046
					//this.GetArguments( list, array[ column, row ], skipEmptyValues );
					this.GetArguments(list, array[column, row], skipEmptyValues, skipHiddenCells);
				}
		    }
		}

		// MD 12/2/11 - TFS97046
		private static bool ShouldSkipReference(bool skipHiddenCells, IUltraCalcReference reference)
		{
			if (skipHiddenCells)
			{
				CellCalcReference cellCalcReference = ExcelCalcEngine.GetResolvedReference(reference) as CellCalcReference;
				if (cellCalcReference != null && cellCalcReference.IsHidden)
					return true;
			}
		...
		internal static bool IsDateTime(UltraCalcValue value)
		{
			...
			return value.GetResolvedValue( false ) is DateTime; 
		...
		// MD 4/10/12 - TFS108678
		#region DoesParameterAllowIntermediateResultArray
		...
		#endregion // DoesParameterAllowIntermediateResultArray

		// MD 7/25/08 - Excel formula solving
		#region GetEquivalentExcelFunction

		internal Function GetEquivalentExcelFunction()
		{
			string name = this.Name;
			switch ( name.ToLower() )
			{
				// The ERROR.TYPE function needs to be manually mapped because the calc manager doesn't support the '.' char in the 
				// function names (I think) and the function that maps to it was already defined and named ERRORTYPE.
				case "errortype":
		...
		#endregion GetEquivalentExcelFunction
		...
		internal virtual TokenClass GetExpectedParameterClass( int index )
		{
			Function function = this.GetEquivalentExcelFunction();
			...
			return function.GetExpectedParameterClass( index );
		}
		...
		internal static Workbook GetWorkbook(UltraCalcNumberStack numberStack)
		{
			...
			return excelRef == null
				? null
				: excelRef.Workbook;
		}

		#endregion // GetWorkbook

		// MD 4/10/12 - TFS108678
		#region IsSelectorFunction

		internal virtual bool IsSelectorFunction
		{
			get { return false; }
		}

		#endregion // IsSelectorFunction

		// MD 4/10/12 - TFS108678
		#region PerformEvaluationForIntermediateResultArray

		private UltraCalcValue PerformEvaluationForIntermediateResultArray(UltraCalcNumberStack numberStack, int argumentCount)
		{
			bool[] parametersToSplit = new bool[argumentCount];
			bool foundParameterToSplit = false;
			for (int i = 0; i < argumentCount; i++)
			{
				if (this.ShouldSplitParameter(numberStack, i, parametersToSplit))
				{
					foundParameterToSplit = true;
					parametersToSplit[i] = true;
				}
			}

			if (foundParameterToSplit == false)
				return null;

			UltraCalcValue[] parameters = new UltraCalcValue[argumentCount];
			for (int i = argumentCount - 1; i >= 0; i--)
				parameters[i] = numberStack.Pop(false);

			int resultWidth = 1;
			int resultHeight = 1;
			for (int i = 0; i < argumentCount; i++)
			{
				if (parametersToSplit[i] == false)
					continue;

				ArrayProxy array = parameters[i].ToArrayProxyRaw();
				if (array == null)
					continue;
				...
				if (width != 1)
				{
					if (resultWidth != 1 && resultWidth != width)
						return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));
				...
				if (height != 1)
				{
					if (resultHeight != 1 && resultHeight != height)
						return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));
			...
			UltraCalcValue[,] resultArray = new UltraCalcValue[resultWidth, resultHeight];
			...
			for (int columnIndex = 0; columnIndex < resultWidth; columnIndex++)
			{
				for (int rowIndex = 0; rowIndex < resultHeight; rowIndex++)
				{
					for (int i = 0; i < argumentCount; i++)
					{
						UltraCalcValue parameter = parameters[i];
						if (parametersToSplit[i])
						{
							ArrayProxy array = parameter.ToArrayProxyRaw();

							int width = array.GetLength(0);
							int height = array.GetLength(1);
							int x = width == 1 ? 0 : columnIndex;
							int y = height == 1 ? 0 : rowIndex;

							parameter = array[x, y];
						}

						numberStack.Push(parameter);
					}

					resultArray[columnIndex, rowIndex] = this.Evaluate(numberStack, argumentCount);
				}
			}

			return new UltraCalcValue(resultArray);
		}

		#endregion // PerformEvaluationForIntermediateResultArray

		// MD 4/10/12 - TFS108678
		#region ShouldSplitParameter

		private bool ShouldSplitParameter(UltraCalcNumberStack numberStack, int parameterIndex, bool[] parametersToSplit)
		{
			// For the IF and CHOOSE functions, if the first parameter will cause the function to result in an intermediate 
			// array, the arrays from all parameters should be split.
			if (this.IsSelectorFunction && parameterIndex > 0 && parametersToSplit[0])
				return numberStack.GetParameterForCurrentFunction(parameterIndex).IsArrayRaw;

			if (this.GetExpectedParameterClass(parameterIndex) != TokenClass.Value)
				return false;

			UltraCalcValue value = numberStack.GetParameterForCurrentFunction(parameterIndex);
			if (value.IsArrayRaw == false)
				return false;

			bool isCreatedFromRegionReference;
			if (((ExcelCalcFormula)numberStack.FormulaOwner.Formula).IsArrayFormula)
			{
				// For array formulas, we only need to determine if the function allows intermediate results array from 
				// constant arrays. If it allows that, it also allows region references when the formula is an array formula.
				isCreatedFromRegionReference = false;
			}
			else
			{
				isCreatedFromRegionReference = value.IsReference;
			}

			UltraCalcFunctionToken functionToken = numberStack.CurrentFunctionToken;
			while (functionToken != null)
			{
				if (functionToken.Function.DoesParameterAllowIntermediateResultArray(parameterIndex, isCreatedFromRegionReference) == false)
					return false;

				if (functionToken.Function.GetExpectedParameterClass(parameterIndex) != TokenClass.Value)
					return true;

				parameterIndex = functionToken.ArgumentIndexInParentFunction;
				functionToken = functionToken.ParentFunctionToken;
			}

			return false;
		}

		#endregion // ShouldSplitParameter
		...
		internal virtual bool WillParameterAlwaysBeDereferenced( int parameterIndex )
		{
 ...
 abstract class UltraCalcBinaryOperatorBase : BuiltInFunctionBase
	{
		...
		protected override bool DoesParameterAllowIntermediateResultArray(int parameterIndex, bool isCreatedFromRegionReference)
		{
			return true;
		}
		...
		internal override TokenClass GetExpectedParameterClass(int index)
		{
			...
			Utilities.DebugFail("Invalid argument index for the binary operator.");
...
#region Infragistics Source Cleanup (Region)
        ...
        class UltraCalcFunctionIf : BuiltInFunctionBase
	{
		...
		internal override bool IsSelectorFunction
		{
			get { return true; }
		}
		...
		internal override bool WillParameterAlwaysBeDereferenced( int parameterIndex )
		{
			if ( parameterIndex == 1 || parameterIndex == 2 )
				return false;

			return base.WillParameterAlwaysBeDereferenced( parameterIndex );
		}  
        ...
        class UltraCalcFunctionDate : BuiltInFunctionBase
	{
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			if (UltraCalcValue.UseExcelFunctionCompatibility)
			{
				...
				Workbook workbook = UltraCalcFunction.GetWorkbook(numberStack);
				if (workbook != null && workbook.DateSystem == DateSystem.From1904 && year < 1904)
					return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Num));
...
//#endif
				...
				object value = UltraCalcValue.DateTimeToExcelDate(
					...
					// MD 8/26/11 - TFS84363
					//workbook,
					UltraCalcFunction.GetWorkbook(numberStack),
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			if (value.IsError)
			{
				...
				switch (errorValue.Code)
				{
					...
					// Added other error codes to map to all Excel error values
					case UltraCalcErrorCode.Null:
						type = 1;
						break;
...
#region Infragistics Source Cleanup (Region)
        ...
        class UltraCalcFunctionIRR : BuiltInFunctionBase
	{
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			// get the range.  If we don't have an enumerable range, we have a range error;
			UltraCalcValue rangeArg = numberStack.Pop();

			if( rangeArg.IsReference == false || rangeArg.ToReference().IsEnumerable == false ) 
...
#region Infragistics Source Cleanup (Region)
				...
				// loop through all the values in the collection, calculating fElement and fElementPrime for this iteration
				IUltraCalcReferenceCollection ReferenceCollection = rangeArg.ToReference().References;

				foreach( IUltraCalcReference cell in ReferenceCollection ) 
				{
					// ignore what can't be converted to a double
					// SSP 3/7/05 BR02518
					// This is a typo. This was introduced when we started using double.TryParse.
					//
					//if (!cell.Value.ToDouble(out temp))
					if ( ! cell.Value.ToDouble( out element ) )
						continue;
...
valueArg.Workbook,
 ...
 class UltraCalcFunctionAveDev : BuiltInFunctionBase
	...
	// MD 4/2/12 - TFS99854
	#region UltraCalcConditionalFunctionBase

	internal abstract class UltraCalcConditionalFunctionBase : BuiltInFunctionBase
	{
		#region Base Class Overrides
		...
		protected sealed override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			UltraCalcValue funcRangeArg = null;
			if (this.SupportsFuncRangeArg && argumentCount == 3)
				funcRangeArg = numberStack.Pop();

			UltraCalcValue criteriaArg = numberStack.Pop();
			UltraCalcValue rangeArg = numberStack.Pop();
			...
			// MD 4/6/12 - TFS101506
			//CriteriaEvaluator evaluator = new CriteriaEvaluator(criteriaArg.GetResolvedValue());
			CriteriaEvaluator evaluator = new CriteriaEvaluator(numberStack.Culture, criteriaArg.GetResolvedValue());

			if (funcRangeArg != null && (funcRangeArg.IsArrayGroup || funcRangeArg.IsReference == false))
				return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));

			// MD 7/5/12 - TFS112278
			// Moved these to the EvaluateHelper
			//int valueCount = 0;
			//double totalValue = 0.0;

			ArrayProxy rangeArray = rangeArg.ToArrayProxy();

			CellCalcReference rootCellForFuncRange = null;
			if (funcRangeArg != null)
			{
				rootCellForFuncRange = funcRangeArg.ToArrayProxy()[0, 0].Value as CellCalcReference;
				if (rootCellForFuncRange == null)
					return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));
			}

			// MD 7/5/12 - TFS112278
			// Refactored this code into the EvaluateHelper;
			#region Old Code

			//int length0 = rangeArray.GetLength(0);
			//int length1 = rangeArray.GetLength(1);

			//for (int i = 0; i < length0; i++)
			//{
			//    for (int j = 0; j < length1; j++)
			//    {
			//        UltraCalcValue currentTestValue = rangeArray[i, j];

			//        if (currentTestValue.IsError)
			//            continue;

			//        if (evaluator.MeetsCriteria(currentTestValue) == false)
			//            continue;

			//        UltraCalcValue itemToSum = currentTestValue;
			//        if (rootCellForFuncRange != null)
			//        {
			//            int rowIndex = rootCellForFuncRange.Row.Index + j;
			//            short columnIndex = (short)(rootCellForFuncRange.ColumnIndex + i);

			//            Worksheet worksheet = rootCellForFuncRange.Row.Worksheet;
			//            int maxRowIndex = worksheet.Rows.MaxCount - 1;
			//            int maxColumnIndex = worksheet.Columns.MaxCount - 1;
			//            if (maxRowIndex < rowIndex || maxColumnIndex < columnIndex)
			//                continue;

			//            itemToSum = new UltraCalcValue(worksheet.Rows[rowIndex].GetCellCalcReference(columnIndex));
			//            itemToSum.ExpectedTokenClass = currentTestValue.ExpectedTokenClass;
			//            itemToSum.Owner = currentTestValue.Owner;
			//            itemToSum.IsDynamicReference = true;
			//        }

			//        valueCount++;

			//        if (this.ShouldTotalValues)
			//        {
			//            double number;
			//            if (itemToSum.ToDouble(out number))
			//                totalValue += number;
			//        }
			//    }
			//}

			//return this.GetReturnValue(totalValue, valueCount);

			#endregion // Old Code
			EvaluateHelper helper = new EvaluateHelper(this, evaluator, rangeArray);

			if (rootCellForFuncRange == null)
			{
				rangeArray.IterateValues(false, helper.ValueCallback);
			}
			else
			{
				int funcRangeFirstColumnIndex = rootCellForFuncRange.ColumnIndex;
				int funcRangeFirstRowIndex = rootCellForFuncRange.Row.Index;

				Worksheet worksheet = rootCellForFuncRange.Worksheet;
				int maxColumnIndex = worksheet.Columns.MaxCount - 1;
				int funcRangeLastColumnIndex = Math.Min(maxColumnIndex, funcRangeFirstColumnIndex + rangeArray.GetLength(0) - 1);

				int maxRowIndex = worksheet.Rows.MaxCount - 1;
				int funcRangeLastRowIndex = Math.Min(maxRowIndex, funcRangeFirstRowIndex + rangeArray.GetLength(1) - 1);

				WorksheetRegion funcRangeRegionResolved = worksheet.GetCachedRegion(
					funcRangeFirstRowIndex, funcRangeFirstColumnIndex,
					funcRangeLastRowIndex, funcRangeLastColumnIndex);

				ExcelCalcValue funcRangeCalcReference = new UltraCalcValue(funcRangeRegionResolved.CalcReference);
				funcRangeCalcReference.ExpectedTokenClass = funcRangeArg.ExpectedTokenClass;
				funcRangeCalcReference.Owner = funcRangeArg.Owner;
				funcRangeCalcReference.IsDynamicReference = true;

				ArrayProxy funcRangeResolvedArray = funcRangeCalcReference.ToArrayProxy();
				funcRangeResolvedArray.IterateValues(false, helper.ValueCallback);
			}

			return this.GetReturnValue(helper.TotalValue, helper.ValueCount);
		}
		...
		protected abstract UltraCalcValue GetReturnValue(double totalValue, int valueCount);

		protected virtual bool ShouldTotalValues
		{
			get { return true; }
		}

		protected virtual bool SupportsFuncRangeArg 
		{
			get { return true; }
		}

		#region CriteriaEvaluator class

		private class CriteriaEvaluator
		{
			private UltraCalcValue _operand;
			private Operator _operator = Operator.Equals;

			// MD 4/6/12 - TFS101506
			//public CriteriaEvaluator(object criteria)
			public CriteriaEvaluator(CultureInfo culture, object criteria)
			{
				object operand;
				...
				if (String.IsNullOrEmpty(strCriteria) == false)
				{
					...
					switch (strCriteria[0])
					{
						case '=':
							_operator = Operator.Equals;
							strOperand = strCriteria.Substring(1);
							break;

						case '<':
							if (strCriteria.Length > 1)
							{
								switch (strCriteria[1])
								{
									case '>':
										_operator = Operator.NotEquals;
										strOperand = strCriteria.Substring(2);
										break;
						...
						case '>':
							if (strCriteria.Length > 1 && strCriteria[1] == '=')
							{
					...
					double number;
					// MD 4/6/12 - TFS101506
					//if (Double.TryParse(strOperand.Trim(), NumberStyles.Number, CultureInfo.InvariantCulture, out number))
					if (MathUtilities.DoubleTryParse(strOperand.Trim(), culture, out number))
						operand = number;
					else
						operand = strOperand;
				}
				...
				_operand = new UltraCalcValue(operand);
			}

			public bool MeetsCriteria(UltraCalcValue testValue)
			{
				if (testValue.IsNull != _operand.IsNull)
					return false;

				if (testValue.IsString != _operand.IsString)
					return false;

				if (testValue.IsBoolean != _operand.IsBoolean)
					return false;

				int compareResult = UltraCalcValue.CompareTo(testValue, _operand);
				switch (_operator)
				{
					case Operator.Equals: return compareResult == 0;
					case Operator.NotEquals: return compareResult != 0;
					case Operator.GreaterThan: return compareResult > 0;
					case Operator.GreaterThanOrEqual: return compareResult >= 0;
					case Operator.LessThan: return compareResult < 0;
					case Operator.LessThanOrEqual: return compareResult <= 0;

					default:
						Utilities.DebugFail("Unknown Operator: " + _operator);
						return false;
				}
			}
		...
		#endregion // CriteriaEvaluator class

		// MD 7/5/12 - TFS112278
		#region EvaluateHelper class

		private class EvaluateHelper
		{
			private CriteriaEvaluator evaluator;
			private UltraCalcConditionalFunctionBase owner;
			private ArrayProxy rangeArray;
			private double totalValue;
			private int valueCount;

			public EvaluateHelper(UltraCalcConditionalFunctionBase owner, CriteriaEvaluator evaluator, ArrayProxy rangeArray)
			{
				this.owner = owner;
			...
			public void ValueCallback(UltraCalcValue currentValue, int relativeRowIndex, int relativeColumnIndex)
			{
				UltraCalcValue currentTestValue = this.rangeArray[relativeColumnIndex, relativeRowIndex];
				if (currentTestValue.IsError)
					return;

				if (this.evaluator.MeetsCriteria(currentTestValue) == false)
					return;

				this.valueCount++;

				if (this.owner.ShouldTotalValues)
				{
					double number;
					if (currentValue.ToDouble(out number))
			...
			{
				get { return this.valueCount; }
			}
		}

		#endregion // EvaluateHelper class
	}
	#endregion //UltraCalcConditionalFunctionBase

	#region UltraCalcFunctionAddress
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionAddress : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			#region Get arguments from the stack

			UltraCalcValue sheetNameArg = null;

			if ( argumentCount == 5 )
				sheetNameArg = numberStack.Pop();

			UltraCalcValue a1Arg = null;

			if ( argumentCount >= 4 )
				a1Arg = numberStack.Pop();

			UltraCalcValue absNumArg = null;

			if ( argumentCount >= 3 )
				absNumArg = numberStack.Pop();

			UltraCalcValue columnNumArg = numberStack.Pop();
			UltraCalcValue rowNumArg = numberStack.Pop();

			#endregion Get arguments from the stack

			#region Percolate up any errors
			...
			#endregion Percolate up any errors

			#region Get values from parameters

			int rowNum = rowNumArg.ToInt32();
			int columnNum = columnNumArg.ToInt32();
			int absNum = absNumArg == null ? 1 : absNumArg.ToInt32();
			bool a1 = a1Arg == null ? true : a1Arg.ToBoolean();
			string sheetName = sheetNameArg == null ? null : sheetNameArg.ToString();

			#endregion Get values from parameters

			WorkbookFormat format = this.GetWorkbookFormat( numberStack );
			int maxRowNum = Workbook.GetMaxRowCount( format );
			int maxColumnNum = Workbook.GetMaxColumnCount( format );
			...
			#region Determine which numbers are relative
			...
			#endregion Determine which numbers are relative

			CellReferenceMode cellReferenceMode = a1 ? CellReferenceMode.A1 : CellReferenceMode.R1C1;

			string addressString = string.Empty;

			if ( sheetName != null )
				addressString += Utilities.CreateReferenceString( null, sheetName );

			if ( cellReferenceMode == CellReferenceMode.R1C1 )
			{
				if ( relativeRow )
					rowNum++;
			...
			addressString += CellAddress.GetCellReferenceString(
				rowNum - 1, columnNum - 1,
				relativeRow, relativeColumn,
				// MD 4/12/11 - TFS67084
				// Moved away from using WorksheetCell objects.
				//format, null, false, cellReferenceMode );
				// MD 2/20/12 - 12.1 - Table Support
				//format, null, -1, false, cellReferenceMode);
				format, -1, -1, false, cellReferenceMode);

			return new UltraCalcValue( addressString );
		}

		#region GetWorkbookFormat

		private WorkbookFormat GetWorkbookFormat( UltraCalcNumberStack numberStack )
		{
			...
			ExcelCalcFormula formula = formulaOwner.Formula as ExcelCalcFormula;
			...
			return formula.Workbook.CurrentFormat;
		} 

		#endregion GetWorkbookFormat
	...
	#endregion UltraCalcFunctionAddress

	#region UltraCalcFunctionAreas
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionAreas : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			// If a reference was not passed to the function, return a #VALUE! error.
			if ( arg.IsReference == false )
			{
				// Percolate up any errors if this is not a reference
				if ( arg.IsError )
					return arg;
			...
			// Convert reference value to a group of arrays and return the number of arrays.
			// MD 12/1/11 - TFS96113
			// ToArrayGroup is deprecated and replaced by ToArrayProxyGroup.
			//return new UltraCalcValue( arg.ToArrayGroup().Length );
			return new UltraCalcValue(arg.ToArrayProxyGroup().Length);
		}
	...
	#endregion UltraCalcFunctionRow

	// MD 4/2/12 - TFS99854
	#region UltraCalcFunctionAverageIf

	internal class UltraCalcFunctionAverageIf : UltraCalcConditionalFunctionBase
	{
		#region GetReturnValue

		protected override UltraCalcValue GetReturnValue(double totalValue, int valueCount)
		{
			if (valueCount == 0)
				return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Div));

			return new UltraCalcValue(totalValue / valueCount);
		}

		#endregion // GetReturnValue

		#region Name

		public override string Name
		{
			get { return "averageif"; }
		}

		#endregion //Name
	}
	#endregion //UltraCalcFunctionAverageIf

	#region UltraCalcFunctionColumn
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionColumn : BuiltInFunctionBase
	{
		private const int ReferenceErrorIndex = -1;
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			// If no arguments were specified, the 1-based column index of the cell owning the formula should be returned.
			if ( argumentCount == 0 )
			{
				columnNumbers = UltraCalcFunctionColumn.GetColumnNumbersFromReference( numberStack.FormulaOwner );
			}
			else
			{
				UltraCalcValue arg = numberStack.Pop();
				...
				// Get the 1-based column index from the reference.
				columnNumbers = UltraCalcFunctionColumn.GetColumnNumbersFromReference( arg.ToReference() );
			}

			if ( columnNumbers[ 0, 0 ] == UltraCalcFunctionColumn.ReferenceErrorIndex )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			int numColumns = columnNumbers.GetLength( 0 );
			int numRows = columnNumbers.GetLength( 1 );
			UltraCalcValue[ , ] array = new UltraCalcValue[ numColumns, numRows ];

			for ( int y = 0; y < numRows; y++ )
			{
				for ( int x = 0; x < numColumns; x++ )
				{
					Debug.Assert( columnNumbers[ x, y ] > 0, "Invalid column number." );
					array[ x, y ] = new UltraCalcValue( columnNumbers[ x, y ] );
				}
			}

			return new UltraCalcValue( array );
		}

		#region GetColumnNumbersFromReference

		private static int[ , ] GetColumnNumbersFromReference( IUltraCalcReference reference )
		{
			// MD 4/12/11 - TFS67084
			// Moved away from using WorksheetCell objects.
			//IWorksheetCell cell = reference.Context as IWorksheetCell;
			//
			//if ( cell != null )
			//    return new int[ , ] { { cell.ColumnIndex + 1 } };
			CellCalcReference cellReference = reference.Context as CellCalcReference;
			if (cellReference != null)
				return new int[,] { { cellReference.ColumnIndex + 1 } };

			WorksheetRegion region = reference.Context as WorksheetRegion;

			if ( region != null )
				return UltraCalcFunctionColumn.GetColumnNumbersFromRegion( region );
			...
			//List<WorksheetRegion> regions = reference.Context as List<WorksheetRegion>;
			ReadOnlyCollection<WorksheetRegion> regions = reference.Context as ReadOnlyCollection<WorksheetRegion>;

			if ( regions != null )
			{
				// The row index cannot be obtained from a group of multiple arrays.
				if ( regions.Count != 1 )
					return new int[ , ] { { UltraCalcFunctionColumn.ReferenceErrorIndex } };

				return UltraCalcFunctionColumn.GetColumnNumbersFromRegion( regions[ 0 ] );
			}

			Utilities.DebugFail( "Unknown context" );
			return new int[ , ] { { UltraCalcFunctionColumn.ReferenceErrorIndex } };
		}

		#endregion GetColumnNumbersFromReference

		#region GetColumnNumbersFromRegion

		private static int[ , ] GetColumnNumbersFromRegion( WorksheetRegion region )
		{
			int[ , ] rows = new int[ region.Width, region.Height ];
		...
		#endregion GetColumnNumbersFromRegion
	...
	#endregion UltraCalcFunctionColumn

	#region UltraCalcFunctionColumns
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionColumns : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			// MD 12/1/11 - TFS96113
			// ToArrayGroup is deprecated and replaced by ToArrayProxyGroup.
			//UltraCalcValue[][ , ] arrayGroup = arg.ToArrayGroup();
			ArrayProxy[] arrayGroup = arg.ToArrayProxyGroup();

			// The column count cannot be obtained from a group of multiple arrays.
			if ( arrayGroup.Length != 1 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			// Return the column count of the only array in the group.
			return new UltraCalcValue( arrayGroup[ 0 ].GetLength( 0 ) );
		}
	...
	#endregion UltraCalcFunctionColumns

	#region UltraCalcFunctionCorrel
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionCorrel : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			double[ , ] matrix1;
			double[ , ] matrix2;
			UltraCalcFunctionTwoMatrixFunction.GetArguments( numberStack, out error, out matrix1, out matrix2 );
			...
			double total1;
			double total2;
			UltraCalcFunctionTwoMatrixFunction.GetTotals( matrix1, matrix2, out elementCount, out total1, out total2 );

			SumElementCallback1 squareTotalCallback = new SumElementCallback1( delegate( double element )
				{
					return element * element;
				} );

			double squareTotal1 = UltraCalcFunctionTwoMatrixFunction.SumElements( matrix1, squareTotalCallback );
			double squareTotal2 = UltraCalcFunctionTwoMatrixFunction.SumElements( matrix2, squareTotalCallback );

			double ssX = squareTotal1 - ( ( total1 * total1 ) / elementCount );
			double ssY = squareTotal2 - ( ( total2 * total2 ) / elementCount );

			double sumProduct = UltraCalcFunctionTwoMatrixFunction.SumElements( matrix1, matrix2,
				new SumElementCallback2( delegate( double element1, double element2 )
				{
					return element1 * element2;
				} ) );

			double scpXY = sumProduct - ( ( total1 * total2 ) / elementCount );

			if ( ssX == 0 || ssY == 0 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Div ) );

			double final = scpXY / ( Math.Sqrt( ssX ) * Math.Sqrt( ssY ) );

			return new UltraCalcValue( final );
		}
	...
	#endregion UltraCalcFunctionCorrel

	// MD 4/2/12 - TFS99854
	#region UltraCalcFunctionCountIf

	internal class UltraCalcFunctionCountIf : UltraCalcConditionalFunctionBase
	{
		#region GetReturnValue

		protected override UltraCalcValue GetReturnValue(double totalValue, int valueCount)
		{
			return new UltraCalcValue(valueCount);
		}
		...
		#region ShouldTotalValues

		protected override bool ShouldTotalValues
		{
			get { return false; }
		}

		#endregion // ShouldTotalValues

		#region SupportsFuncRangeArg

		protected override bool SupportsFuncRangeArg
		{
			get { return false; }
		}

		#endregion // SupportsFuncRangeArg
	}
	#endregion //UltraCalcFunctionCountIf

	#region UltraCalcFunctionCovar
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionCovar : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			double average1 = total1 / elementCount;
			double average2 = total2 / elementCount;

			double numerator = UltraCalcFunctionTwoMatrixFunction.SumElements( matrix1, matrix2,
				new SumElementCallback2( delegate( double element1, double element2 )
				{
					return ( element1 - average1 ) * ( element2 - average2 );
				} ) );

			return new UltraCalcValue( numerator / elementCount );
		}
	...
	#endregion UltraCalcFunctionCovar

	#region UltraCalcFunctionForecast
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionForecast : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			double[ , ] knownYs;
			double[ , ] knownXs;
			UltraCalcFunctionTwoMatrixFunction.GetArguments( numberStack, out error, out knownYs, out knownXs );

			UltraCalcValue xArg = numberStack.Pop();

			if ( xArg.IsError )
				return xArg;

			double xValue = xArg.ToDouble();

			if ( error != null )
				return new UltraCalcValue( error );

			double knownYAverage;
			double knownXAverage;
			double regressionLineSlope;
			double regressionLineYIntercept;
			UltraCalcFunctionTwoMatrixFunction.GetCompareInfo( 
				knownYs, knownXs, out knownYAverage, out knownXAverage, 
				out regressionLineSlope, out regressionLineYIntercept, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			return new UltraCalcValue( regressionLineYIntercept + ( regressionLineSlope * xValue ) );
		}
	...
	#endregion UltraCalcFunctionForecast

	// MD 7/9/10 - TFS35240
	#region UltraCalcFunctionHLookup
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionHLookup : UltraCalcFunctionLookupBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			UltraCalcValue rangeLookupArg = null;

			if (argumentCount == 4)
				rangeLookupArg = numberStack.Pop();

			UltraCalcValue rowIndexNumArg = numberStack.Pop();
			UltraCalcValue tableArrayArg = numberStack.Pop();
			UltraCalcValue lookupValueArg = numberStack.Pop();
			...
			if (lookupValueArg.IsArray || lookupValueArg.IsArrayGroup)
				return new UltraCalcValue(new UltraCalcErrorValue(ExcelCalcErrorCode.NA));

			// MD 3/22/12 - TFS105673
			// For the DB functions, when the lookup value is null, an #N/A error should be returned.
			if (lookupValueArg.IsNull)
				return new UltraCalcValue(new UltraCalcErrorValue(ExcelCalcErrorCode.NA));

			// MD 3/8/12 - TFS104321
			// Moved this to the LookupValue method called below.
			//bool lookupValueIsString = lookupValueArg.IsString;
			//string lookupValueString = lookupValueIsString ? lookupValueArg.ToString() : null;
			//bool lookupValueIsBoolean = lookupValueArg.IsBoolean;
			//bool lookupValueBoolean = lookupValueIsBoolean ? lookupValueArg.ToBoolean() : false;

			//double lookupValueNumber = 0;
			//if (lookupValueIsBoolean == false && lookupValueIsString == false)
			//    lookupValueArg.ToDouble(out lookupValueNumber);

			// MD 3/8/12 - TFS104325
			// This is wrong. If the lookup value is a single cell, we should search just that cell.
			//if (tableArrayArg.IsArray == false)
			//    return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));

			// MD 12/1/11 - TFS96113
			// ToArray is deprecated and replaced by ToArrayProxy.
			//UltraCalcValue[,] tableArray = tableArrayArg.ToArray();
			ArrayProxy tableArray = tableArrayArg.ToArrayProxy();

			double rowIndexNum;
			if (rowIndexNumArg.ToDouble(out rowIndexNum) == false || rowIndexNum < 1)
				return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));

			int rowIndex = (int)Math.Floor(rowIndexNum) - 1;

			// MD 3/8/12 - TFS104321
			// Moved this logic to the base TryParseRangeLookupArg method.
			#region Moved
			...
			//        rangeLookup = rangeLookupArg.ToBoolean();
			//    else if (rangeLookupArg.IsDateTime)
			//        rangeLookup = true;
			//    else
			//    {
			//        double result;
			//        if (rangeLookupArg.ToDouble(out result))
			...
			#endregion // Moved
			bool rangeLookup;
			if (UltraCalcFunctionLookupBase.TryParseRangeLookupArg(rangeLookupArg, out rangeLookup) == false)
				return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));
			...
			//if (lookupValueIsString && rangeLookup == false)
			//    matchRegex = UltraCalcFunctionLookupBase.ConvertLookupValueToRegex(lookupValueString);

			#region Search the table for the correct column

			int columnCount = tableArray.GetLength(0);

			// MD 3/8/12 - TFS104321
			// Moved this logic to the base LookupValue method.
			#region Moved

			//int matchingColumn = rangeLookup ? columnCount - 1 : -1;

			//// MD 12/1/11 - TFS96113
			//// Instead of iterating all cells in the lookup region, only iterate the already created cells in the region.
			//// This could drastically improve performance when a large range is specified but only a small region in that range
			//// has data.
			////for (int columnIndex = 0; columnIndex < columnCount; columnIndex++)
			////{
			////    UltraCalcValue firstValueInColumn = tableArray[columnIndex, 0];
			//foreach (KeyValuePair<int, UltraCalcValue> pair in tableArray.GetIterator(1, 0))
			//{
			//    int columnIndex = pair.Key;
			//    UltraCalcValue firstValueInColumn = pair.Value;

			//    int? matchingColumnTemp = UltraCalcFunctionLookupBase.MatchValue(
			//        firstValueInColumn,
			//        columnIndex,
			//        lookupValueIsString,
			//        lookupValueString,
			//        lookupValueIsBoolean,
			//        lookupValueBoolean,
			//        lookupValueNumber,
			//        matchRegex,
			//        // MD 3/2/12 - 12.1 - Table Support
			...
			//    if (matchingColumnTemp.HasValue)
			//    {
			//        matchingColumn = matchingColumnTemp.Value;
			//        break;
			//    }
			//}

			#endregion // Moved
			int matchingColumn = UltraCalcFunctionLookupBase.LookupValue(
				tableArray.GetIterator(1, 0),
				lookupValueArg, 
				rangeLookup ? 1 : 0);

			#endregion // Search the table for the correct column

			if (matchingColumn < 0 || columnCount <= matchingColumn)
				return new UltraCalcValue(new UltraCalcErrorValue(ExcelCalcErrorCode.NA));

			int rowCount = tableArray.GetLength(1);
			if (rowCount <= rowIndex)
				return new UltraCalcValue(new UltraCalcErrorValue(ExcelCalcErrorCode.Reference));

			UltraCalcValue returnValue = tableArray[matchingColumn, rowIndex];
			return new UltraCalcValue(returnValue.GetResolvedValue());
		}
	...
	#endregion UltraCalcFunctionHLookup

	#region UltraCalcFunctionHyperlink
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionHyperlink : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			UltraCalcValue friendlyNameArg = null;

			if ( argumentCount == 2 )
				friendlyNameArg = numberStack.Pop();

			UltraCalcValue linkLocationArg = numberStack.Pop();
			...
			string linkLocation = linkLocationArg.ToString();
			string friendlyName = friendlyNameArg == null ? linkLocation : friendlyNameArg.ToString();

			return new UltraCalcValue( friendlyName );
		}
	...
	#endregion UltraCalcFunctionHyperlink

	#region UltraCalcFunctionIndex
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionIndex : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			UltraCalcValue areaNumArg = null;

			if ( argumentCount == 4 )
				areaNumArg = numberStack.Pop();

			UltraCalcValue columnNumArg = null;

			if ( argumentCount >= 3 )
				columnNumArg = numberStack.Pop();

			UltraCalcValue rowNumArg = numberStack.Pop();
			UltraCalcValue referenceArg = numberStack.Pop(); 

			#endregion Get arguments from the stack

			#region Percolate up any errors

			// Only percolate up this error if it is not a reference
			...
			// For some reason, string and boolean parameters cannot be converted to an array of values when using
			// this functions. If one is specified as the first parameter, it should return a #VALUE! error.
			// MD 4/5/12 - TFS100994
			// If the parameter is a cell reference which contains a string or boolean, we should not return a #VALUE! error.
			...
			// Determine the row, column, and area index based on the parameters specified.
			int rowIndex = rowNumArg.ToInt32() - 1;
			int columnIndex = ( columnNumArg == null ? 1 : columnNumArg.ToInt32() ) - 1;
			int areaIndex = ( areaNumArg == null ? 1 : areaNumArg.ToInt32() ) - 1;

			// Convert the value to an array group if it is not already one
			// MD 12/1/11 - TFS96113
			// ToArrayGroup is deprecated and replaced by ToArrayProxyGroup.
			//UltraCalcValue[][ , ] arrayGroup = referenceArg.ToArrayGroup();
			ArrayProxy[] arrayGroup = referenceArg.ToArrayProxyGroup();

			// If the array index is out of range, return a #REF! error.
			if ( arrayGroup.Length <= areaIndex )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			// Get the array where the value exists
			// MD 12/1/11 - TFS96113
			// Instead of storing arrays for rectangular regions of values we now use an ArrayProxy.
			//UltraCalcValue[ , ] array = arrayGroup[ areaIndex ];
			ArrayProxy array = arrayGroup[areaIndex];

			int columnCount = array.GetLength( 0 );
			int rowCount = array.GetLength( 1 );

			// If only the row index is specified and there are multiple columns in the reference...
			if ( columnNumArg == null && columnCount > 1 )
			{
				// ... and there is only one row in the reference, the row index should instead be used as a column index.
				if ( rowCount == 1 )
				{
				...
				{
					// Otherwise, a #REF! error should be returned.
					return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );
				}
			}

			// If either the row of column indices are out of range, return a #REF! error.
			if ( columnCount <= columnIndex || rowCount <= rowIndex )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			// Return the approprate value in the array.
	...
	#endregion UltraCalcFunctionIndex

	#region UltraCalcFunctionIndirect
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionIndirect : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			string reference = referenceArg.ToString();
			bool a1 = a1Arg == null ? true : a1Arg.ToBoolean();

			CellReferenceMode cellReferenceMode = a1 ? CellReferenceMode.A1 : CellReferenceMode.R1C1;

			ExcelRefBase formulaOwner = ExcelCalcEngine.GetResolvedReference( numberStack.FormulaOwner ) as ExcelRefBase;

			if ( formulaOwner == null )
				return new UltraCalcValue( ExcelReferenceError.Instance );

			// MD 4/12/11 - TFS67084
			// Moved away from using WorksheetCell objects.
			//IUltraCalcReference referenceValue = CalcUtilities.GetReference( reference, formulaOwner.Cell, formulaOwner.Worksheet, formulaOwner.Workbook, cellReferenceMode );
			IUltraCalcReference referenceValue = CalcUtilities.GetReference(reference, 
				formulaOwner.Row, formulaOwner.ColumnIndex, 
				formulaOwner.Worksheet, 
				formulaOwner.Workbook, 
				cellReferenceMode);

			if ( referenceValue is UltraCalcReferenceError )
				referenceValue = ExcelReferenceError.Instance;

			return new UltraCalcValue( referenceValue );
		}
	...
	#endregion UltraCalcFunctionIndirect

	#region UltraCalcFunctionIntercept
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionIntercept : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			double regressionLineSlope;
			double regressionLineYIntercept;
			UltraCalcFunctionTwoMatrixFunction.GetCompareInfo( numberStack, 
				out regressionLineSlope, out regressionLineYIntercept, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			return new UltraCalcValue( regressionLineYIntercept );
		}
	...
	#endregion UltraCalcFunctionIntercept

	// MD 10/14/10 - TFS48845
	#region UltraCalcFunctionLookup
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionLookup : UltraCalcFunctionLookupBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			UltraCalcValue resultVectorArg = null;

			if (argumentCount == 3)
				resultVectorArg = numberStack.Pop();

			UltraCalcValue lookupArrayArg = numberStack.Pop();
			UltraCalcValue lookupValueArg = numberStack.Pop();
			...
			// MD 12/1/11 - TFS96113
			// ToArray is deprecated and replaced by ToArrayProxy.
			//UltraCalcValue[,] lookupArray = lookupArrayArg.ToArray();
			ArrayProxy lookupArray = lookupArrayArg.ToArrayProxy();

			int columnCount = lookupArray.GetLength(0);
			int rowCount = lookupArray.GetLength(1);

			bool searchInFirstRow = (rowCount < columnCount);
			int maxItemCount = searchInFirstRow ? columnCount : rowCount;

			// MD 12/1/11 - TFS96113
			// Instead of storing arrays for rectangular regions of values we now use an ArrayProxy.
			//UltraCalcValue[,] resultArray = null;
			ArrayProxy resultArray = null;

			int resultColumnCount = 0;
			int resultRowCount = 0;
			bool searchInResultRow = false;

			if (resultVectorArg != null)
			{
				// MD 12/1/11 - TFS96113
				// ToArray is deprecated and replaced by ToArrayProxy.
				//resultArray = resultVectorArg.ToArray();
			...
			#region Search the table for the correct row

			// MD 3/8/12 - TFS104321
			// Moved this logic to the base LookupValue method.
			#region Moved

			//int matchingItemIndex = maxItemCount - 1;

			//// MD 12/1/11 - TFS96113
			//// Instead of iterating all columns/rows in the lookup region, only iterate the already created columns/rows in the region.
			//// This could drastically improve performance when a large range is specified but only a small region in that range
			//// has data.
			////for (int itemIndex = 0; itemIndex < maxItemCount; itemIndex++)
			////{
			////    UltraCalcValue value = searchInFirstRow 
			////        ? lookupArray[itemIndex, 0] 
			////        : lookupArray[0, itemIndex];
			//foreach (KeyValuePair<int, UltraCalcValue> pair in lookupArray.GetIterator(searchInFirstRow ? 1 : 0, 0))
			//{
			//    int itemIndex = pair.Key;
			//    UltraCalcValue value = pair.Value;
			...
			//    if (matchingColumnTemp.HasValue)
			//    {
			//        matchingItemIndex = matchingColumnTemp.Value;
			//        break;
			//    }
			//}

			#endregion // Moved
			int matchingItemIndex = UltraCalcFunctionLookupBase.LookupValue(
				lookupArray.GetIterator(searchInFirstRow ? 1 : 0, 0),
				lookupValueArg,
				1);

			#endregion // Search the table for the correct row

			if (matchingItemIndex < 0 || maxItemCount <= matchingItemIndex)
				return new UltraCalcValue(new UltraCalcErrorValue(ExcelCalcErrorCode.NA));

			UltraCalcValue returnValue;
			if (resultArray != null)
			{
				if (searchInResultRow)
				{
					...
					returnValue = resultArray[matchingItemIndex, 0];
				}
				else
				{
					...
					returnValue = resultArray[0, matchingItemIndex];
				}
			}
			else
			{
				if (searchInFirstRow)
					returnValue = lookupArray[matchingItemIndex, rowCount - 1];
				else
					returnValue = lookupArray[columnCount - 1, matchingItemIndex];
			}
	...
	#endregion UltraCalcFunctionLookup

	// MD 7/9/10 - TFS35240
	#region UltraCalcFunctionLookupBase
	...
	internal abstract class UltraCalcFunctionLookupBase : BuiltInFunctionBase
	{
		// MD 3/8/12 - TFS104321
		#region CompareValues

		private static int? CompareValues(
			UltraCalcValue testValue,
			bool lookupValueIsString,
			string lookupValueString,
			bool lookupValueIsBoolean,
			bool lookupValueBoolean,
			double lookupValueNumber,
			Regex matchRegex)
		{
			if (lookupValueIsString)
			{
				if (testValue.IsString == false)
					return null;
				...
				{
					// MD 4/6/12 - TFS101506
					//return string.Compare(testValueString, lookupValueString, StringComparison.CurrentCultureIgnoreCase);
					return string.Compare(testValueString, lookupValueString, testValue.Culture, CompareOptions.IgnoreCase);
				}

				Match match = matchRegex.Match(testValueString);
			...
			if (lookupValueIsBoolean)
			{
				...
				return testValue.ToBoolean().CompareTo(lookupValueBoolean);
			}

			double testValueNumber;
			if (testValue.ToDouble(out testValueNumber) == false)
				return null;

			return testValueNumber.CompareTo(lookupValueNumber);
		}

		#endregion // CompareValues

		#region ConvertLookupValueToRegex

		protected static Regex ConvertLookupValueToRegex(string lookupValue)
		{
			// MD 3/5/12 - 12.1 - Table Support
			// Moved this code to Utilities.CreateWildcardRegex
			#region Moved

			//System.Text.StringBuilder patternBuilder = new System.Text.StringBuilder(Regex.Escape(lookupValue));
			...
			#endregion // Moved
			bool hasWildcards;
			return Utilities.CreateWildcardRegex(lookupValue, out hasWildcards);
		} 

		#endregion // ConvertLookupValueToRegex

		// MD 3/8/12 - TFS104321
		#region LookupValue

		protected static int LookupValue(IEnumerable<KeyValuePair<int, UltraCalcValue>> lookupRange,
			UltraCalcValue lookupValue,
			int matchType)
		{
			bool lookupValueIsString = lookupValue.IsString;
			string lookupValueString = lookupValueIsString ? lookupValue.ToString() : null;
			bool lookupValueIsBoolean = lookupValue.IsBoolean;
			bool lookupValueBoolean = lookupValueIsBoolean ? lookupValue.ToBoolean() : false;

			double lookupValueNumber = 0;
			if (lookupValueIsBoolean == false && lookupValueIsString == false)
				lookupValue.ToDouble(out lookupValueNumber);

			Regex matchRegex = null;
			if (lookupValueIsString && matchType == 0)
				matchRegex = UltraCalcFunctionLookupBase.ConvertLookupValueToRegex(lookupValueString);

			int lastPotentialMatch = -1;

			foreach (KeyValuePair<int, UltraCalcValue> pair in lookupRange)
			{
				int itemIndex = pair.Key;
				UltraCalcValue testValue = pair.Value;

				int? compareResult = UltraCalcFunctionLookupBase.CompareValues(testValue,
					lookupValueIsString,
					lookupValueString,
					lookupValueIsBoolean,
					lookupValueBoolean,
				...
				if (matchType != 0)
				{
					// When the match type is less than zero, the list should be in descending order, and we want the smallest value
					// that is greater than or equal to the lookup value, so the current value could be a potential match if it is
					// greater than the lookup value. Otherwise, if the math type if greater than zero, the list should be in ascending
					// order and we want the largest value that is less than the lookup value, so the current value could be a potential 
					// match if it is less than the lookup value.
					if (matchType < 0)
					{
						if (compareResult.Value > 0)
		...
		//protected static int? MatchValue(
		//    UltraCalcValue testValue,
		//    int currentIndex,
		//    bool lookupValueIsString, 
		//    string lookupValueString, 
		...
		//        string firstValueString = testValue.ToString();

		//        // MD 3/2/12 - 12.1 - Table Support
		//        //if (rangeLookup)
		//        if (matchType != 0)
		//        {
		//            int compareValue = string.Compare(lookupValueString, firstValueString, StringComparison.CurrentCultureIgnoreCase);
		//            if (compareValue == 0)
		//                return currentIndex;

		//            // MD 3/2/12 - 12.1 - Table Support
		//            // When the match type is less than zero, the list should be in descending order, and we want the smallest value
		//            // that is greater than or equal to the lookup value, so if the lookup value is greater than the current value,
		//            // use the previous value, which is greater than the lookup value.
		//            if (matchType < 0)
		//            {
		//                if (compareValue > 0)
		...
		//            // If the lookup value is less than the column value, use the previous column's value, 
		//            // which was less than the lookup value.
		//            if (compareValue < 0)
		//                return currentIndex - 1;
		//        }
		//        else
		//        {
		//            // If rangeLookup is false, wildcard searches can be used, so use the regex created earlier.
		//            Match match = matchRegex.Match(firstValueString);
		//            if (match.Success && match.Length == firstValueString.Length)
		//                return currentIndex;
		//        }
		...
		//        bool firstValueBoolean = testValue.ToBoolean();
		//        if (lookupValueBoolean == firstValueBoolean)
		//            return currentIndex;
		...
		//        // If the lookup value is false, the column value is true. Therefore the lookup value is lesser.
		//        // If the lookup value is less than the column value, use the previous column's value, 
		//        // which was less than the lookup value.
		//        if (lookupValueBoolean == false)
		//            return currentIndex - 1;

		//        return null;
		//    }

		//    double firstValueNumber;
		//    if (testValue.ToDouble(out firstValueNumber) == false)
		//        return null;

		//    if (lookupValueNumber == firstValueNumber)
		//        return currentIndex;
		...
		//#endregion // MatchValue

		#endregion // Removed

		// MD 3/8/12 - TFS104321
		#region TryParseRangeLookupArg

		protected static bool TryParseRangeLookupArg(UltraCalcValue rangeLookupArg, out bool rangeLookup)
		{
			rangeLookup = true;
			if (rangeLookupArg != null)
			{
		...
		#endregion // TryParseRangeLookupArg
		...
		internal override bool WillParameterAlwaysBeDereferenced(int parameterIndex)
		{
			// The lookup range is only dereferenced if the first parameter is not an error.
			// Also, only the first column/row of the lookup range is actually dereferenced, so this will prevent
			// us from taking a dependency of the whole range.
			if (parameterIndex == 1)
				return false;
	...
	#endregion UltraCalcFunctionLookupBase

	// MD 3/2/12 - TFS103729
	#region UltraCalcFunctionMatch
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionMatch : UltraCalcFunctionLookupBase
	{
		#region Evaluate
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			UltraCalcValue matchTypeArg = null;
			...
			double matchTypeTemp = 1;
			if (matchTypeArg != null && matchTypeArg.ToDouble(out matchTypeTemp) == false)
				return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));

			int matchType = (int)MathUtilities.Truncate(matchTypeTemp);
			...
			//// MD 3/8/12 - TFS104324
			//// For match types of 0 and positive values, if we don't find the value, #N/A should be returned, not the index 
			//// after the range.
			////int matchingItemIndex = maxItemCount - 1;
			//int matchingItemIndex = (matchType < 0) ? maxItemCount - 1 : -1;
			...
			return new UltraCalcValue(matchingItemIndex + 1);
		}
	...
	#endregion //UltraCalcFunctionMatch

	#region UltraCalcFunctionMdeterm
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionMdeterm : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			UltraCalcValue arrayArg = numberStack.Pop();
			...
			// MD 12/1/11 - TFS96113
			// ToArray is deprecated and replaced by ToArrayProxy.
			//UltraCalcValue[ , ] array = arrayArg.ToArray();
			ArrayProxy array = arrayArg.ToArrayProxy();
			...
			UltraCalcErrorValue error;
			double[ , ] matrix = MatrixUtilities.ConvertArrayToMatrix( array, MatrixUtilities.NonNumericElementBehavior.CausesError, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			return new UltraCalcValue( MatrixUtilities.CalculateDeterminant( matrix, n ) );
		}
	...
	#endregion UltraCalcFunctionMdeterm

	#region UltraCalcFunctionMinverse
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionMinverse : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			double[ , ] inverse = MatrixUtilities.CalculateInverse( matrix, n, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			UltraCalcValue[ , ] finalValues = new UltraCalcValue[ n, n ];

			for ( int x = 0; x < n; x++ )
			{
			    for ( int y = 0; y < n; y++ )
					finalValues[ x, y ] = new UltraCalcValue( inverse[ x, y ] );
			}

			return new UltraCalcValue( finalValues );
		}
	...
	#endregion UltraCalcFunctionMinverse

	#region UltraCalcFunctionMmult
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionMmult : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			UltraCalcValue array2Arg = numberStack.Pop();
			UltraCalcValue array1Arg = numberStack.Pop();
			...
			// MD 12/1/11 - TFS96113
			// ToArray is deprecated and replaced by ToArrayProxy.
			//UltraCalcValue[ , ] array1 = array1Arg.ToArray();
			ArrayProxy array1 = array1Arg.ToArrayProxy();

			int array1Columns = array1.GetLength( 0 );
			int array1Rows = array1.GetLength( 1 );

			// MD 12/1/11 - TFS96113
			// ToArray is deprecated and replaced by ToArrayProxy.
			//UltraCalcValue[ , ] array2 = array2Arg.ToArray();
			ArrayProxy array2 = array2Arg.ToArrayProxy();

			int array2Columns = array2.GetLength( 0 );
			int array2Rows = array2.GetLength( 1 );

			if ( array1Columns != array2Rows )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Value ) );

			UltraCalcErrorValue error;
			double[ , ] matrix1 = MatrixUtilities.ConvertArrayToMatrix( array1, MatrixUtilities.NonNumericElementBehavior.CausesError, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			double[ , ] matrix2 = MatrixUtilities.ConvertArrayToMatrix( array2, MatrixUtilities.NonNumericElementBehavior.CausesError, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			UltraCalcValue[ , ] finalMatrix = new UltraCalcValue[ array2Columns, array1Rows ];

			for ( int finalMatrixColumn = 0; finalMatrixColumn < array2Columns; finalMatrixColumn++ )
			{
				for ( int finalMatrixRow = 0; finalMatrixRow < array1Rows; finalMatrixRow++ )
				{
					double currentTotal = 0;

					for ( int element = 0; element < array1Columns; element++ )
					{
						currentTotal += 
							matrix1[ element, finalMatrixRow ] * 
							matrix2[ finalMatrixColumn, element ];
					}

					finalMatrix[ finalMatrixColumn, finalMatrixRow ] = new UltraCalcValue( currentTotal );
				}
			}

			return new UltraCalcValue( finalMatrix );
		}
	...
	#endregion UltraCalcFunctionMmult

	#region UltraCalcFunctionOffset
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionOffset : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			UltraCalcValue widthArg = null;

			if ( argumentCount == 5 )
				widthArg = numberStack.Pop();

			UltraCalcValue heightArg = null;

			if ( argumentCount >= 4 )
				heightArg = numberStack.Pop();

			UltraCalcValue colsArg = numberStack.Pop();
			UltraCalcValue rowsArg = numberStack.Pop();

			// When offsetting a reference, if the refernence used was a cell reference and it was a row or column input
			// cell of a data table, we don't want to replaced reference. Get the actual reference. We only want to replace
			// the reference when obtaining the value.
			UltraCalcValue referenceArg = numberStack.Pop( false );
			...
			//IUltraCalcFormula formulaBeingSolved = numberStack.FormulaOwner.Formula;
			//RegionGroupCalcReference regionGroupReference = RegionGroupCalcReference.FromReference( referenceArg.ToReference(), formulaBeingSolved );
			RegionGroupCalcReference regionGroupReference = RegionGroupCalcReference.FromReference(referenceArg.ToReference());

			if ( regionGroupReference.Regions.Count != 1 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Value ) );

			WorksheetRegion region = regionGroupReference.Regions[ 0 ];

			// Determine the row, column, and area index based on the parameters specified.
			int rowOffset = rowsArg.ToInt32();
			int colOffset = colsArg.ToInt32();
			int height = heightArg == null ? region.Height : heightArg.ToInt32();
			int width = widthArg == null ? region.Width : widthArg.ToInt32();

			if ( height <= 0 || width <= 0 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			int newFirstRow = region.FirstRow + rowOffset;
			int newFirstColumn = region.FirstColumn + colOffset;

			if ( newFirstColumn < 0 || newFirstRow < 0 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			int newLastRow = newFirstRow + height - 1;
			int newLastColumn = newFirstColumn + width - 1;

			// MD 2/29/12 - 12.1 - Table Support
			// The worksheet can now be null.
			//Workbook workbook = region.Worksheet.Workbook;
			...
			// MD 2/29/12
			// Found while implementing 12.1 - Table Support
			// We can get the max count from the worksheet.
			//if ( workbook != null )
			//{
			//    int maxRowIndex = workbook.MaxRowCount - 1;
			//    int maxColumnIndex = workbook.MaxColumnCount - 1;
			//
			...
			return new UltraCalcValue(
				// MD 2/29/12 - 12.1 - Table Support
				// Use the cached region from above.
				//region.Worksheet.GetCachedRegion( newFirstRow, newFirstColumn, newLastRow, newLastColumn ).CalcReference );
				worksheet.GetCachedRegion(newFirstRow, newFirstColumn, newLastRow, newLastColumn).CalcReference);
		}
	...
	#endregion UltraCalcFunctionOffset

	#region UltraCalcFunctionPearson
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionPearson : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		{
			UltraCalcErrorValue error;
			double pearsons = UltraCalcFunctionTwoMatrixFunction.CalculatePearsonsCorrelationCoefficient( numberStack, out error );

			if ( error != null )
				return new UltraCalcValue( error );

			return new UltraCalcValue( pearsons );
		}
	...
	#endregion UltraCalcFunctionPearson

	#region UltraCalcFunctionRow
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionRow : BuiltInFunctionBase
	{
		private const int ReferenceErrorIndex = -1;
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			// If no arguments were specified, the 1-based row index of the cell owning the formula should be returned.
			if ( argumentCount == 0 )
			{
				rowNumbers = UltraCalcFunctionRow.GetRowNumbersFromReference( numberStack.FormulaOwner );
			}
			else
			{
				UltraCalcValue arg = numberStack.Pop();
				...
				// Get the 1-based row index from the reference.
				rowNumbers = UltraCalcFunctionRow.GetRowNumbersFromReference( arg.ToReference() );
			}

			if ( rowNumbers[ 0, 0 ] == UltraCalcFunctionRow.ReferenceErrorIndex )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			int numColumns = rowNumbers.GetLength( 0 );
			int numRows = rowNumbers.GetLength( 1 );
			UltraCalcValue[ , ] array = new UltraCalcValue[ numColumns, numRows ];
			...
			for ( int y = 0; y < numRows; y++ )
			{
				for ( int x = 0; x < numColumns; x++ )
				{
					Debug.Assert( rowNumbers[ x, y ] > 0, "Invalid row number." );
					array[ x, y ] = new UltraCalcValue( rowNumbers[ x, y ] );
				}
			}
		...
		#region GetRowNumbersFromReference

		private static int[ , ] GetRowNumbersFromReference( IUltraCalcReference reference )
		{
			// MD 4/12/11 - TFS67084
			// Moved away from using WorksheetCell objects.
			//IWorksheetCell cell = reference.Context as IWorksheetCell;
			//
			//if ( cell != null )
			//    return new int[ , ] { { cell.RowIndex + 1 } };
			CellCalcReference cellReference = reference.Context as CellCalcReference;
			if (cellReference != null)
				return new int[,] { { cellReference.Row.Index + 1 } };

			WorksheetRegion region = reference.Context as WorksheetRegion;

			if ( region != null )
				return UltraCalcFunctionRow.GetRowNumbersFromRegion( region );
			...
			if ( regions != null )
			{
				// The row index cannot be obtained from a group of multiple arrays.
				if ( regions.Count != 1 )
					return new int[ , ] { { UltraCalcFunctionRow.ReferenceErrorIndex } };

				return UltraCalcFunctionRow.GetRowNumbersFromRegion( regions[ 0 ] );
			}
		...
		#endregion GetRowNumbersFromReference

		#region GetRowNumbersFromRegion

		private static int[ , ] GetRowNumbersFromRegion( WorksheetRegion region )
		{
			int[ , ] rows = new int[ region.Width, region.Height ];
		...
		#endregion GetRowNumbersFromRegion
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionRows : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			// The row count cannot be obtained from a group of multiple arrays.
			if ( arrayGroup.Length != 1 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Reference ) );

			// Return the row count of the only array in the group.
			return new UltraCalcValue( arrayGroup[ 0 ].GetLength( 1 ) );
		}
	...
	#endregion UltraCalcFunctionRows

	#region UltraCalcFunctionRsq
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionRsq : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			return new UltraCalcValue( pearsons * pearsons );
		}
	...
	#endregion UltraCalcFunctionRsq

	#region UltraCalcFunctionSlope
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionSlope : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			return new UltraCalcValue( regressionLineSlope );
		}
	...
	#endregion UltraCalcFunctionSlope

	#region UltraCalcFunctionSteyx
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionSteyx : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			double knownYTotal;
			double knownXTotal;
			UltraCalcFunctionTwoMatrixFunction.GetTotals( knownYs, knownXs, out elementCount, out knownYTotal, out knownXTotal );

			if ( elementCount < 3 )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Div ) );

			double knownYAverage = knownYTotal / elementCount;
			double knownXAverage = knownXTotal / elementCount;

			double sumOfXDiffsTimesYDiffs = UltraCalcFunctionTwoMatrixFunction.SumElements( knownYs, knownXs,
				new SumElementCallback2( delegate( double elementY, double elementX )
				{
					return ( elementY - knownYAverage ) * ( elementX - knownXAverage );
				} ) );

			double sumOfSquaredYDiffs = UltraCalcFunctionTwoMatrixFunction.SumElements( knownYs, 
				new SumElementCallback1( delegate( double element )
				{
					return Math.Pow( ( element - knownYAverage ), 2 );
				} ) );

			double sumOfSquaredXDiffs = UltraCalcFunctionTwoMatrixFunction.SumElements( knownXs,
				new SumElementCallback1( delegate( double element )
				{
			...
			double part1 = 1d / ( elementCount - 2 );
			double part2 = sumOfSquaredYDiffs - ( ( sumOfXDiffsTimesYDiffs * sumOfXDiffsTimesYDiffs ) / sumOfSquaredXDiffs );
	...
	#endregion UltraCalcFunctionSteyx

	// MD 4/2/12 - TFS99854
	#region UltraCalcFunctionSumIf

	internal class UltraCalcFunctionSumIf : UltraCalcConditionalFunctionBase
	{
		#region GetReturnValue
	...
	#endregion //UltraCalcFunctionSumIf

	#region UltraCalcFunctionSumproduct
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionSumproduct : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			double[][ , ] arrays = new double[ argumentCount ][ , ];

			int columns = 0;
			int rows = 0;

			int argumentsPopped = 0;
			try
			{
				for ( ; argumentsPopped < argumentCount; argumentsPopped++ )
				{
					...
					// MD 12/1/11 - TFS96113
					// ToArray is deprecated and replaced by ToArrayProxy.
					//double[ , ] array = MatrixUtilities.ConvertArrayToMatrix( value.ToArray(), MatrixUtilities.NonNumericElementBehavior.TreatAsZero, out error );
					double[,] array = MatrixUtilities.ConvertArrayToMatrix(value.ToArrayProxy(), MatrixUtilities.NonNumericElementBehavior.TreatAsZero, out error);
			...
			return new UltraCalcValue( total );
		}
	...
	#endregion UltraCalcFunctionSumproduct

	#region UltraCalcFunctionSumx2my2
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionSumx2my2 : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			return this.EvaluateHelper( numberStack,
				new SumElementCallback2( delegate( double element1, double element2 )
				{
	...
	#endregion UltraCalcFunctionSumx2my2

	#region UltraCalcFunctionSumx2py2
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionSumx2py2 : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
	...
	#endregion UltraCalcFunctionSumx2py2

	#region UltraCalcFunctionSumxmy2
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionSumxmy2 : UltraCalcFunctionTwoMatrixFunction
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
	...
	#endregion UltraCalcFunctionSumxmy2

	#region UltraCalcFunctionTranspose
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionTranspose : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			// MD 12/1/11 - TFS96113
			// ToArray is deprecated and replaced by ToArrayProxy.
			//return new UltraCalcValue( MatrixUtilities.TransposeMatrix( arg.ToArray() ) );
			return new UltraCalcValue(MatrixUtilities.TransposeMatrix(arg.ToArrayProxy()));
		}
	...
	#endregion UltraCalcFunctionTranspose

	#region UltraCalcFunctionTwoMatrixFunction

	internal abstract class UltraCalcFunctionTwoMatrixFunction : BuiltInFunctionBase
	{
		protected delegate double SumElementCallback1( double element1 );
		protected delegate double SumElementCallback2( double element1, double element2 );

		protected UltraCalcValue EvaluateHelper( UltraCalcNumberStack numberStack, SumElementCallback2 callback )
		{
			UltraCalcErrorValue error;
			double[ , ] matrix1;
			double[ , ] matrix2;
			...
			return new UltraCalcValue( UltraCalcFunctionTwoMatrixFunction.SumElements( matrix1, matrix2, callback ) );
		}

		#region CalculatePearsonsCorrelationCoefficient

		protected static double CalculatePearsonsCorrelationCoefficient( UltraCalcNumberStack numberStack, out UltraCalcErrorValue error )
		{
			error = null;
			...
			double denominator1 = UltraCalcFunctionTwoMatrixFunction.SumElements( matrix1,
				new SumElementCallback1( delegate( double element )
				{
					return Math.Pow( element - average1, 2 );
				} ) );

			double denominator2 = UltraCalcFunctionTwoMatrixFunction.SumElements( matrix2,
				new SumElementCallback1( delegate( double element )
				{
			...
			if ( denominator == 0 )
			{
				error = new UltraCalcErrorValue( UltraCalcErrorCode.Div );
				return 0;
			}
		...
		#endregion CalculatePearsonsCorrelationCoefficient

		#region GetArguments

		protected static void GetArguments( UltraCalcNumberStack numberStack, out UltraCalcErrorValue error, out double[ , ] matrix1, out double[ , ] matrix2 )
		{
			error = null;
			matrix1 = null;
			matrix2 = null;
			...
			{
				error = new UltraCalcErrorValue( UltraCalcErrorCode.NA );
				return;
			}

			matrix1 = MatrixUtilities.ConvertArrayToMatrix( array1, MatrixUtilities.NonNumericElementBehavior.Ignore, out error );

			if ( error != null )
				return;

			matrix2 = MatrixUtilities.ConvertArrayToMatrix( array2, MatrixUtilities.NonNumericElementBehavior.Ignore, out error );
		}
		...
		protected static void GetCompareInfo( UltraCalcNumberStack numberStack, 
			out double regressionLineSlope, out double regressionLineYIntercept, out UltraCalcErrorValue error )
		{
			error = null;
			regressionLineSlope = 0d;
		...
		protected static void GetCompareInfo( 
			double[ , ] knownYs, double[ , ] knownXs, out double knownYAverage, out double knownXAverage,
			out double regressionLineSlope, out double regressionLineYIntercept, out UltraCalcErrorValue error )
		{
			error = null;
			regressionLineSlope = 0d;
			...
			double knownYAverageCopy = knownYAverage;
			double knownXAverageCopy = knownXAverage;

			double numerator = UltraCalcFunctionTwoMatrixFunction.SumElements( knownYs, knownXs,
				 new SumElementCallback2( delegate( double elementY, double elementX )
				 {
			...
			double denominator = UltraCalcFunctionTwoMatrixFunction.SumElements( knownXs,
				new SumElementCallback1( delegate( double elementX )
				{
		...
		#endregion GetCompareInfo

		#region GetTotals

		protected static void GetTotals( double[ , ] matrix1, double[ , ] matrix2, out int elementCount, out double total1, out double total2 )
		{
			int columns = matrix1.GetLength( 0 );
			...
			for ( int x = 0; x < columns; x++ )
			{
				for ( int y = 0; y < rows; y++ )
				{
					double value1 = matrix1[ x, y ];
					double value2 = matrix2[ x, y ];

					if ( Double.IsNaN( value1 ) || Double.IsNaN( value2 ) )
					{
						elementCount--;
						matrix1[ x, y ] = Double.NaN;
						matrix2[ x, y ] = Double.NaN;
						continue;
					}
		...
		protected static double SumElements( double[ , ] matrix, SumElementCallback1 callback )
		{
		...
		protected static double SumElements( double[ , ] matrix1, double[ , ] matrix2, SumElementCallback2 callback )
		{
			int columns = matrix1.GetLength( 0 );
			int rows = matrix1.GetLength( 1 );
	...
	#endregion UltraCalcFunctionSumArrayElementsBase

	// MD 7/9/10 - TFS35240
	#region UltraCalcFunctionVLookup
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionVLookup : UltraCalcFunctionLookupBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate(UltraCalcNumberStack numberStack, int argumentCount)
		{
			...
			UltraCalcValue colIndexNumArg = numberStack.Pop();
			UltraCalcValue tableArrayArg = numberStack.Pop();
			UltraCalcValue lookupValueArg = numberStack.Pop();
			...
			double colIndexNum;
			if (colIndexNumArg.ToDouble(out colIndexNum) == false || colIndexNum < 1)
				return new UltraCalcValue(new UltraCalcErrorValue(UltraCalcErrorCode.Value));

			int columnIndex = (int)Math.Floor(colIndexNum) - 1;
			...
			//int matchingRow = rangeLookup ? rowCount - 1 : -1;

			//// MD 12/1/11 - TFS96113
			//// Instead of iterating all rows in the lookup region, only iterate the already created rows in the region.
			//// This could drastically improve performance when a large range is specified but only a small region in that range
			//// has data.
			////for (int rowIndex = 0; rowIndex < rowCount; rowIndex++)
			////{
			////    UltraCalcValue firstValueInRow = tableArray[0, rowIndex];
			//foreach (KeyValuePair<int, UltraCalcValue> pair in tableArray.GetIterator(0, 0))
			//{
			//    int rowIndex = pair.Key;
			//    UltraCalcValue firstValueInRow = pair.Value;

			//    int? matchingRowTemp = UltraCalcFunctionLookupBase.MatchValue(
			//        firstValueInRow,
			//        rowIndex,
			//        lookupValueIsString,
			//        lookupValueString,
			...
			//    if (matchingRowTemp.HasValue)
			//    {
			//        matchingRow = matchingRowTemp.Value;
			//        break;
			//    }
			//}

			#endregion // Moved
			int matchingRow = UltraCalcFunctionLookupBase.LookupValue(
				tableArray.GetIterator(0, 0),
				lookupValueArg,
				rangeLookup ? 1 : 0);
			...
			UltraCalcValue returnValue = tableArray[columnIndex, matchingRow];
			return new UltraCalcValue(returnValue.GetResolvedValue());
		}
	...
	#endregion UltraCalcFunctionVLookup

	#region UltraCalcFunctionXLIntersect
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionXLIntersect : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			// When creating a dynamic range, if the refernence used was a cell reference and it was a row or column input
			// cell of a data table, we don't want to replaced reference. Get the actual reference. We only want to replace
			// the reference when obtaining the value.
			UltraCalcValue ref1 = numberStack.Pop( false );
			...
			// cell of a data table, we don't want to replaced reference. Get the actual reference. We only want to replace
			// the reference when obtaining the value.
			UltraCalcValue ref2 = numberStack.Pop( false );

			if ( ref2.IsError )
				return ref2;

			if ( ref1.IsReference == false || ref2.IsReference == false )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Value ) );

			// MD 2/27/12 - TFS102520
			//IUltraCalcFormula formulaBeingSolved = numberStack.FormulaOwner.Formula;
			//RegionGroupCalcReference generalRef1 = RegionGroupCalcReference.FromReference( ref1.ToReference(), formulaBeingSolved );
			//RegionGroupCalcReference generalRef2 = RegionGroupCalcReference.FromReference( ref2.ToReference(), formulaBeingSolved );
			RegionGroupCalcReference generalRef1 = RegionGroupCalcReference.FromReference(ref1.ToReference());
			RegionGroupCalcReference generalRef2 = RegionGroupCalcReference.FromReference(ref2.ToReference());

			// If the references specified were from different worksheets, a #VALUE! error should be returned
			if ( generalRef1 == null || generalRef2 == null || generalRef1.Worksheet == null || generalRef1.Worksheet != generalRef2.Worksheet )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Value ) );

			IExcelCalcReference intersection = RegionGroupCalcReference.Intersect( generalRef1, generalRef2 );

			if ( intersection == null )
				return new UltraCalcValue( new UltraCalcErrorValue( UltraCalcErrorCode.Null ) );

			return new UltraCalcValue( intersection );
		}
		...
		internal override TokenClass ReturnTokenClass
		{
			get { return TokenClass.Reference; }
		}
	...
	#endregion UltraCalcFunctionXLIntersect

	#region UltraCalcFunctionXLRange
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionXLRange : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			return new UltraCalcValue( RegionGroupCalcReference.Range( generalRef1, generalRef2 ) );
		}
	...
	#endregion UltraCalcFunctionXLRange

	#region UltraCalcFunctionXLUnion
...
#region Infragistics Source Cleanup (Region)
	...
	internal class UltraCalcFunctionXLUnion : BuiltInFunctionBase
	{
		/// <summary>
		/// Evaluates the function against the arguments on the number stack.
		/// </summary>
		...
		protected override UltraCalcValue Evaluate( UltraCalcNumberStack numberStack, int argumentCount )
		{
			...
			return new UltraCalcValue( RegionGroupCalcReference.Union( generalRef1, generalRef2 ) );
		}
	...
	#endregion UltraCalcFunctionXLUnion
