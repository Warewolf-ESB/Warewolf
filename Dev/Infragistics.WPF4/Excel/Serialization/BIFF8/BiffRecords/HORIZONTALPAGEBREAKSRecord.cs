...
namespace Infragistics.Documents.Excel.Serialization.BIFF8.BiffRecords
{
	...
	internal class HORIZONTALPAGEBREAKSRecord : Biff8RecordBase
	{
		public override void Load( BIFF8WorkbookSerializationManager manager )
		{
			...
			for (int i = 0; i < numberOfBreaks; i++)
			{
				...
				printOptions.HorizontalPageBreaks.Add(
					new HorizontalPageBreak(id, printOptions.PrintAreas.GetPrintArea(id, minValue, maxValue, false)));
			}
		}
		...
		public override void Save(BIFF8WorkbookSerializationManager manager)
		{
			...
			manager.CurrentRecordStream.Write((ushort)printOptions.HorizontalPageBreaks.Count);
			for (int i = 0; i < printOptions.HorizontalPageBreaks.Count; i++)
			{
				PageBreak pageBreak = printOptions.HorizontalPageBreaks[i];

				manager.CurrentRecordStream.Write((ushort)pageBreak.Id);
				manager.CurrentRecordStream.Write((ushort)pageBreak.MinResolved);
				int maxResolved = Math.Min(pageBreak.MaxResolved, manager.Workbook.MaxColumnCount - 1);
		...
		{
			get { return BIFF8RecordType.HORIZONTALPAGEBREAKS; }
		}
	}
}
