...
{
    class ItalicElement : XLSXElementBase
    {
        #region Constants

        public const string LocalName = "i";

        public const string QualifiedName =
            XLSXElementBase.DefaultXmlNamespace +
            XmlElementBase.NamespaceSeparator +
            ItalicElement.LocalName;
...
#region Infragistics Source Cleanup (Region)
        ...
        protected override void Load(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode)
        {
            ...
            foreach (ExcelXmlAttribute attribute in element.Attributes)
            {
                ...
                {
                    case ItalicElement.ValAttributeName:
                        bool val = (bool)XLSXElementBase.GetAttributeValue(attribute, DataType.Boolean, true);
                        if (val)
                        {
                            fontData.Italic = ExcelDefaultableBoolean.True;
                        }
                        else
                        {
                            fontData.Italic = ExcelDefaultableBoolean.False;
                        }
                        break;

                    default:
                        Utilities.DebugFail("Unknown attribute type in the i (italic) element: " + attributeName);
                        break;
                }
            }
            ...
            if (fontData.Italic == ExcelDefaultableBoolean.Default)
                fontData.Italic = ExcelDefaultableBoolean.True;
        }
...
#region Infragistics Source Cleanup (Region)
        ...
        protected override void Save(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, ref string value)
        {
            ...
            if (fontData != null)
            {
                ...
                if (fontData.Italic == ExcelDefaultableBoolean.Default || fontData.Italic == ExcelDefaultableBoolean.False)
                    XmlElementBase.AddAttribute(element, ItalicElement.ValAttributeName, XmlElementBase.GetXmlString(false, DataType.Boolean));
            }
        }
		...
		{
			writer.WriteElementString(ItalicElement.LocalName, null);
		} 
