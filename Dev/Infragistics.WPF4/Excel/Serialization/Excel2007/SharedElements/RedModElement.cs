...

{
    ...
    internal class RedModElement : XmlElementBase
    {
        #region Constants

        public const string LocalName = "redMod";

        public const string QualifiedName =
            ThemePart.DefaultNamespace +
            XmlElementBase.NamespaceSeparator +
            RedModElement.LocalName;
        ...
        {
            get { return RedModElement.QualifiedName; }
        }
        ...
        protected override void Load(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode)
        {
            ...
            int val = (int)XmlElementBase.GetAttributeValue(element.Attributes[RedModElement.ValAttributeName], DataType.Int32, -1);
            if (val == -1)
            {
            ...
            try
            {
                val = Convert.ToInt32((double)val * color.R / 100000);
                if (val > 255)
                    val = 255;
                else if (val < 0)
                    val = 0;
        ...
        {
            Utilities.DebugFail("Save() method for redMod element not yet implemented.");
        }
