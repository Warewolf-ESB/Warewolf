...

{
    ...
    internal class GreenModElement : XmlElementBase
    {
        #region Constants

        public const string LocalName = "greenMod";

        public const string QualifiedName =
            ThemePart.DefaultNamespace +
            XmlElementBase.NamespaceSeparator +
            GreenModElement.LocalName;
        ...
        {
            get { return GreenModElement.QualifiedName; }
        }
        ...
        protected override void Load(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode)
        {
            ...
            int val = (int)XmlElementBase.GetAttributeValue(element.Attributes[GreenModElement.ValAttributeName], DataType.Int32, -1);
            if (val == -1)
            {
            ...
            try
            {
                val = Convert.ToInt32((double)val * color.G / 100000);
                if (val > 255)
                    val = 255;
                else if (val < 0)
                    val = 0;
                color = Color.FromArgb(color.A, color.R, (byte)val, color.B);
            }
            catch (Exception ex)
            {
        ...
        {
            Utilities.DebugFail("Save() method for greenMod element not yet implemented.");
        }
