...
{
    internal class HueOffElement : XmlElementBase
    {
        #region Constants

        public const string LocalName = "hueOff";

        public const string QualifiedName =
            ThemePart.DefaultNamespace +
            XmlElementBase.NamespaceSeparator +
            HueOffElement.LocalName;
        ...
        protected override void Load(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode)
        {
            ...
            int val = (int)XmlElementBase.GetAttributeValue(element.Attributes[HueOffElement.ValAttributeName], DataType.Int32, -1);
            if (val == -1)
            {
            ...
            try
            {
				...
				//int newHue = val + h * 60000;
				//if (newHue >= 21600000)
				//    newHue = 21599999;
				//else if (newHue < 0)
				//    newHue = 0;
				//color = Utilities.ConvertHLSToRGB(newHue / 60000, l, s);
				int h, s, l;
                Utilities.ConvertRGBToHLS(color, out h, out l, out s);

				int newHue = (int)(h + Utilities.HLSMAX * ((val / 60000d) / 360d));
        ...
        {
            Utilities.DebugFail("Save() method for hueOff element not yet implemented.");
        }
