...
{
    internal class SatOffElement : XmlElementBase
    {
        #region Constants

        public const string LocalName = "satOff";

        public const string QualifiedName =
            ThemePart.DefaultNamespace +
            XmlElementBase.NamespaceSeparator +
            SatOffElement.LocalName;
        ...
        {
            get { return SatOffElement.QualifiedName; }
        }
        ...
        protected override void Load(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode)
        {
            ...
            int val = (int)XmlElementBase.GetAttributeValue(element.Attributes[SatOffElement.ValAttributeName], DataType.Int32, -1);
            if (val == -1)
            {
            ...
            try
            {
                ...
                //int newS = s + (255 * (val / 100000));
				int newS = Math.Max(0, Math.Min(s + (Utilities.HLSMAX * val / 100000), Utilities.HLSMAX));

                color = Utilities.ConvertHLSToRGB(h, l, newS);
            }
            catch (Exception ex)
            {
        ...
        {
            Utilities.DebugFail("Save() method for satOff element not yet implemented.");
        }
