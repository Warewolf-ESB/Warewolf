...
{
    internal class TitlesOfPartsElement : XmlElementBase
	{
		#region Constants

		public const string LocalName = "TitlesOfParts";

		public const string QualifiedName =
			ExtendedPropertiesPart.DefaultNamespace +
			XmlElementBase.NamespaceSeparator +
			TitlesOfPartsElement.LocalName; 
		...
		{
			get { return TitlesOfPartsElement.QualifiedName; }
		}
		...
		protected override void Load( Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode )
		{
			...
			//    "The TitlesOfParts element should contain exactly one vector element." );
		...
		protected override void OnAfterLoadChildElements( Excel2007WorkbookSerializationManager manager, ElementDataCache elementCache )
		{
			...
			// RoundTrip - Page 5118
			//
			// This element specifies the title of each document. These parts are not document parts but conceptual
			// representations of document sections.
		}
		...
		protected override void Save( Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, ref string value )
		{
			...
			string[] worksheetNames = new string[ workbook.Worksheets.Count ];

			for ( int i = 0; i < worksheetNames.Length; i++ )
				worksheetNames[ i ] = workbook.Worksheets[ i ].Name;

			manager.ContextStack.Push( new ChildDataItem( worksheetNames ) );
