...

{
    ...
    internal class AlphaModElement : XmlElementBase
    {
        #region Constants

        public const string LocalName = "alphaMod";

        public const string QualifiedName =
            ThemePart.DefaultNamespace +
            XmlElementBase.NamespaceSeparator +
            AlphaModElement.LocalName;
        ...
        {
            get { return AlphaModElement.QualifiedName; }
        }
        ...
        protected override void Load(Excel2007WorkbookSerializationManager manager, ExcelXmlElement element, string value, ref bool isReaderOnNextNode)
        {
            ...
            int val = (int)XmlElementBase.GetAttributeValue(element.Attributes[AlphaModElement.ValAttributeName], DataType.Int32, -1);
            if (val == -1)
            {
            ...
            try
            {
                val = Convert.ToInt32((double)val * color.A / 100000);
                if (val > 255)
                    val = 255;
                else if (val < 0)
                    val = 0;
                color = Color.FromArgb((byte)val, color.R, color.G, color.B);
            }
            catch (Exception ex)
            {
        ...
        {
            Utilities.DebugFail("Save() method for alphaMod element not yet implemented.");
        }
