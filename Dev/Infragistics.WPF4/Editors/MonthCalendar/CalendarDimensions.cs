...
{
    /// <summary>
    /// Structure used to indicate the preferred rows/columns of <see cref="CalendarItemGroup"/> instances within a <see cref="XamMonthCalendar"/>
    /// </summary>
    ...
    public struct CalendarDimensions : IEquatable<CalendarDimensions>
    {
        ...
        {
            if (rows < 0 || columns < 0)
				throw new ArgumentOutOfRangeException(XamMonthCalendar.GetString("LE_InvalidCalendarDimensions"));
        ...
        internal static CalendarDimensions FromString(string value, CultureInfo culture)
        {
            ...
            if (values.Length == 2 && values[0].Trim().Length > 0 && values[1].Trim().Length > 0)
            {
                ...
                int rows = (int)intConverter.ConvertFromString(null, culture, values[0]);
                int cols = (int)intConverter.ConvertFromString(null, culture, values[1]);

                return new CalendarDimensions(rows, cols);
            }

			throw new FormatException(XamMonthCalendar.GetString("LE_InvalidDimensionsString", value));
        }
    ...
    public class CalendarDimensionsConverter : TypeConverter
    {
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            ...
            if (destinationType != typeof(InstanceDescriptor))
                throw new ArgumentException(XamMonthCalendar.GetString("LE_CannotConvertType", typeof(CalendarDimensions), destinationType.FullName));
