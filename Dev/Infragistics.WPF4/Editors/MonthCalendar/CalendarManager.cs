...
namespace Infragistics.Windows.Editors
{
	#region CalendarMode
	/// <summary>
	/// Enumeration that determines what is displayed and interacted within each <see cref="CalendarItemGroup"/> of a <see cref="XamMonthCalendar"/>
	/// </summary>
	/// <seealso cref="XamMonthCalendar.MinCalendarMode"/>
	/// <seealso cref="XamMonthCalendar.CurrentCalendarMode"/>
	public enum CalendarMode : short
	{
		

		/// <summary>
		/// Each <see cref="CalendarItemGroup"/> of the <see cref="XamMonthCalendar"/> is displaying days of the month.
		/// </summary>
		Days,

		/// <summary>
		/// Each <see cref="CalendarItemGroup"/> of the <see cref="XamMonthCalendar"/> is displaying months of the year.
		/// </summary>
		Months,

		/// <summary>
		/// Each <see cref="CalendarItemGroup"/> of the <see cref="XamMonthCalendar"/> is displaying years of a decade.
		/// </summary>
		Years,

		/// <summary>
		/// Each <see cref="CalendarItemGroup"/> of the <see cref="XamMonthCalendar"/> is displaying ranges of 10 years of a century.
		/// </summary>
		Decades,
	...
	#endregion //CalendarMode

	#region DayOfWeekHeaderFormat

	/// <summary>
	/// Enumeration used to identify the format of the day of week captions in the <see cref="XamMonthCalendar"/>
	/// </summary>
	...
	internal class CalendarManager : CalendarHelper
	{
		...
		internal DateTime? TryAddGroupOffset(DateTime date, int offset, CalendarMode mode, bool adjustForStart)
		{
			try
			{
				switch (mode)
				{
					...
					case CalendarMode.Centuries:
						...
						int adjustment = (int)Math.Pow(10, 1 + (mode - CalendarMode.Years));
		...
		internal DateTime AddGroupOffset(DateTime date, int offset, CalendarMode mode, bool adjustForStart)
		{
		...
		internal DateTime? TryAddItemOffset(DateTime date, int offset, CalendarMode mode)
		{
			try
			{
				switch (mode)
				{
					...
					case CalendarMode.Centuries:
						...
						int adjustment = (int)Math.Pow(10, mode - CalendarMode.Years);
						date = this.Calendar.AddYears(date, offset * adjustment);
						break;
				}
			}
		...
		internal DateTime AddItemOffset(DateTime date, int offset, CalendarMode mode)
		{
		...
		#region GetDateCount
		private int GetDateCount(CalendarMode mode, bool includeLeadingDates, bool includeTrailingDates, ref DateTime firstDate)
		{
			...
			switch (mode)
			{
				...
				case CalendarMode.Centuries:
					
					{
						...
						Debug.Assert(mode <= CalendarMode.Centuries, "Unrecognized mode!");
						// see how many years between first item and last item date
						double adjustment = Math.Pow(10, mode - CalendarMode.Years);
		...
		public int GetDayOfWeekNumber(DateTime date, bool honorDisabledDays)
		{
            ...
            // This was broken when the class was refactored. We should always pass the 
            // actual first day of week.
            //
			//return this.GetDayOfWeekNumber(date, hiddenDays, null);
			return this.GetDayOfWeekNumber(date, hiddenDays, _firstDayOfWeek);
		}
...
#pragma warning restore 436
		...
		/// <returns></returns>
		internal static int GetItemColumnCount(CalendarMode mode)
		{
			switch (mode)
			{
				default:
		...
		#region GetItemDates
		internal DateTime[] GetItemDates(DateTime date, CalendarMode mode,
			...
			DateTime minDate, DateTime maxDate)
		{
			...
			DayOfWeekFlags daysToSkip = mode == CalendarMode.Days ? this._hiddenDays : 0;
			int datesAdded = 0;
		...
		internal void GetItemRowColumn(DateTime date, CalendarMode mode,
			...
			out int colOffset, out int rowOffset)
		{
			switch (mode)
			{
				...
				case CalendarMode.Centuries:
					...
					{
						Debug.Assert(mode <= CalendarMode.Centuries);
						...
						//decimal adjustment = (decimal)Math.Pow(10, mode - CalendarMode.Years);
						double adjustment = Math.Pow(10, mode - CalendarMode.Years);
		...
		internal static string GetDayOfWeekCaption(DayOfWeekHeaderFormat format, DayOfWeek day, DateTimeFormatInfo formatInfo)
		{
			int index = (int)day;
			string caption;
			formatInfo = formatInfo ?? CultureInfo.CurrentCulture.DateTimeFormat;
		...
		internal DateTime GetGroupEndDate(DateTime date, CalendarMode mode)
		{
			return GetGroupEndDate(date, mode, this.Calendar, this._hiddenDays);
		}

		internal static DateTime GetGroupEndDate(DateTime date, CalendarMode mode, Calendar calendar, DayOfWeekFlags hiddenDays)
		{
			...
			try
			{
				switch (mode)
				{
					case CalendarMode.Days:		// last day of month containing the date
						break;

					case CalendarMode.Months:	// last day of the last month of year containing the date
						month = calendar.GetMonthsInYear(year, era);
						days = calendar.GetDaysInMonth(year, month, era);
						break;
					case CalendarMode.Years:	// last year of the decade containing the date
					case CalendarMode.Decades:	// first decade of the century containing the date
					case CalendarMode.Centuries:
		...
		internal DateTime GetItemEndDate(DateTime date, CalendarMode mode)
		{
			return GetItemEndDate(date, mode, this.Calendar);
		}

		internal static DateTime GetItemEndDate(DateTime date, CalendarMode mode, Calendar calendar)
		{
			int month = calendar.GetMonth(date);
			int year = calendar.GetYear(date);
			int era = calendar.GetEra(date);
			...
			try
			{
				switch (mode)
				{
					case CalendarMode.Days:		// same day
						day = calendar.GetDayOfMonth(date);
						break;

					case CalendarMode.Months:	// last day of the month
						break;
					case CalendarMode.Years:	// last day of the year
						month = calendar.GetMonthsInYear(year, era);
						day = calendar.GetDaysInMonth(year, month, era);
						break;
					case CalendarMode.Decades:	// last day of the decade
					case CalendarMode.Centuries:
						
						int adjustment = (int)Math.Pow(10, 1 + (mode - CalendarMode.Decades));
						year += (adjustment - 1) - (year % adjustment);
						month = calendar.GetMonthsInYear(year, era);
						day = calendar.GetDaysInMonth(year, month, era);
						break;
		...
		internal DateTime GetGroupStartDate(DateTime date, CalendarMode mode)
		{
			return GetGroupStartDate(date, mode, this.Calendar, this._hiddenDays);
		}

		internal static DateTime GetGroupStartDate(DateTime date, CalendarMode mode, Calendar calendar, DayOfWeekFlags hiddenDays)
		{
			int month = 1;
			int year = calendar.GetYear(date);
			int era = calendar.GetEra(date);
			...
			switch (mode)
			{
				case CalendarMode.Days:		// first day of month containing the date
					month = calendar.GetMonth(date);
					break;

				case CalendarMode.Months:	// first day of the first month of year containing the date
					break;

				case CalendarMode.Years:	// first day of the first month of the first year of the decade containing the date
				case CalendarMode.Decades:	// first day of the first month of the first year of a century containing the date
				case CalendarMode.Centuries:
		...
		internal DateTime GetItemStartDate(DateTime date, CalendarMode mode)
		{
			return GetItemStartDate(date, mode, this.Calendar);
		}

		internal static DateTime GetItemStartDate(DateTime date, CalendarMode mode, Calendar calendar)
		{
			int month = 1;
			int year = calendar.GetYear(date);
			int era = calendar.GetEra(date);
			...
			switch (mode)
			{
				case CalendarMode.Days:		// the date itself
					return date;

				case CalendarMode.Months:	// the first day of the month
					month = calendar.GetMonth(date);
					break;

				case CalendarMode.Years:	// first day of the year
				case CalendarMode.Decades:	// first year of the decade
				case CalendarMode.Centuries:
		...
		internal void InitializeCulture(CultureInfo cultureInfo)
		{
			if (cultureInfo == null)
				cultureInfo = CultureInfo.CurrentCulture;

			this.Initialize(cultureInfo.Calendar, cultureInfo.DateTimeFormat);
		}
