...
#pragma warning restore 436
        ...
        [ThreadStatic()]
		//private static List<DynamicResourceString> _dynamicResourceStrings = null;
        private static Dictionary<DynamicResourceString.ID, DynamicResourceString> _dynamicResourceStrings = null;
...
#pragma warning restore 436
			{
                DynamicResourceString.ID id = new DynamicResourceString.ID(culture, name, args);
                DynamicResourceString drs = null;

                if (_dynamicResourceStrings == null)
                {
                    // JJD 4/15/08 - optiimization
                    // Use dictionary to search DynamicResourceStrings
                    //_dynamicResourceStrings = new List<DynamicResourceString>();
                    _dynamicResourceStrings = new Dictionary<DynamicResourceString.ID,DynamicResourceString>();
                }
                else
                {
                    //int count = _dynamicResourceStrings.Count;

                    //// look for one that matches
                    //for (int i = 0; i < count; i++)
                    //{
                    //    if (_dynamicResourceStrings[i].DoPropertiesMatch(culture, name, args))
                    //        return _dynamicResourceStrings[i];
                    //}

                    _dynamicResourceStrings.TryGetValue(id, out drs);
                }

				// since it wasn't found create a new one
                if (drs == null)
                {
#pragma warning disable 436
					drs = new InternalDynamicResourceString(ResourceCustomizerLocator.Customizer, culture, name, args);
#pragma warning restore 436

					// add it to the cache
                    _dynamicResourceStrings.Add(id, drs);
                }
...
#region Infragistics Source Cleanup (Region)
	...
	internal sealed class InternalDynamicResourceString : DynamicResourceString
	{
		...
		/// </summary>
		internal InternalDynamicResourceString(ResourceCustomizer customizer, CultureInfo cultureInfo, string resourceName, object[] args)
			: base(customizer, cultureInfo, resourceName, args)
		{
		}
		...
		/// </summary>
		protected override string GetStringFromResource(string resourceName, CultureInfo culture)
		{
#pragma warning disable 436
			return SR.GetString(culture, resourceName, null);
#pragma warning restore 436
		}
	}
