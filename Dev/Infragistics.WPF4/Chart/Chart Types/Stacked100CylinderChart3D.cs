...
namespace Infragistics.Windows.Chart
{
    ...
    /// </summary>
    internal class Stacked100CylinderChart3D : CylinderChart3D
    {
        ...
        protected override void Draw3D(Model3DGroup model3DGroup, RotateTransform3D rotateTransform3D)
        {
            ...
            int numOfPoints = GetStackedNumberOfPoints(ChartType.Stacked100Cylinder, seriesList);
            ...
            for (int pointIndx = 0; pointIndx < numOfPoints; pointIndx++)
            {
                ...
                {
                    if (seriesList[seriesIndx].ChartType != ChartType.Stacked100Cylinder || seriesList[seriesIndx].DataPoints.Count <= pointIndx)
                    {
                ...
                for (int seriesIndx = 0; seriesIndx < seriesNum; seriesIndx++)
                {
                    ...
                    if (point.Value > 0)
                    {
                        ...
                        y = StackedColumnChart3D.GetYValue(stackedStartPositive, AxisY);
                    ...
                    else
                    {
                        ...
                        x = AxisX.GetPosition(pointIndx + 1);
                        y = StackedColumnChart3D.GetYValue(stackedStartNegative, AxisY);
                        z = AxisZ.GetPosition(GetZSeriesPosition(seriesIndx, seriesList) - 0.5);
