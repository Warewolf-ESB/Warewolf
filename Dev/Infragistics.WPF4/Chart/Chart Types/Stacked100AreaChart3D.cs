...
namespace Infragistics.Windows.Chart
{
    ...
    /// </summary>
    internal class Stacked100AreaChart3D : AreaChart3D
    {
        ...
        protected override void Draw3D(Model3DGroup model3DGroup, RotateTransform3D rotateTransform3D)
        {
            ...
            for (int pointIndx = 0; pointIndx < numOfPoints - 1; pointIndx++)
            {
                ...
                {
                    if (seriesList[seriesIndx].ChartType != ChartType.Stacked100Area || seriesList[seriesIndx].DataPoints.Count <= pointIndx + 1)
                    {
                ...
                for (int seriesIndx = 0; seriesIndx < seriesNum; seriesIndx++)
                {
                    ...
                    if (val1 > 0 || val2 > 0)
                    {
                        upStart = AxisY.GetLineValue(stackedStart + val1);
                        upEnd = AxisY.GetLineValue(stackedEnd + val2);
                        downStart = AxisY.GetLineValue(stackedStart);
                        downEnd = AxisY.GetLineValue(stackedEnd);
                    }
                    else
                    {
                        downStart = AxisY.GetLineValue(stackedStart + val1);
                        downEnd = AxisY.GetLineValue(stackedEnd + val2);
