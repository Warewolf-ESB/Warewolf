...
namespace Infragistics.Windows.Chart
{
    ...
    /// </summary>
    internal class StackedBarChart3D : BarChart3D
    {
        ...
        protected override void Draw3D(Model3DGroup model3DGroup, RotateTransform3D rotateTransform3D)
        {
            ...
            int numOfPoints = GetStackedNumberOfPoints(ChartType.StackedBar, seriesList);
            ...
            for (int pointIndx = 0; pointIndx < numOfPoints; pointIndx++)
            {
                ...
                {
                    if (seriesList[seriesIndx].ChartType != ChartType.StackedBar || seriesList[seriesIndx].DataPoints.Count <= pointIndx)
                    {
                        continue;
                    }
                    ...
                    GeometryModel3D geometry = CreateBar(columnIndx, new Point3D(x, y, z), size, height, size, brush, rotateTransform3D, point.Value < 0, point, pointIndx, edge);
                    ...
                    {
                        GeometryModel3D label = CreateColumnLabel(new Point3D(x, y, z), size, height, size, brush, point.Value < 0, point, pointIndx, edge);
