...
namespace Infragistics.Windows.Chart
{
    ...
    /// </summary>
    internal class BarChart3D : ColumnBar3DShapes
    {
        #region Fields
        ...
        protected override void Draw3D(Model3DGroup model3DGroup, RotateTransform3D rotateTransform3D)
        {
            ...
            {
                // Series is not bar chart
                if (seriesList[seriesIndx].ChartType != ChartType.Bar)
                {
                    continue;
                }
                ...
                for (int pointIndx = 0; pointIndx < pointNum; pointIndx++)
                {
                    ...
                    // Create a bar for a data point
                    GeometryModel3D geometry = CreateBar(columnIndx, new Point3D(x, y, z), size, height, size, brush, rotateTransform3D, point.Value < AxisY.Crossing, point, pointIndx, edge);

                    // Create hit test info for this bar
                    SetHitTest3D(geometry, point);

                    // Add bar to the group
                    ...
                    if (point.GetMarker() != null)
                    {
                        ...
                        GeometryModel3D label = CreateColumnLabel(new Point3D(x, y, z), size, height, size, brush, point.Value < AxisY.Crossing, point, pointIndx, edge);
        ...
        protected GeometryModel3D CreateBar(int index, Point3D center, double width, double height, double depth, Brush brush, RotateTransform3D rotateTransform3D, bool negativeValue, DataPoint point, int pointIndex, double edge)
        {
            GeometryModel3D geometryModel = new GeometryModel3D();

            // The geometry specifes the shape of the 3D bar.
            ...
            // Do not draw bar if the height is 0.
            if (height == 0)
            {
            ...
            if (IsAnimationEnabled(point.GetDoubleAnimation(pointIndex)))
            {
                ...
                // Change animation for negative bars
                if (negativeValue)
                {
        ...
        protected GeometryModel3D CreateColumnLabel(Point3D center, double width, double height, double depth, Brush brush, bool negativeValue, DataPoint point, int pointIndex, double edge)
        {
            GeometryModel3D geometryModel = new GeometryModel3D();
            ...
            // Create transformations
            Transform3DGroup transformGroup = new Transform3DGroup();
            TranslateTransform3D translate = new TranslateTransform3D(center.X + height - textHeight - 2 * edge, center.Y, center.Z);
            if (negativeValue)
            {
        ...
        protected override void AddSide(Point3DCollection points, Vector3DCollection normals, PointCollection texturePoints, Int32Collection indices, double height, double edge, Point start, Point end, Point nextEnd, int segments)
        {
            ...
            // Edge Top
            this.AddPolygon(normals, points, texturePoints, indices, GetPosition(new Point(0, 0), end, height, false, edge), GetPosition(start, end, height - edge, false, edgeRight), GetPosition(start, end, height - edge, true, edgeRight), GetPosition(new Point(0, 0), start, height, false, edge), FindNormalCenter(start, end, height - edge, edge, new Vector3D(1,0,0)), Math.Abs(start.X - end.X), edge);
            ...
            // Edge Bottom
            this.AddPolygon(normals, points, texturePoints, indices, GetPosition(new Point(0, 0), start, 0, false, edge), GetPosition(start, end, edge, true, edgeRight), GetPosition(start, end, edge, false, edgeRight), GetPosition(new Point(0, 0), end, 0, false, edge), FindNormalCenter(start, end, edge, edge, new Vector3D(1, 0, 0)), Math.Abs(start.X - end.X), edge);
        ...
        private Point3D FindNormalCenter(Point start, Point end, double y, double edge, Vector3D normal)
        {
            ...
            Point3D start3D = new Point3D(y, start.X, start.Y);
            Point3D end3D = new Point3D(y, end.X, end.Y);
            Point3D mid3D = new Point3D(y, mid.X, mid.Y);
        ...
        private Point3D GetPosition(Point start, Point end, double y, bool startEdge, double edge)
        {
            if (startEdge)
            {
                ...
                return new Point3D(y, position.X, position.Y);
            }
            else
            {
