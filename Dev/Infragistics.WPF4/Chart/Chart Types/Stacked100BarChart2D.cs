...
namespace Infragistics.Windows.Chart
{
    ...
    /// </summary>
    internal class Stacked100BarChart2D : BarChart2D
    {
        ...
        protected override void Draw2D()
        {
            ...
            // Find max number of data points for 100% bar stacked chart type 
            int numOfPoints = Math.Max(GetStackedNumberOfPoints(ChartType.Stacked100Bar, SeriesList), GetStackedNumberOfPoints(ChartType.Stacked100CylinderBar, SeriesList));

            // Search series for point width. Because of cluster and stacked chart types 
            // all series have to have same point width.
            double width = ColumnChart2D.FindClusterPointWidth(ChartType.Stacked100Bar, ChartType.Stacked100CylinderBar, SeriesList, _width);
            ...
            for (int pointIndx = 0; pointIndx < numOfPoints; pointIndx++)
            {
                ...
                for (int seriesIndx = 0; seriesIndx < seriesNum; seriesIndx++)
                {
                    if ((SeriesList[seriesIndx].ChartType != ChartType.Stacked100Bar && SeriesList[seriesIndx].ChartType != ChartType.Stacked100CylinderBar) || SeriesList[seriesIndx].DataPoints.Count <= pointIndx)
                    {
                        continue;
                    }
