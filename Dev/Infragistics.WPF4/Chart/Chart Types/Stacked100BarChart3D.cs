...
namespace Infragistics.Windows.Chart
{
    ...
    /// </summary>
    internal class Stacked100BarChart3D : BarChart3D
    {
        ...
        protected override void Draw3D(Model3DGroup model3DGroup, RotateTransform3D rotateTransform3D)
        {
            ...
            int numOfPoints = GetStackedNumberOfPoints(ChartType.Stacked100Bar, seriesList);
            ...
            for (int pointIndx = 0; pointIndx < numOfPoints; pointIndx++)
            {
                ...
                {
                    if (seriesList[seriesIndx].ChartType != ChartType.Stacked100Bar || seriesList[seriesIndx].DataPoints.Count <= pointIndx)
                    {
                ...
                for (int seriesIndx = 0; seriesIndx < seriesNum; seriesIndx++)
                {
                    ...
                    if (point.Value > 0)
                    {
                        ...
                        x = GetYValue(stackedStartPositive, AxisY);
                    ...
                    else
                    {
                        ...
                        y = AxisX.GetPosition(pointIndx + 1);
                        x = GetYValue(stackedStartNegative, AxisY);
                        z = AxisZ.GetPosition(GetZSeriesPosition(seriesIndx, seriesList)- 0.5);
        ...
        internal static double GetHeightValue(double stackStartPositive, double stackEndPositive, AxisValue axis)
        {
            ...
            return axis.GetPositionLogarithmic(Math.Abs(height)) - axis.GetPositionLogarithmic(0);
        }
