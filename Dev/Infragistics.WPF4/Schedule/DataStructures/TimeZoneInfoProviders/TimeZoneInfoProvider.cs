...

{
	...
	public abstract class TimeZoneInfoProvider : DependencyObject, ISupportPropertyChangeNotifications
	{
		...
		[ThreadStatic]
		internal static string _timeZoneIdSelected;
		...
		internal void OnDefaultLocalTimeZomeChanged()
		{
			...
			if (this._localToken != null && this._localToken.Id == TimeZoneInfoProvider._timeZoneIdSelected)
				return;
			...
			if (!string.IsNullOrWhiteSpace(TimeZoneInfoProvider._timeZoneIdSelected))
			{
				...
				if (this._localTimeZoneId != null &&
					this._localTimeZoneId != TimeZoneInfoProvider._timeZoneIdSelected)
					this._localTimeZoneId = null;
			}
