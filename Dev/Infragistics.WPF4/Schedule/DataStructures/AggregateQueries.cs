...

{
    ...
    #region AggregateQueries Class

    internal class AggregateQueries
    {
        #region Member Vars

        private WeakList<ActivityQuery> _pendingQueries = new WeakList<ActivityQuery>( );
        private WeakList<ActivityQueryResult> _results = new WeakList<ActivityQueryResult>( );
        private ListScheduleDataConnector _connector;

        #endregion // Member Vars

        #region Constructor

        internal AggregateQueries( ListScheduleDataConnector connector )
        {
            ScheduleUtilities.ValidateNotNull( connector );
            _connector = connector;
        }

        #endregion // Constructor

        private int GetIntersectingQuery( ActivityQuery query )
        {
            return -1;
        }
        ...
        public ActivityQueryResult PerformQuery( ActivityQuery query )
        {
            ScheduleUtilities.ValidateNotNull( query );
            ActivityTypes activityTypes = query.ActivityTypesToQuery;

            List<ListQueryResult> queryResults = new List<ListQueryResult>( );

			if ( 0 != ( ActivityTypes.Appointment & activityTypes ) )
			{
				_connector._appointmentListManager.GetQueryResults( query, queryResults );
				_connector._recurringAppointmentListManager.GetQueryResults( query, queryResults );
			}

            if ( 0 != ( ActivityTypes.Journal & activityTypes ) )
				_connector._journalListManager.GetQueryResults( query, queryResults );

            if ( 0 != ( ActivityTypes.Task & activityTypes ) )
				_connector._taskListManager.GetQueryResults( query, queryResults );

			// Remove null entries in case any of the PerformActivityQueryHelper calls above returned null.
			// 
            CoreUtilities.RemoveAll<ListQueryResult>( queryResults, null );

            ListConnectorActivityQueryResult result = new ListConnectorActivityQueryResult( query, queryResults );
            return result;
        }
    ...
    #endregion // AggregateQueries Class 
