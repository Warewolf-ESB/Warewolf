...

{
	...
	public class DaySettingsOverride : DependencyObject, ISupportPropertyChangeNotifications
	{
        ...
        internal bool DoesDayMatch( Resource resource, DateTime date, XamScheduleDataManager dm )
        {
			DateTime thisDate = this.Date;
			if ( ScheduleUtilities.IsSameDay( thisDate, date ) )
				return true;

			DateRecurrence recurrence = this.Recurrence;
			if ( null != recurrence )
			{
				RecurrenceCalculatorFactoryBase factory = dm.RecurrenceCalculatorFactory;
				TimeZoneToken timeZone = ScheduleUtilities.GetTimeZoneToken( resource, dm.TimeZoneInfoProviderResolved );

				DateRecurrenceCalculatorBase calc = null != factory ? factory.GetCalculator(
					new RecurrenceInfo( recurrence, thisDate, TimeSpan.Zero, timeZone, this )
				) 
				: null;

				if ( null != calc && calc.DoesRecurOnDate( ScheduleUtilities.ConvertToUtc( timeZone, date ) ) )
					return true;
			}
