...

{
	...
	internal class ResourceListManager : ScheduleItemCollectionListManager<Resource, ResourceProperty>
	{
		...
		internal bool HasCalendarsDataSource
		{
			get
			{
				...
				ListScheduleDataConnector connector = _connector as ListScheduleDataConnector;
				ResourceCalendarListManager calendarListManager = null != connector ? connector._resourceCalendarListManager : null;
				return null != calendarListManager && null != calendarListManager.List;
		...
		private RelationPropertyStore<Resource, ResourceCalendar, ObservableCollectionExtended<ResourceCalendar>> CreateRelatedCalendarsStore( ResourceCollection resources, IMap<int, IPropertyStore> stores )
		{
			...
			ListScheduleDataConnector connector = _connector as ListScheduleDataConnector;
			ResourceCalendarListManager calendarListManager = null != connector ? connector._resourceCalendarListManager : null;
			ResourceCalendarCollection calendars = null != calendarListManager && null != calendarListManager.List ? calendarListManager.ResourceCalendars : null;

			PropertyStorage<ResourceCalendar, int> storage = null != calendarListManager ? calendarListManager.PropertyStorage as PropertyStorage<ResourceCalendar, int> : null;
			IPropertyStore calendarResourceIdStore = null != storage ? storage.PropertyStores[ResourceCalendar.StorageProps.OwningResourceId] : null;
			IPropertyStore resourceIdStore = null != stores ? stores[Resource.StorageProps.Id] : null;

			ResourceItemManager itemManager = (ResourceItemManager)_viewItemFactory;
			if ( null != itemManager )
				itemManager.HasResourceCalendarsDataSource = this.HasCalendarsDataSource;

			if ( null != resources && null != calendars && null != resourceIdStore && null != calendarResourceIdStore )
			{
				return new RelationPropertyStore<Resource, ResourceCalendar, ObservableCollectionExtended<ResourceCalendar>>(
					resources,
					resourceIdStore,
					calendars,
					calendarResourceIdStore,
					new DictionaryPropertyStore<Resource, ObservableCollectionExtended<ResourceCalendar>>( ),
					(IDefaultValueFactory<Resource, ObservableCollectionExtended<ResourceCalendar>>)Resource.StorageProps.Info.Instance.Props[Resource.StorageProps.Calendars]._defaultValueFactory,
					( calendar, resource ) => calendar.InitializeResource( resource ),
					ScheduleUtilities.ReplaceCollectionContents
				);
			}
		...
		private void RecreateAllActivities( )
		{
			...
			if ( null != connector )
			{
				foreach ( IListManager ii in connector._listManagers )
				{
					IActivityListManager activityListManager = ii as IActivityListManager;
					if ( null != activityListManager )
						activityListManager.RecreateAllViewItems( );
				}
			}
	...
	internal class ResourceCalendarListManager : ScheduleItemCollectionListManager<ResourceCalendar, ResourceCalendarProperty>
    {
		...
		internal override void DirtyFieldValueAccessors( )
		{
			...
			ListScheduleDataConnector connector = _connector as ListScheduleDataConnector;
			if ( null != connector && null != connector._resourceListManager )
				connector._resourceListManager.DirtyFieldValueAccessors( );
		...
		where TViewItem : ActivityBase, new( )
	{
		...
		protected override void MarkCleanPendingDirtyResults( IEnumerable<ListQueryResult> results )
		{
			...
			ListScheduleDataConnector listConnector = _connector as ListScheduleDataConnector;
			var resourceListManager = null != listConnector ? listConnector._resourceListManager : null;
			if ( null != resourceListManager )
				resourceListManager.MarkCleanPendingDirtyResults( );
