...
namespace Infragistics.Windows.Design.Ribbon
{
    ...
    /// </summary>
    public class DALLabelTool : DesignerActionList
    {
        #region Constructors
		...
		public DALLabelTool(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
			: base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_LabelTool") : alternateAdornerTitle;
			...
			DALHelpers.AddStringsGroup(typeof(LabelTool), this.Items, true, ref groupSequence);

			#endregion //Strings Group

			#region Images Group

			DALHelpers.AddImagesGroup(typeof(LabelTool), this.Items, true, ref groupSequence);

			#endregion //Images Group

			#region QAT Group

			DALHelpers.AddQatGroup(typeof(LabelTool), this.Items, false, ref groupSequence);
