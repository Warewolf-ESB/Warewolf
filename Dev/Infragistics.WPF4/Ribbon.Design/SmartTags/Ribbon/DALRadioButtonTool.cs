...
namespace Infragistics.Windows.Design.Ribbon
{
    ...
    /// </summary>
	public class DALRadioButtonTool : DesignerActionList
    {
        #region Constructors
		...
		public DALRadioButtonTool(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
			: base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_RadioButtonTool") : alternateAdornerTitle;
			...
			// Add a group of properties common to all button tools, then add a couple of extra properties.
			DesignerActionItemGroup groupButton = DALHelpers.AddButtonGroup(typeof(RadioButtonTool), this.Items, true, ref groupSequence);
			using (PropertyItemCreator pic		= new PropertyItemCreator(typeof(RadioButtonTool), groupButton))
			{
			...
			// Add a group of string properties common to all tools, then add an extra string property.
			DesignerActionItemGroup groupStrings	= DALHelpers.AddStringsGroup(typeof(RadioButtonTool), this.Items, true, ref groupSequence);
			using (PropertyItemCreator pic			= new PropertyItemCreator(typeof(RadioButtonTool), groupStrings))
			{
				this.Items.Add(pic.GetPropertyActionItem("GroupName"));
			}
			...
			DALHelpers.AddImagesGroup(typeof(RadioButtonTool), this.Items, true, ref groupSequence);

			#endregion //Images Group

			#region QAT Group

			DesignerActionItemGroup groupQat = DALHelpers.AddQatGroup(typeof(RadioButtonTool), this.Items, false, ref groupSequence);
