...
namespace Infragistics.Windows.Design.Ribbon
{
    ...
    /// </summary>
    public class DALTextEditorTool : DesignerActionList
    {
        #region Constructors
		...
		public DALTextEditorTool(EditingContext context, ModelItem modelItem, List<string> itemsToShow, string alternateAdornerTitle)
			: base(context, modelItem, itemsToShow, alternateAdornerTitle)
        {
			//You can set the title of the smart tag. By default it is "Tasks".
			this.GenericAdornerTitle = string.IsNullOrEmpty(alternateAdornerTitle) ? SR.GetString("SmartTag_T_TextEditorTool") : alternateAdornerTitle;
			...
			using (PropertyItemCreator pic = new PropertyItemCreator(typeof(TextEditorTool), groupEditors))
			{
			...
			DALHelpers.InitializeDALForEditorTool(typeof(TextEditorTool), this, this.Items, groupSequence);
		}
