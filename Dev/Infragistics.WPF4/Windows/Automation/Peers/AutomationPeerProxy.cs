...
{
	/// <summary>
	/// An <see cref="AutomationPeer"/> used to represent another automation peer.
	/// </summary>
	public abstract class AutomationPeerProxy : AutomationPeer
	{
		...
		/// <summary>
		/// Creates a new instance of the <see cref="AutomationPeerProxy"/> class
		/// </summary>
		protected AutomationPeerProxy()
			: base()
		{
		}
		...
		/// <summary>
		/// Returns the Automation Identifier for the <see cref="UIElement"/> that corresponds with the object that is associated with this <see cref="AutomationPeer"/>.
		/// </summary>
...
#region Infragistics Source Cleanup (Region)
				...
				// AS 5/11/11 TFS74791
				// The AutomationPeer class will store a reference to the List<AutomationPeer> 
				// returned. If we return the same list reference that another automationpeer 
				// is returning (in this case that of the associated element) and the list 
				// is manipulated by a derived peerproxy then really we will be manipulating 
				// the list of the associated element's peer. That was what was happening 
				// here. The RecordAutomationPeer was manipulating the base list (i.e. 
				// that of the associated recordpresenter) so if the recordautomationpeer 
				// got into its getchildrencore again but the recordpresenter didn't then 
				// when it asked for the list from the recordpresenter's peer it would 
				// have gotten the same list that it manipulated previously which would 
				// then contain the CellAutomationPeer instances it added in the last 
				// time. So really the proxy should return its own copy of the list.
				//
				//return peer.GetChildren();
				List<AutomationPeer> children = peer.GetChildren();

				if (null != children)
					return new List<AutomationPeer>(children);
			}
		...
		/// </summary>
		/// <returns>The <see cref="AutomationPeer"/> of the Label that targets this element</returns>
