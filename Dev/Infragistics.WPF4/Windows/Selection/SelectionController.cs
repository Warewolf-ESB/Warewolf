...
namespace Infragistics.Windows.Selection
{
    ...
    internal sealed class SelectionController
    {
        ...
        public void OnMouseLeftButtonDown(System.Windows.Input.MouseButtonEventArgs e)
        {
			...
			if (!wasSuccessful || Mouse.LeftButton != MouseButtonState.Pressed)
            {
				if (wasSuccessful)
            ...
            //
            //root.CaptureMouse();
            bool oldIgnore = strategy.IgnoreNextMouseMove;
            strategy.IgnoreNextMouseMove = true;
            ...
            {
                strategy.IgnoreNextMouseMove = oldIgnore;
            }
        ...
        {
            if (this._host.RootElement.IsMouseCaptured)
        ...
        static internal ISelectableItem GetSelectableItem(System.Windows.Input.MouseEventArgs e, DependencyObject root)
        {
			...
			Point pt = e.MouseDevice.GetPosition((IInputElement)root);

			// [JM BR19235 2-23-07]
			// Use a HitTestResultCallback so that we can skip elements in the Zorder stack that are marked
			// IsHitTestVisible = false (you would think that HitTest would do this by default but it doesn't)
			SelectionController._hitTestResult = null;

			// JJD 5/31/07
			// If the root is a UIelement call the InputHitTest method
			if (root is UIElement)
				SelectionController._hitTestResult = ((UIElement)root).InputHitTest(pt) as DependencyObject;
			else
			{
				// JJD 5/31/07
				// Added filter callback as well to filter out sections of the tree that are not visible
				//VisualTreeHelper.HitTest((Visual)root, null, new HitTestResultCallback(OnHitTestResultCallback), new PointHitTestParameters(pt));
				VisualTreeHelper.HitTest((Visual)root, new HitTestFilterCallback(OnHitTestFilterCallback), new HitTestResultCallback(OnHitTestResultCallback), new PointHitTestParameters(pt));
			}

			if (SelectionController._hitTestResult == null)
				return null;

           // look for an element in the parent chain that implements ISelectableElement
			ISelectableElement selectableElement = GetSelectableElement(SelectionController._hitTestResult, root);
...
//			Debug.WriteLine(selectableElement.SelectableItem.ToString());
		...
		private static DependencyObject _hitTestResult = null;
		private static HitTestResultBehavior OnHitTestResultCallback(HitTestResult result)
		{
			// If the returned element is marked IsHitTestVisible = false, continue hit testing through to 
			// elements that are lower in the zorder.
			//if (result.VisualHit is UIElement && ((UIElement)result.VisualHit).IsHitTestVisible == false)
			//    return HitTestResultBehavior.Continue;

			SelectionController._hitTestResult = result.VisualHit;
		...
		// JJD 5/31/07
		// Added filter callback as well to filter out sections of the tree that are not visible
		private static HitTestFilterBehavior OnHitTestFilterCallback(DependencyObject target)
		{
