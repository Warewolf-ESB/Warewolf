...
{
	/// <summary>
	/// A cell that represents a particular column in the header of the <see cref="XamMultiColumnComboEditor"/>
	/// </summary>

	[InfragisticsFeature(FeatureName = FeatureInfo.FeatureName_MultiColumnCombo, Version = FeatureInfo.Version_11_2)]

	public class ComboHeaderCell : ComboCellBase
	{
        #region Constructor

        /// <summary>
		/// Initializes a new instance of the <see cref="ComboHeaderCell"/> class.
		/// </summary>
		...
		public ComboHeaderCell(ComboRowBase row, ComboColumn column)
			: base(row, column)
		{
		}
		...
		protected override Style ResolveStyle
		{
			get
			{
				if (this.Style != null)
					...
					return this.Column.HeaderStyle;
			}
		}
		...
		{
			return new ComboHeaderCellControl();
		}
		...
		{
			if (this.Control != null && this.Row != null)
			{
				
				if (this.Row.IsSelected)
				{
					VisualStateManager.GoToState(this.Control, "Selected", true);
				}
				else
				{
					VisualStateManager.GoToState(this.Control, "Normal", true);
				}
