...
{
    /// <summary>
    /// A collection of <see cref="ComboColumnTypeMapping"/> objects.
    /// </summary>

	[InfragisticsFeature(FeatureName = FeatureInfo.FeatureName_MultiColumnCombo, Version = FeatureInfo.Version_11_2)]

	public class ComboColumnTypeMappingsCollection : CollectionBase<ComboColumnTypeMapping>
    {
        #region Members

        Dictionary<Type, ComboColumnTypeMapping> _usedDataTypes;

        #endregion // Members

        /// <summary>
        /// Creates a new instance of the <see cref="ComboColumnTypeMappingsCollection"/> object.
        /// </summary>
        public ComboColumnTypeMappingsCollection()
        {
            this._usedDataTypes = new Dictionary<Type, ComboColumnTypeMapping>();
        }
        ...
        protected override void OnItemAdded(int index, ComboColumnTypeMapping item)
        {
            if (item == null || item.ColumnType == null || item.DataType == null)
                throw new Exception("Invalid ColumnType Mapping");

            if (this._usedDataTypes.ContainsKey(item.DataType))
            {
                ComboColumnTypeMapping mapping = this._usedDataTypes[item.DataType];
        ...
        {
            ComboColumnTypeMapping mapping = this.Items[index];
