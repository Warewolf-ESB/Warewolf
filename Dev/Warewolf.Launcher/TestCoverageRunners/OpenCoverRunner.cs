...
namespace Warewolf.Launcher.TestCoverageRunners
{
    public class OpenCoverRunner : ITestCoverageRunner
    {
        ...
        string WriteRunnerScriptFile(string TestsResultsPath, string JobName, string OpenCoverSnapshotFile)
        {
            ...
            var commandLine = $"\"{CoverageToolPath}\" -target:\"{TestsResultsPath}\\..\\Run {JobName}.bat\" -register:user -output:\"{OpenCoverSnapshotFile}\" -filter:\"+[Warewolf*]* +[Dev2*]* -[Dev2*Tests]* -[Warewolf*Tests]* -[Dev2*Specs]* -[Warewolf*Specs]* -[Warewolf.UIBindingTests.*]*\"{mergeOutput}";
        ...
        public string InstallServiceWithCoverage(string ServerPath, string TestsResultsPath, string JobName, bool IsExistingService)
        {
            ...
            var doubleEscapedCommandLine = $"\\\"{CoverageToolPath}\\\" -target:\\\"{ServerPath}\\\" -register:user -output:\\\"{OpenCoverSnapshotFile}\\\" -filter:\\\"+[Warewolf*]* +[Dev2*]* -[Dev2*Tests]* -[Warewolf*Tests]* -[Dev2*Specs]* -[Warewolf*Specs]* -[Warewolf.UIBindingTests.*]*\\\"{mergeOutput}";
        ...
        public void StartProcessWithCoverage(string processPath, string TestsResultsPath, string JobName)
        {
            ...
            Process.Start(CoverageToolPath, $" -target:\"{processPath}\" -register:user -output:\"{OpenCoverSnapshotFile}\" -filter:\"+[Warewolf*]* +[Dev2*]* -[Dev2*Tests]* -[Warewolf*Tests]* -[Dev2*Specs]* -[Warewolf*Specs]* -[Warewolf.UIBindingTests.*]*\"{mergeOutput}");
        }
    }
}
