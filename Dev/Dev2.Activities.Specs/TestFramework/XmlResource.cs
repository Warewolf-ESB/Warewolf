...
namespace Dev2.Activities.Specs.TestFramework
{
    ...
    public static class XmlResource
    {
        ...
        {
            //Dev2.Activities.Specs.TestFramework.
            //var resourceName = String.Format("Dev2.Activities.Specs\\TestFramework\\{0}.xml", name);

            var resourceName = string.Format("Dev2.Activities.Specs.XML.{0}.xml", name);
            using (var stream = File.OpenRead(resourceName))
            {
                ...
                {
                    using (var s = new MemoryStream(File.ReadAllBytes(resourceName)))
                    {
