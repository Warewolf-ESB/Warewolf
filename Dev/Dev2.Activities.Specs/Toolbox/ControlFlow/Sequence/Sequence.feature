...
Scenario: Sending Error in error variable and calling webservice when inner activity errors
    ...
    And call the web service "http://tst-ci-remote:3142/services/OnError_WriteToFileSequence.xml?errorLog=[[error]]"
    When the Sequence tool is executed
    Then the execution has "AN" error
    And the result from the web service "http://tst-ci-remote:3142/services/OnError_ReadFromFileSeq.xml" will have the same data as variable "[[error]]"
   And the "Date&Time" debug inputs as  
	  | Input 1    | Input 2    | Input Format | Output In |
...
#this requires the execution stack work to be done once that is completed then this spec needs to come online
@ignore
Scenario: Execute a Sequence with Saved workflow, Gather System Information, Date and Time Difference, Date and Time, Random, and Format Number tools.
      Given I have a Sequence "Test"
	  And "Test" contains Gather System Info "Sys info1" as
	  | Variable | Selected  |
	  | [[test]] | Date&Time |
	  And "Test" contains Date and Time Difference "Date&Time1" as	
	  | Input1     | Input2     | Input Format | Output In | Result      |
	  | 2013-11-29 | 2050-11-29 | yyyy-mm-dd   | Years     | [[result1]] |  
	  And "Test" contains Date and Time "Date1" as
	  | Input      | Input Format | Add Time | Output Format | Result      |
	  | 2013-11-29 | 2013-11-29   | 1        | yyyy-mm-dd    | [[result2]] |
	  And "Test" contains Random "Random1" as
	  | Type   | From | To | Result      |
	  | Number | 1    | 10 | [[result3]] |
	  And "Test" contains Format Number "Fnumber1" as 
	  | number           | Rounding Selected | Rounding To | Decimal to show | Result  |
	  | 788.894564545645 | up                | 3           | 3               | result4 |
	  And it contains the underlying dropped activity is a(n) "Assignwf"
	  When the Sequence tool is executed
	  Then the execution has "NO" error
	  And the "Sys info1" debug inputs as
	  | # |          |
	  | 1 | [[test]] |	   
	  And the "Sys info1" debug outputs as 
	  | # |                   |
	  | 1 | [[test]] = String |
      And the "Date&Time1" debug inputs as  
	  | Input 1    | Input 2    | Input Format | Output In |
	  | 2013-11-29 | 2050-11-28 | yyyy-mm-dd   | Years     |
	  And the "Date&Time1" debug outputs as 
	  |                 |
	  | [[result]] = 37 |
	  And the "Date1" debug inputs as  
	  | Input      | Input Format | Add Time |   | Output Format |
	  | 2013-11-29 | yyyy-mm-dd   | Years    | 1 | yyyy-mm-dd    |	
	  And the "Date1" debug outputs as 
	  |                         |
	  | [[result]] = 2014-11-29 |
	  ...
	  | Number           | Rounding | Rounding Value | Decimals to show |
	  | 788.894564545645 | Up       | 3              | 3                |
	  And the "Fnumber1" debug outputs as 
	  |                      |
	  | [[result]] = 788.895 |
	  And the "Assignwf" debug inputs as
      | # | Variable | New Value |
      | 1 | [[a]] | 1         |
      | 2 | [[b]] | 2         |
      And the "Assignwf" debug outputs as
      | # |              |
      | 1 | [[a]] = 1 |
      | 2 | [[b]] = 2 |           
...
Scenario: Execute a Sequence with Web Service, Assign, Base Convert, Case Convert and xpath
      ...
      | [[rec().a]] | 0x4141    |
      | [[rec().a]] | warewolf  |
      And "Test" contains case convert "Case Convert2" as 
	  | Variable     | Type  |
	  | [[rec(2).a]] | UPPER |
	  And "Test" contains Base convert "Base Convert" as
	  | Variable     | From | To     |
	  ...
	  And "Test" contains xpath "xpathtool" as
	  | XML                                                                                              | Variable     | XPath                         |
	  ...
	  | 1 | [[rec().a]] =  0x4141    |
	  | 2 | [[rec().a]] =   warewolf |
	  And the "Case Tonvert2" debug inputs as
	  | # | Convert                 | To    |
	  | 1 | [[rec(2).a]] = warewolf | UPPER |
	  And the "Case Convert2" debug outputs as  
	  | # |                         |
	  | 1 | [[rec(2).a]] = WAREWOLF |
	  And the "Base Convert2" debug inputs as  
	  | # | Convert               | From | To     |
	  | 1 | [[rec(1).a]] = 0x4141 | Hex  | Binary |
	  And the "Base Convert2" debug outputs as  
	  | # |                                 |
	  | 1 | [[rec(1).a]] = 0100000101000001 |
	  And the "Web service" debug inputs as
	  | Variable      |  
	  | [[extension]] |  
	  | [[prefix]]    |  
	  And the "Web service" debug inputs as
	  ...
	  And the "xpathtool" debug outputs as 
	  | # |                    |
	  | 1 | [[firstNum]] = Two |
...
Scenario: Execute a Sequence with Database Service, Assign.
      Given I have a Sequence "Test"
	  And "Test" contains an Assign "Sequence assign" as
      | variable    | value |
      | [[rec().a]] | test    |
      | [[rec().a]] | warewolf  |
	  ...
	  When the Sequence tool is executed
	  Then the execution has "NO" error
	  And the "Sequence assign" debug inputs as
	  | # | Variable      | New Value |
	  | 1 | [[rec().a]] = | test      |
	  | 2 | [[rec().a]] = | warewolf  |
	  And the "Sequence assign" debug outputs as    
	  |   | Records                  |
	  | 1 | [[rec(1).a]] =  test     |
	  | 2 | [[rec(2).a]] =  warewolf |
	  And the "dbservicemaillist" debug outputs as
	  |                                                               |
