...
namespace Dev2.Runtime.WebServer.Controllers
{
    public abstract class AbstractController : ControllerBase
    {
        ...
        {
            var user = User;
            var r = new System.Net.Http.HttpRequestMessage();
            try
            {
                    if (isUrlWithTokenPrefix)
                    {
                        ...
                        {
                            return r.CreateWarewolfErrorResponse(new WarewolfErrorResponseArgs { StatusCode = HttpStatusCode.Unauthorized, Title = GlobalConstants.TOKEN_UNAUTHORIZED, Message = ErrorResource.AuthorizationDeniedForThisToken });
                        }
                    }
                    else
                    {
                        if (!IsAuthenticated())
                        {
                            return r.CreateWarewolfErrorResponse(new WarewolfErrorResponseArgs { StatusCode = HttpStatusCode.Unauthorized, Title = GlobalConstants.USER_UNAUTHORIZED, Message = ErrorResource.AuthorizationDeniedForThisUser });
                        }
                    }
            ...
            {
                return r.CreateWarewolfErrorResponse(new WarewolfErrorResponseArgs { StatusCode = HttpStatusCode.InternalServerError, Title = GlobalConstants.INTERNAL_SERVER_ERROR, Message = e.Message });
            }
            finally
            {
                r.Dispose();
            ...
            where TRequestHandler : class, IRequestHandler, new()
        {
            using (var r = new System.Net.Http.HttpRequestMessage())
            {
                ...
                var context = new WebServerContext(r) { Request = { User = User } };
                var handler = CreateHandler<TRequestHandler>();
                handler.ProcessRequest(context);
