...
$TestSettingsFile = "$PSScriptRoot\ServerPermissionsSecuritySpecs.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name="ServerPermissionsSecuritySpecs"
...
if ($TestList -eq "") {
	# Create full VSTest argument string.
	$FullArgsList = " `"" + $TestAssemblyPath + "`" /logger:trx /Settings:`"" + $TestSettingsFile + "`"" + " /TestCaseFilter:`"TestCategory=ServerPermissionsSecurity`""
} else {
	# Create full VSTest argument string.
