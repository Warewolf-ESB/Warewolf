...
$TestSettingsFile = "$PSScriptRoot\DotNetConnectorToolUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"DotNetConnectorToolUITests`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run DotNet Connector Tool UI Tests.</Description>
...
if ($TestList -eq "") {
	$FullArgsList = " `"" + $TestAssemblyPath + "`" /logger:trx /Settings:`"" + $TestSettingsFile + "`"" + " /TestCaseFilter:`"TestCategory=DotNet Connector Tool`""
} else {
