...
$TestSettingsFile = "$PSScriptRoot\ServerSourcesUITests.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"ServerSourcesUITests`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Server Sources UI Tests.</Description>
...
if ($TestList -eq "") {
	$FullArgsList = " `"" + $TestAssemblyPath + "`" /logger:trx /Settings:`"" + $TestSettingsFile + "`"" + " /TestCaseFilter:`"TestCategory=Server Sources`""
} else {
