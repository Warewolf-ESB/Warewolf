...
$TestSettingsFile = "$PSScriptRoot\ConflictingViewExecutePermissionsSecuritySpecs.testsettings"
[system.io.file]::WriteAllText($TestSettingsFile,  @"
<?xml version=`"1.0`" encoding="UTF-8"?>
<TestSettings
  id=`"" + [guid]::NewGuid() + @"`"
  name="ConflictingViewExecutePermissionsSecuritySpecs"
  enableDefaultDataCollectors="false"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run conflicting view and execute permissions security specs.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = "`"" + $SolutionDir + "\Warewolf.SecuritySpecs\bin\Debug\Warewolf.SecuritySpecs.dll`" /logger:trx /Settings:`"" + $TestSettingsFile + "`"" + " /TestCaseFilter:`"TestCategory=ConflictingViewExecutePermissionsSecurity`""
} else {
	# Create full VSTest argument string.
