...
$TestSettingsFile = "$PSScriptRoot\SaveDialogUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"SaveDialogUITests`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Save Dialog UI Tests.</Description>
...
$FullArgsList = " `"" + $SolutionDir + "\Warewolf.UITests\bin\Debug\Warewolf.UITests.dll`" /logger:trx /Settings:`"" + $TestSettingsFile + "`"" + $TestList + " /TestCaseFilter:`"TestCategory=Save Dialog`""
...
<AnalyseParams>
	...
	<Output>$PSScriptRoot\UITestsDotCoverOutput.dcvr</Output>
	<Scope>
		<ScopeEntry>$SolutionDir/Dev2.Server/bin/Debug/**/*.dll</ScopeEntry>
		<ScopeEntry>$SolutionDir/Dev2.Server/bin/Debug/**/*.exe</ScopeEntry>
		<ScopeEntry>$SolutionDir/Dev2.Studio/bin/Debug/**/*.dll</ScopeEntry>
		<ScopeEntry>$SolutionDir/Dev2.Studio/bin/Debug/**/*.exe</ScopeEntry>
	</Scope>
</AnalyseParams>
"@
