...
$TestSettingsFile = "$PSScriptRoot\ResourcePermissionsSecurity.testsettings"
[system.io.file]::WriteAllText($TestSettingsFile,  @"
<?xml version=`"1.0`" encoding="UTF-8"?>
<TestSettings
  id=`"3264dd0f-6fc1-4cb9-b44f-c649fef29609`"
  name="ResourcePermissionsSecurity"
...
if ($TestList -eq "") {
	...
	$FullArgsList = "/testcontainer:`"" + $SolutionDir + "\Warewolf.SecuritySpecs\bin\Debug\Warewolf.SecuritySpecs.dll`" /resultsfile:" + $SolutionDir + "\TestResults\ResourcePermissionsSecuritySpecsResults.trx /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"ResourcePermissionsSecurity`""
} else {
	# Create full VSTest argument string.
