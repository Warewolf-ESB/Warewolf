...
if ($TestList -eq "") {
	# Create full VSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:" + $PSScriptRoot + "\TestResults\ResourcePermissionsSecuritySpecsResults.trx /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"ResourcePermissionsSecurity`""
} else {
	# Create full VSTest argument string.
