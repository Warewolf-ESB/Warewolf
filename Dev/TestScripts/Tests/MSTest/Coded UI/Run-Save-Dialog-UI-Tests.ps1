...
"@ + [guid]::NewGuid() + @"
`"
  name=`"SaveDialogUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Save Dialog UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\SaveDialogUITesting.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Save Dialog`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\SaveDialogUITesting.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
