...
"@ + [guid]::NewGuid() + @"
`"
  name=`"RecordsetToolsUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Recordset Tools UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\RecordsetToolsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Recordset Tools`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\RecordsetToolsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
