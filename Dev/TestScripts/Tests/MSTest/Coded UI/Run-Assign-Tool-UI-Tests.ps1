...
$TestSettingsFile = "$PSScriptRoot\AssignToolUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"AssignToolUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Assign Tool UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\AssignToolUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Assign Tool`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\AssignToolUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
