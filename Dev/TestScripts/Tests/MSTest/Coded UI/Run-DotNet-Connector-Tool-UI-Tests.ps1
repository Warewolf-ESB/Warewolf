...
"@ + [guid]::NewGuid() + @"
`"
  name=`"DotNetConnectorToolUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run DotNet Connector Tool UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\DotNetConnectorToolUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"DotNet Connector Tool`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\DotNetConnectorToolUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
