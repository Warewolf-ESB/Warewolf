...
"@ + [guid]::NewGuid() + @"
`"
  name=`"HelloWorldMockingUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Hello World Mocking UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\HelloWorldMockingUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Hello World Mocking`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\HelloWorldMockingUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
