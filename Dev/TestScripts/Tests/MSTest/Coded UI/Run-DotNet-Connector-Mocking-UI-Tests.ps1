...
$TestSettingsFile = "$PSScriptRoot\DotNetConnectorMockingUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"DotNetConnectorMockingUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run DotNet Connector Mocking UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\DotNetConnectorMockingUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"DotNet Connector Mocking Tests`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\DotNetConnectorMockingUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
