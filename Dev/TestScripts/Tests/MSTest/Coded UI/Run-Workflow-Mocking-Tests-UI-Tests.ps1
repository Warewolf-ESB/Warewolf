...
$TestSettingsFile = "$PSScriptRoot\WorkflowMockingTestsUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"WorkflowMockingTestsUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Workflow Mocking Tests UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\WorkflowMockingTestsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Workflow Mocking Tests`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\WorkflowMockingTestsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
