...
$TestSettingsFile = "$PSScriptRoot\ServerSourcesUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"ServerSourcesUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Server Sources UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\ServerSourcesUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Server Sources`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\ServerSourcesUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
