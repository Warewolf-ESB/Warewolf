...
"@ + [guid]::NewGuid() + @"
`"
  name=`"HTTPToolsUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run HTTP Tools UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\HTTPToolsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"HTTP Tools`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\HTTPToolsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
