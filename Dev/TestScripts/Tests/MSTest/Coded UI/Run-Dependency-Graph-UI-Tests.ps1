...
$TestSettingsFile = "$PSScriptRoot\DependencyGraphUItests.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"DependencyGraphUItests`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Dependency Graph UI Tests.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\DependencyGraphUItestsResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Dependency Graph`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\DependencyGraphUItestsResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
