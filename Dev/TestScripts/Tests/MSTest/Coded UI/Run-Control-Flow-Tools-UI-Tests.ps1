...
$TestSettingsFile = "$PSScriptRoot\ControlFlowToolsUITesting.testsettings"
...
"@ + [guid]::NewGuid() + @"
`"
  name=`"ControlFlowToolsUITesting`"
  enableDefaultDataCollectors=`"false`"
  xmlns=`"http://microsoft.com/schemas/VisualStudio/TeamTest/2010`">
  <Description>Run Control Flow Tools UI Testing.</Description>
...
if ($TestList -eq "") {
	...
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\ControlFlowToolsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + " /category:`"Control Flow Tools`""
} else {
	# Create full MSTest argument string.
	$FullArgsList = " /testcontainer:`"" + $TestAssemblyPath + "`" /resultsfile:`"" + $PSScriptRoot + "\TestResults\ControlFlowToolsUITestingResults.trx`" /testsettings:`"" + $TestSettingsFile + "`"" + $TestList
}
