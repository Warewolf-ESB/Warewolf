...
| `Invoke-WwEnginePreWarm.ps1`                  | **Consumption-plan engine warm-up** — adapted from `origin/8504-Execution-Engine-Queue-Processor-End-to-end-testing`'s script of the same name (self-contained here: takes an already-minted `-AccessToken` instead of depending on that branch's unported `WwE2E.Common.psm1`). Calls a workflow's `Secure/{route}` endpoint sequentially until latency stabilizes (Phase A — proves one instance is warm), then at `-TargetConcurrency` for several rounds (Phase B — forces the Consumption plan to scale out to roughly the instance count a coming burst needs). Exists to prevent the cold-start/scale-out failure signature root-caused in `docs/ShovelBridge-Architecture.md`'s 2026-08-16 entry (Service Bus `MessageLockLost`, Roslyn `OutOfMemoryException`, cascading token-validation cancellations) — wired into `pipeline-LOADTEST.yml`'s `ShovelBridgeLoadTest_ExternalServiceBus` job immediately before the 1000-message burst. `-DryRun` prints the phase plan without any HTTP call; an incomplete warm-up only warns (non-fatal) rather than failing the pipeline. `-LoadFunctionsOnly` test hook for Pester. |
