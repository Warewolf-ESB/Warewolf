...
try
{
    ...
    ThreadPoolStartupConfigurator.Configure(ServiceBusTriggerOptions.FromEnvironment(config).MaxConcurrentExecutions);
    ...
    var host = new HostBuilder()
        ...
        //
        //   • WAREWOLF_LOGGING_CONFIG.console (WOLF-8516; formerly ENABLECONSOLELOGGING) —
        //     when true, attach a console provider so worker logs reach stdout → Azure
        //     Functions FILESYSTEM logs / Live Log Stream. When false, NO console provider is
        //     attached, so execution logs travel ONLY to the sinks selected in
        //     AddExecutionLogging (e.g. the AI SDK when appInsights=true) and never reach the
        //     filesystem logs. This is the single switch that turns the worker's
        //     filesystem/console channel on or off.
        ...
        //   (a) process stdout  → captured by the host → FILESYSTEM logs   (gated by WAREWOLF_LOGGING_CONFIG.console)
        //   (b) worker AI SDK    → Application Insights                     (gated by WAREWOLF_LOGGING_CONFIG.appInsights)
        ...
        .ConfigureServices(services =>
         {
             ...
             if (loggingConfig.RegisterApplicationInsightsSdk)
             {
                 ...
                 // worker stdout to AI at Information (severity 1) even when
                 // WAREWOLF_LOGGING_CONFIG.appInsights=false. Using a host-unrecognised name keeps
                 // host-side AI dormant, so WAREWOLF_LOGGING_CONFIG.appInsights (WOLF-8516; formerly
                 ...
                 // Opt-in only (WAREWOLF_LOGGING_CONFIG.performanceCounters, WOLF-8516; formerly
                 // ENABLEPERFORMANCECOUNTERS) — the AI SDK never registers this on
    ...
    // NOTE: with WAREWOLF_LOGGING_CONFIG.appInsights=false the host's outer capture stamp is still its own
