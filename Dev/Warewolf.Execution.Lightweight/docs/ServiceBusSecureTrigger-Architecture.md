...
| `WAREWOLF_ENTRA_CONFIG` | JSON app setting also used by the HTTP path — its `serviceBusAudience` field is the expected `aud` claim for tokens carried in Service-Bus-triggered messages, deliberately **separate** from `audience` (the HTTP audience) so the two trust boundaries can use different app registrations/scopes if desired; its `tenantId` field is reused from the existing HTTP Entra config (same tenant). **Both** writers of this setting read-merge-write it, because an app setting write is a whole-value overwrite and Azure has no JSON-merge primitive: `Enable-ServiceBusSecureTrigger.ps1` layers on `tenantId`/`serviceBusAudience` while preserving `audience`/`clientId`, and `Configure-WwExecutionAuth.ps1` (Stage 8, `Merge-EntraConfigJson`) layers on `tenantId`/`audience`/`clientId` while preserving `serviceBusAudience`. **Regression history (2026-09-03):** Stage 8 previously rebuilt the value from scratch, so every engine deploy silently deleted `serviceBusAudience`. That makes `ServiceBusEntraAuthOptions.IsEnabled` false and the trigger fails closed — dead-lettering every message with no startup error and no App Insights telemetry, the only symptom a rising dead-letter count. Both scripts now also write the setting via one quote-free `--settings @<tempfile>` argument per setting; passing a JSON value inline gets its double quotes stripped by `az.cmd` on Windows while az still reports success, storing unparseable JSON with the same fail-closed result. |
