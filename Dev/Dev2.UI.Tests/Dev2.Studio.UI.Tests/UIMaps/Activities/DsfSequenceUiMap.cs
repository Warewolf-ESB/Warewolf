...
namespace Dev2.Studio.UI.Tests.UIMaps.Activities
{
    public class DsfSequenceUiMap : ToolsUiMapBase
    {
        ...
        {
            VisualTreeWalker vtw = new VisualTreeWalker();
            UITestControl dropPoint = vtw.GetChildByAutomationIDPath(Activity, "LargeViewContent", "ActivitiesPresenter");
            var boundingRectangle = dropPoint.BoundingRectangle;
            ToolboxUIMap.DragControlToWorkflowDesigner(toolType, new Point(boundingRectangle.X + 10, boundingRectangle.Y + 10));
        }
        ...
        public List<string> GetActivityList()
        {
            ...
            VisualTreeWalker vtw = new VisualTreeWalker();
            WpfTable table = vtw.GetChildByAutomationIDPath(Activity, "SmallViewContent", "InitialFocusElement") as WpfTable;
            if(table != null)
            {
