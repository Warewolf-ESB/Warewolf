...
namespace Dev2.Studio.Dock
{
	...
	public class ContentPaneFactory : ContainerFactory
	{
		...
		/// <summary>
		/// Initializes a new <see cref="ContentPaneFactory"/>
		/// </summary>
		...
		protected virtual void AddPane(ContentPane pane)
		{
		    if (_target is DocumentContentHost)
			{
				...
				if (null == tgp)
				{
					...
					sp.Panes.Add(tgp);
					DocumentContentHost dch = (DocumentContentHost)_target;
					dch.Panes.Add(sp);
				}
			...
			else
			{
				...
				if (_target is SplitPane)
					targetCollection = ((SplitPane)_target).Panes;
				else if (_target is TabGroupPane)
					targetCollection = ((TabGroupPane)_target).Items;
