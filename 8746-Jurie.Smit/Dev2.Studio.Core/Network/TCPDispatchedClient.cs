...
namespace Dev2.Studio.Core
{
    public class TCPDispatchedClient : NetworkHost, INetworkOperator
    {
        ...
        NetworkStateEventHandler _onNetworkStateChangedCore;

        NetworkStateEventHandler _onNetworkStateChanged;

        LoginStateEventHandler _onLoginStateChangedCore;

        LoginStateEventHandler _onLoginStateChanged;
        ...
        ManualResetEventSlim _socketConnectionRecieved;

        bool _clientDetailsReveived;

        DispatcherFrameToken<object> _clientDetailsReveivedToken = new DispatcherFrameToken<object>(false);

        DispatcherFrame _frame;

        NetworkStateEventArgs _connectFrameArgs;

        LoginStateEventArgs _loginFrameArgs;
        ...
        ServerMessaging _serverMessaging;

        int _commandSerial;

        object _commandLock;

        Dictionary<int, DispatcherFrameToken<ByteBuffer>> _pendingCommands;

        #endregion

        #region Public Properties

        public AsyncPacketHandlerCollection[] Channels
        {
            get
            {
                return _channels;
        ...
        public AsyncExtensionHandler[] Extensions
        {
            get
            {
                return _extensions;
        ...
        TCPDispatchedClient(string name, bool isAuxiliary)
            : base(name)
        {
        ...
        void SetNetworkState(NetworkState state, string message)
        {
            SetNetworkState(state, true, message);
        }

        void SetNetworkState(NetworkState state, bool isError, string message)
        {
        ...
        readonly object _writerGuard = new object();

        readonly Dictionary<Guid, IDebugWriter> _debugWriters = new Dictionary<Guid, IDebugWriter>();
        ...
        void OnNetworkMessageRevieved(INetworkOperator op, ByteBuffer reader)
        {
            if(Disposed)
            {
                throw new ObjectDisposedException("TCPDispatchedClient");
        ...
        public TCPDispatchedClient CreateAuxiliaryClient()
        {
            ...
            if(result != null)
            {
                ...
                if(netArgs.ToState == System.Network.NetworkState.Online)
                {
                    ...
                    LoginStateEventArgs loginArgs = client.Login(new TcpLoginBroker(username, password));
        ...
        void RaiseNetworkStateChanged(NetworkStateEventArgs args)
        {
        ...
        void RaiseLoginStateChanged(LoginStateEventArgs args)
        {
            if(!args.LoggedIn)
            {
                CancelPending();
        ...
        {
            password = "abc123xyz";
            return Login(new TcpLoginBroker(username, password));
        }
        ...
        void LoginImpl(OutboundAuthenticationBroker broker)
        {
            if(_loggedIn)
                return;
        ...
        void LoginOperation_NetworkStateChangedCore(NetworkHost client, NetworkStateEventArgs args)
        {
            DispatcherFrame frame = null;
        ...
        void LoginOperation_LoginStateChangedCore(NetworkHost client, LoginStateEventArgs args)
        {
            DispatcherFrame frame = null;
        ...
        void ConnectImpl(string hostNameOrAddress, int port)
        {
        ...
        void OnEnvoyReturned(Envoy envoy, bool successful, string sourceHostNameOrAddress, IPHostEntry resolvedHostEntry)
        {
            int port = envoy.Port;
            envoy.EnvoyReturned -= new EnvoyReturnedEventHandler(OnEnvoyReturned);
            envoy.Dispose();
        ...
        void OnEndConnect(IAsyncResult asyncResult)
        {
            Socket socket = null;
        ...
        void ConstructConnection(Socket socket)
        {
            if(_disposing)
            {
        ...
        void ConnectOperation_NetworkStateChangedCore(NetworkHost client, NetworkStateEventArgs args)
        {
            DispatcherFrame frame = null;
