...
// Dynamic button add remove functionality
...
function createGridEditDialog(bindingRegion, gridTitle, editService, optType, webServer, payload){
	try{
		if(!gridAddCreated){
			...
			newBinding.innerHTML = "<iframe src='" + webServer +"/services/"+editService+"?operation=" + optType +"&payload=" + payload + "' style='width:100%; height:95%' scrolling='yes'></iframe>";
...
var gridOn = false;
function toggleGridEdit(bindingRegion, gridTitle, editService,op, webServer, payload){
	var dID = "gridAddDialog";

	alert(payload);
	
	createGridEditDialog(bindingRegion, gridTitle, editService, op, webServer, payload);			
	// now make a dialog
	...
	$("#" + dID ).dialog( "option", "height", 350 );
	$("#" + dID ).dialog( "option", "width", 600 );
