...
foreach ($SolutionFile in $KnownSolutionFiles) {
    if (Test-Path "$PSScriptRoot\$SolutionFile") {
        ...
        if ($SolutionParameterIsPresent -or $NoSolutionParametersPresent) {
			...
			dotnet publish "$PSScriptRoot\$SolutionFile" -c $Config -r linux-x64 --self-contained true -o "$PSScriptRoot\Bin\$OutputFolderName" --nologo -v minimal -p:NoWarn=NETSDK1194
            if ($LASTEXITCODE -ne 0) {
